...
# Bezier — IA（メニュー構想）& Issue モデル 要件定義
...
- Bezier は **「エンジニア用機能」を作らない**。output が実 repo のコード＋docs ＝ **repo そのものが統合面**。
...
- `SoR backend = repo-files（既定・canonical）| notion（後日）| …` のインターフェース。Bezier は抽象越しに読み書き。
...
```
── ephemeral（.bezier = gitignore・ローカル作業ストア）────────────
.bezier/
...
- **main は普通の PR でしか増えない**（G1'）。起票・status 変更で main を汚さない。「未着手 issue」は local `.bezier/drafts/`。コード変更が要る段で branch を切り、issue.md/spec/design/decision を **実コード(src/…)と同じ PR** で main に載せる ＝ レビュー/CI/branch protection を通り、why が what と同 commit（DEC-008）
- **status は main に持たない**（local 作業状態。後日 tracker/Notion）
- **issue ⇄ PR 相互特定**（G2）：branch 名規約 `issue/<ulid>-slug` ／ durable な PR リンクは `issue.md` frontmatter（branch/pr/base/head）／ volatile は `.bezier/issues/<ulid>/worktree.json`
- **Issue 一覧** = main の `docs/issues/`（確定・共有）＋ local `.bezier/drafts`（作業中）を UI で合流表示
...
> **DEC-008 の精緻化**: DEC-008 は「.bezier=repo内 machinery / flat docs/specs+docs/decisions」だったが、本書で **.bezier=gitignore ローカル作業ストア / docs は issue 中心フォルダ / durable は PR 経由のみ** に改訂（DEC-047 で記録）。
...
- **issue.md** frontmatter: `id`(ULID)/`title`/`screens`(Product Boardキー)/`labels`/`branch`/`pr`/`created`。本文=問題/機会＋きっかけ。**status は持たない**（H1: live=local `.bezier`、完了=「PR merged＋decision.md在り」から**導出**、UI が合成表示）
...
- **注釈→agent**: 溜まった注釈（構造化＋ペン画像）を handoff にコンパイル→agent 起動（v0.2 流用）。live ピンは ephemeral（`.bezier/issues/<ulid>/annotations`）、確定要約だけ design/notes.md に durable 化
...
|---|---|---|
| **draft** | `.bezier/drafts/<ulid>/`（local・未追跡） | branch なし |
...
- **J2 昇格点**: `.bezier/drafts/` の issue を **「着手で branch を切る瞬間」に `docs/issues/` へ昇格**（追跡ファイル化）。それまで完全 local
- **main は merge でしか増えない**（status 変更で main に commit しない）
- **J3 worktree** = `.bezier/worktrees/<ulid>/`（gitignore）に **1 issue 1個**（並行 issue＝並行 worktree・隔離）。merge/破棄で除去
- **2回の agent パス**（design→code / build→release品質）は**同じ branch/worktree** で diff が育つ。status は in-progress のまま、thread に2イベント
- **Issue 一覧 = 3ソース合成**: ①local `drafts` ②`.bezier/issues/<ulid>/worktree.json` の in-progress（worktree から folder 読む）③main `docs/issues/`（merged 履歴）
