...
# Bezier アプリ IA & 仕様駆動ループ（グレースケールWF）
...
Bezier 本体（`app/`）の情報設計。グレースケールの shadcn/ui ワイヤーフレームとして実装済み。デザインの作り込みは後工程、まず構造を固める。

## 0. 参照: Amazon Kiro（spec-driven development）
spec を用意 → タスク化 → 実装 を回すAIコーディングツール。受け入れられた背景＝**「いきなりコードでなく、仕様を中心に AI と進める」と手戻りが減り、意図が資産化される**。Bezier はこれを **デザイン/モックの発散・収束まで含めて** 一気通貫にする（Kiro はコード寄り、Bezier は Spec↔Design↔Mock↔QA↔実装 を繋ぐ）。
...
### ① 左サイドナビ（AppSidebar）
- ヘッダー: ワークスペース切替（Bezier / 三上奏太 · Personal）
