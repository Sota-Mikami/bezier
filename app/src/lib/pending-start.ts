...
//
// Flow:
//   1. Modal's Start button: createIssue() → setPendingStart(id, {message, base, imageBlobs?}) → router.push
...
export interface PendingImageBlob {
  /** Original filename. */
  name: string;
  /** Raw bytes (read via arrayBuffer before navigation — Blobs may be GC'd). */
  bytes: Uint8Array;
  mime: string;
}
