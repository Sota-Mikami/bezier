...
const JA_COMMANDS: PackCommand[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "受入基準を書き終わったら、**spec.md の末尾（受入基準セクションの直後）に必ず下記ブロックを追記**する（prose の代わりではなく、prose の後ろに追加で書く）。/bezier:map がこのブロックを読んで状態を確実に取得するために必要:",
      "```",
      "<!-- bezier:states:start -->",
      "{\"screen\": \"<ルートパス>\", \"states\": [\"<状態1>\", \"<状態2>\"]}",
      "<!-- bezier:states:end -->",
      "```",
      "状態ラベルは **lowercase-kebab-case**（例: `empty` / `error` / `loading`）。このブロックが無いと /bezier:map が本文をパースするため精度が下がる。",
    ].join("\n"),
  },
  {
    name: "map",
    description: "diff × Spec の交点から capture manifest を生成する",
    body: [
      "この worktree の変更に対して、**画面（screen）× 状態（state）のキャプチャマニフェストを生成**し、BEZIER.md に記載されている `<issue-dir>/map/manifest.json` に書き出してください。",
      "",
      "## STEP 1 — 変更ルートの導出",
      "`git diff --name-only <base-branch>...HEAD`（または `git status`）で変更ファイルを取得し、**Next.js App Router（`app/**/page.tsx`）または Pages Router（`pages/**/*.tsx`）の規則**からルートを導出してください。",
      "- `app/(group)/foo/page.tsx` → `/foo`（ルートグループは除外、動的 `[id]` は静的プレフィックスまで）",
      "- `pages/foo.tsx` → `/foo`（`api/`・`_app`・`_document`・`_error` は除外）",
      "- Next.js 以外: `routes/`・`vite.config.ts`・ルータ設定を読んでルートを推論し、`routeInferenceMethod: \"agent\"` を設定する",
      "",
      "## STEP 2 — 状態の抜出",
      "spec.md に `<!-- bezier:states:start -->` ブロックがあればそれを読む（`specStatesBlock: true` を設定）。無ければ受入基準の本文から状態ラベルを推測し、`specStatesBlock: false` を設定する。どちらも無い場合は各ルートに `\"default\"` 状態のみを設定する。",
      "",
      "## STEP 3 — エントリの記述",
      "各 `(route × state)` ペアについて ManifestEntry を書く。**reach.kind を選ぶ基準**:",
      "- `\"url\"`: ルートが **認証不要（公開）かつ URL パラメータで状態が作れる**と確認できる場合のみ。",
      "  - 認証ガードの証拠（middleware.ts の `withAuth`、`getServerSession`、Supabase RLS、NextAuth `callbacks.authorized`）があれば `\"manual\"` を使う",
      "- `\"seed\"`: `package.json#scripts` に実在するシードスクリプトがある場合のみ",
      "- `\"steps\"`: DSL に沿って書く（`click:<selector>` / `fill:<selector>:<value>` / `navigate:<path>` / `wait:<selector>` / `wait-ms:<n>`）",
      "- `\"harness\"`: テスト用ハーネスページ/フラグが存在する場合",
      "- `\"manual\"`: 上記で確認できない場合（**デフォルト**）。不確かなら manual — gap セルのほうが誤ったスクリーンショットより有益",
      "",
      "各エントリに **必ず** 次のいずれか（または両方）を付ける:",
      "- `specRef`: この状態を定義する spec.md の行番号または AC ラベル（例: `\"spec.md:42\"` / `\"AC-3\"`）",
      "- `diffRef`: このルートを変更した差分ファイルのパス（例: `\"src/app/dashboard/page.tsx\"`）",
      "",
      "confidence: `\"high\"` = ソースに直接証拠あり / `\"medium\"` = 慣習から推論 / `\"low\"` = 推測",
      "",
      "## STEP 4 — JSON の出力",
      "**JSON のみ出力**（説明文不要）。エントリ数の上限は 20 件（`source: \"diff+spec\"` を優先）。",
      "id は必ず `{routeSlug}--{stateSlug}` 形式（lowercase-kebab-case、区切りは `--`）。",
    ].join("\n"),
  },
  {
    name: "design",
...
const EN_COMMANDS: PackCommand[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "After writing the acceptance criteria, **append the following machine-readable block to spec.md immediately after the states section** (add it AFTER the prose, not INSTEAD of it). This is required so /bezier:map can reliably extract states without parsing prose:",
      "```",
      "<!-- bezier:states:start -->",
      "{\"screen\": \"<route-path>\", \"states\": [\"<state1>\", \"<state2>\"]}",
      "<!-- bezier:states:end -->",
      "```",
      "State labels must be **lowercase-kebab-case** (e.g. `empty` / `error` / `loading`). Without this block, /bezier:map falls back to prose parsing which is less reliable.",
    ].join("\n"),
  },
  {
    name: "states",
  ...
  {
    name: "map",
    description: "Generate a capture manifest from diff x Spec (screen x state targets)",
    body: [
      "Generate a **screen x state capture manifest** for this worktree's current changes and write it to `<issue-dir>/map/manifest.json` (the issue directory is specified in BEZIER.md in your context files).",
      "",
      "## STEP 1 -- Derive changed routes",
      "Run `git diff --name-only <base-branch>...HEAD` (or `git status`) to get changed files. Derive routes:",
      "- **Next.js App Router**: `app/**/page.tsx` -- route (strip route groups `(x)`, parallel slots `@slot`; stop at first dynamic `[id]` segment)",
      "- **Next.js Pages Router**: `pages/**/*.tsx` -- route (exclude `api/`, `_app`, `_document`, `_error`)",
      "- **Other frameworks**: read the router config and infer routes; set `routeInferenceMethod: \"agent\"`",
      "",
      "## STEP 2 -- Extract states",
      "Look for a `<!-- bezier:states:start -->` JSON block in spec.md and use it (`specStatesBlock: true`). If absent, extract state labels from acceptance criteria prose (`specStatesBlock: false`). If nothing found, emit a single `\"default\"` state per route.",
      "",
      "## STEP 3 -- Write each entry",
      "For every `(route x state)` pair, choose `reach.kind` conservatively:",
      "- `\"url\"`: ONLY if you can **positively confirm** the route is public (no auth middleware) AND the state is addressable via URL path/params.",
      "  - Auth guard evidence: `middleware.ts` with `withAuth`, `getServerSession`, Supabase RLS, NextAuth `callbacks.authorized` -- use `\"manual\"` instead.",
      "- `\"seed\"`: ONLY if a matching script exists in `package.json#scripts`.",
      "- `\"steps\"`: DSL: `click:<selector>` / `fill:<selector>:<value>` / `navigate:<path>` / `wait:<selector>` / `wait-ms:<n>`",
      "- `\"harness\"`: test harness page / feature flag exists in the codebase.",
      "- `\"manual\"`: **default when you cannot confirm automated reach**. An honest gap cell is more useful than a wrong screenshot.",
      "",
      "Each entry MUST include at least one citation:",
      "- `specRef`: spec.md line or AC label motivating this state (e.g. `\"spec.md:42\"` / `\"AC-3\"`)",
      "- `diffRef`: the changed file path that makes this route relevant (e.g. `\"src/app/dashboard/page.tsx\"`)",
      "",
      "`confidence`: `\"high\"` = positive source evidence found / `\"medium\"` = inferred from convention / `\"low\"` = guessed.",
      "",
      "## STEP 4 -- Output",
      "Output **ONLY valid JSON** (no prose outside the JSON). Max 20 entries; prioritize `source: \"diff+spec\"` first. The `id` field must follow the format `{route-slug}--{state-slug}` (lowercase-kebab-case, `--` separator).",
      "",
      "Schema (TypeScript):",
      "```typescript",
      ...
      "  specStatesBlock: boolean;  // whether bezier:states block was found",
      "  routeInferenceMethod?: \"static\" | \"agent\";",
      "  entries: Array<{",
      "    id: string;              // \"{route-slug}--{state-slug}\"",
      "    label: string;           // human-readable label",
      "    route: string;           // \"/dashboard\" etc. (starts with /)",
      "    state: string;           // lowercase-kebab-case",
      "    source: \"diff\" | \"spec\" | \"diff+spec\";",
      "    reach:",
      "      | { kind: \"url\";     url: string }",
