...
const META = new Map<string, SlashMeta>(
  ITEMS.map((it) => [it.title, { description: it.description, icon: it.icon }]),
);
...
/**
 * Slash-command completion source. Returns options when the caret follows a
 * "/" that begins a block or follows whitespace; CM filters the options by the
...
export function slashCommands(
  ...
  return {
    // Filter/replace region is the query AFTER the slash, so options are matched
    // against e.g. "head" (not "/head"); `apply` removes the slash itself.
    from: before.from + 1,
    options: OPTIONS,
    validFor: /^[\w-]*$/,
  };
