...
export function PendingEditsBar({
  ...
  applyLabel?: string;
  /** Optional secondary action (the mock uses it for "apply to code" alongside Save). */
  onSecondary?: () => void;
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 border-t bg-muted/40 px-3 py-1.5">
      ...
      {onSecondary && secondaryLabel && (
        <Button size="sm" variant="outline" className="h-7" disabled={busy} onClick={onSecondary}>
          {secondaryLabel}
        </Button>
      )}
