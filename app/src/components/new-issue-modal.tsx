...
//
// On Start:
//   createIssue(folder, "") → setPendingStart(id, {message, base}) → router push
//   → use-implement-session consumes the pending start and auto-fires handleStart.
//
...
export interface NewIssueModalProps {
  ...
  onSubmit: (folder: string, message: string, base: string) => Promise<void>;
...
export function NewIssueModal({
  ...
  const handleStart = React.useCallback(async () => {
    if (!canStart) return;
    await onSubmit(folder, message.trim(), base);
  }, [canStart, folder, message, base, onSubmit]);
