// Derived from onlook-dev/onlook packages/parser/src/ids.ts — Apache-2.0
// (see ./LICENSE, ./NOTICE). MODIFIED: `@onlook/constants` and `@onlook/utility`
// imports localized to ./constants. Algorithm (inject/repair/dedupe unique
// data-oid per JSX opening element, skipping React.Fragment) is verbatim.

import { EditorAttributes, createOid } from "./constants";
...
function handleSingleValidOid(
  ...
  if (
    shouldReplaceOid(oidValue, globalOids, localOids, branchOidMap, currentBranchId)
  ) {
