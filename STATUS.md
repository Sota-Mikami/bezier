<!-- 最終更新: 2026-06-26 / DEC-155〜157(v0.5.0)=①Edit 中の要素削除(Delete・Backspace=戻る バグ解消)②Comment をライブ・スクロール化(凍結廃止・ページ内オーバーレイ・印コンテンツ追従・側パネル)＋スクショ画像添付③画面収録を不要に(screencapture→WKWebView takeSnapshot)。↓DEC-151〜154(v0.4.0)=①画像添付(共有tray+lightbox・paste/drop)②Proceed等を inject 統一(no-restart)③Map/QA Beta＋Edit⊻Comment 統一 ModeToggleGroup④HTML mock Edit を opaque iframe+postMessage で修正(WKWebView same-origin 回避)。↓DEC-143〜149 = ①**Design>HTML を「動く」化**(iframe allow-scripts・same-origin無しで隔離・in-app＋共有ページ)＋プロンプト/ガイドを「単一自己完結HTML・インラインJS可」に(DEC-143)。②**⌘F 検索**=md は CodeMirror @codemirror/search・html は注入オーバーレイ(__bzfind・postMessage)(DEC-144)。③**per-issue base branch**=worktree を選んだ base から切り WorktreeRef.base に pin・Sync/Merge/PR が追従・**検索コンボピッカー**＋**自動 fetch でターミナル不要**(DEC-145・学び[[persona-no-terminal-git]])。④**Share を「リンク/エクスポート」2タブ化**＋**ZIP エクスポート**(選択ページを md/html で .zip・依存ゼロ自作 zip エンコーダ・Finder で表示)(DEC-146)。⑤**v0.2.0 を public Release で配布**=principal-engineer(security)＋qa-lead(完全性)レビュー→**repo を public 化**(playbook 等も公開)→`v0.2.0` タグ→`.dmg`(未署名・`xattr -dr com.apple.quarantine`)・出荷前修正(版整合/base即表示/export空理由/英語README/gate iframe sandbox/古CSPコメント)(DEC-147・[[bezier-release-distribution]])。⑥**Map 再設計**=Anam Hira(Revyl/元Uber)の「誰もアプリの本当の姿を知らない」を起点に、**agent が (コード diff ∩ Design 要件) から撮影対象を導出→capture manifest→worktree の実コードを screen×state で撮影**・**要件にあるが到達不能＝空セル＝coverage gap**(ピクセル比較なし)・**auth-redirect は撮らず空セル**(嘘スクショ防止)。Product 起草(ISSUE-006)→CTO/AI 両レビュー→統合→**Phase 1 実装**(`map-manifest.ts`/`/bezier:map`＋構造化`/bezier:states`/2DグリッドUI/redirect ガード・COO レビューで「reach.url でなく素のroute にナビ＝全状態セルが同画面」核心バグを修正)(DEC-148)。⑦**新規 Issue 作成モーダル**=Chat 主役＋Folder＋Branch(Spec エディタ無し・agent が Spec を書く)・**作成は Start まで遅延**(キャンセルで何も残らない)・着地後 `pending-start` で **message+base から自動 start**・DEC-083/084(作成時 repo ピッカー撤回)と整合(DEC-149)。**現状=v0.5.0 を main で公開リリース（v0.4.0 の次）。v0.4.0 からの追加（DEC-155〜157）＝①Edit 中の要素削除(Delete/Backspace・overly+main両ハンドラ・`preventDefault` で WKWebView の Backspace=戻る バグも解消・`visualEditPrompt` に deletes)②Comment をライブ・スクロール化＝凍結廃止し `bezier-overlay` v2 のページ内オーバーレイに統一(印=コンテンツ座標でスクロール追従・側パネル `live-annotation-panel`・送信時ライブ画面+印をスナップ・`use-annotation-data`)＋スクショを `![](path)` 画像添付に(#1)③**画面収録を不要に**＝`screencapture`→ObjC ヘルパーで WKWebView `takeSnapshot`(自分の web を直接スナップ=権限不要・`webview_snapshot` コマンド・`capture_region` 撤去)。Rust 変更あり→`cargo fmt`+`clippy -D warnings`(1.83) 通し(`#[allow(too_many_arguments)]`)CI 緑維持**。tsc0/eslint0/vitest107/cargo release 成功。前回 v0.4.0=DEC-151〜154(画像添付/inject 統一/Map・QA Beta+ModeToggleGroup/mock Edit を opaque+postMessage)。詳細=decisions-log DEC-143〜149＋`product/issues/ISSUE-006.md`＋`playbook/quality-reviews/2026-06-25_issue-006-*`。 / 旧↓ DEC-142 = コア価値監査 → maker-loop 刷新 → dogfood 反復（21コミット・main `19a2dc4`・origin/本番.app 反映済）。看板「会話で導く Orchestrator」を強化＝非線形ループ哲学(loopBlock・完了ゲート無し)／mock 直接編集(E-1b・iframe注入→ファイル書戻し)＋[選択｜注釈]統合(Pen/Pin合流)／コメントは溜めて走る agent に **inject**(injectToAgent・bracketed paste・再起動しない／injectOrFeedback で共通化)／md **テキスト選択コメント**(意味アンカー・XYでない・Notion風サイドバー一括)／**要素3案の土台**(必ず1HTML・横/縦はagent選択)／**terrain は追加後 dogfood で撤去**(人にもAIにもノイズ)／PR=**gh pr create --draft --body-file**(handoff全文を本文に・URL長対策・docs/handoffファイル非コミット・本物PR URL保存・前提=gh auth)／NextStepCard(agent が next-step ファイルに提案→カード)。build-vs-buy=**wrapper でよい・全体設計が価値**(BlockNote/Liveblocks 繰り延べ・native skip・履歴=git・hosting=Vercel)。競合 **Impeccable**(impeccable.style)は design-in-code バンド重なるが**継承 skill として共存**。**現ステージ=Idea→MVP・CEO 普段使い中**。**要 dogfood 検証**=inject の手触り(走るチャットに paste 入るか)＋E-1b の WKWebView 注入＋gh draft PR。詳細=decisions-log **DEC-142**＋`playbook/research/2026-06-23_core-value-cto-audit.md`／`2026-06-23_maker-loop-rework-spec.md`／`playbook/quality-reviews/2026-06-23_persona-experience-review.md`。次パス候補=要素3案カルーセル→Accept・BlockNote(round-tripゲート先行)・DSガバナンス(Priya)・共有フィードバック。 / 旧↓ DEC-141 #5 ②③ 実装（attach-first Preview 完走）=「Bezier は dev サーバを起動しない・エージェント/人が起動→Bezier が検知して自動表示」(cmux 体験)。スコープ=待機状態のみ・issue worktree 限定・web runner 限定で既存(管理起動/attach DEC-129/Live/Tauri/注釈/login DEC-120)を壊さない。②(b) Rust discover_worktree_url=macOS lsof 2パス(LISTEN loopback ポート→pid→cwd が worktree 配下のものだけ採用・非macOSは空)＋実機 lsof 出力で -F フィールド形式(p/f/n)実測検証。②(a)(b) reader=use-preview-server に新 effect(1.5s poll: (a)<issue.dir>/preview-url を readDeclaredPreviewUrl で読む=prose内URL抽出+origin正規化+loopback限定SSRF封じ → 無ければ (b) discoverWorktreeUrls → httpPing生存→adopt=attach同様 autoAttached)・managed が動けば必ず譲る(ptyIdRef/startingRef/registry を ref で生読み)・死んだら待機へ・cleanup で解放(manual URL=attach が上書き)・新 param declUrlFile(issue=previewUrlDeclPath(issue.dir)/Live未指定)。③ UI=未起動EmptyStateを待機カード(スピナ＋「dev サーバ待ち・起動すれば自動表示」＋手動URL＋「Bezierに起動させる」フォールバック＋ターミナル)へ・ヘッダ常設Startはエラー再試行(showError)のみに格下げ・autoAttached時 RunningBadge は Stop非表示(プロセス非所有 owned=false)。① プロンプト=durable ガイド bezierGuideDoc に previewBlock(JA/EN)新設＝「URLを1行だけ<issue.dir>/preview-urlに書く+ポート変われば更新+チャットでも一言」を毎ターン認識。検証 tsc0/eslint0/vitest96(+6 parseDeclaredPreviewUrl/previewUrlDeclPath)/cargo Finished・本番13:46反映(要⌘Q→再起動)・要 dogfood(代表スタックで自動表示/ターン跨ぎ・⌘Q後の背景 dev サーバ生存/既存 attach・管理・注釈・login の回帰無し/lsof 1.5sコスト)。詳細=DEC-141 追補・spec=playbook/research/2026-06-22_preview-attach-first-spec.md / DEC-132 agent-agnostic v1=あらゆるコーディングエージェント対応(CEO「Claude Codeに縛りたくない・ユーザーがローカルで使うものを自由に」)。核=AgentAdapter能力レジストリ(app/src/lib/agent-adapters.ts)＝if(id==="claude")分岐をデータ宣言(prompt渡し方/resume/contextDirs/settings hooks/headless/notify/commandsDir/conventionFiles/newline/custom template)に置換＋buildLaunch()。組込=claude(全)/codex(idle・no-resume・AGENTS.md)＋customAdapter(任意CLI・argvテンプレ{prompt}/{cwd})。①通知共通化=状態をexit+hooks(claude)+idle(非hook新規・Rust pty_statuses strategy別・had_outputガード)で統一。②起動汎用化=launchAgent/handleResume/fixWithAgentをbuildLaunch化・isClaude撤去。③切替UX=detectAgentsが組込+custom統合→既存ピッカー・codex解禁・Settingsにカスタム追加UI+能力ヒント。moatは各agentが自分の規約ファイル(CLAUDE.md/AGENTS.md/.cursorrules等)を読む=自然に汎用化。コミットad31f37/9235367/f4bcad0/e5e0d2e。claude挙動バイト一致。tsc0/eslint0/vitest75/cargo・本番18:30。延期=headless(claude限定・.env deny依存)/slash(~/.claude据置)/newline/per-issue adapterId/cursor・aider・gemini組込(今はcustom登録でカバー)。詳細=playbook/research/2026-06-19_agent-agnostic-design.md・memory[[agent-agnostic]] / DEC-131 ビジュアル編集(Edit Mode)は CP1/CP2＋R1〜R4 まで実装し本番反映済み・CEO 指示で【一旦休止】次の指示待ち。全容は decisions-log の DEC-131（CP1/CP2/R1/R2/R3/R3.1/R4 追記）・memory[[visual-editor]]・spec(playbook/research/2026-06-19_visual-editor-spec.md の各追記)に記録。要点=Edit Modeトグル＋Resize集約＋Edit⊻Annotate排他(CP1)／select+ライブstyle編集+apply-to-code エンジン(CP2・eval_with_callbackチャネル)／ペルソナ検証でFigma-feel強化(R1)／Figma/Framer風inspector(R2)／Layer全ページツリー+↑↓並べ替え(R3・WKWebViewのDnDはeffectAllowed/dropEffect必須が真因)／大ページ向け概観+選択auto-expand(R3.1)／テキスト要素のテキスト編集(R4)。反映はagent経由のみ・将来=ビルド計装で決定論をidiom横断に拡大(記録)。本番16:58。未着手候補=画面内インライン文字編集/scrub/スタイルコピペ⌘⌥C-V/トークンガバナンス(Priya・企業配布前提)。 / DEC-131 Previewにビジュアル編集(Edit Mode=Layer/Styleパネル+実コード反映·CEO「Figma/Framer風にGUIでスタイル・並び替え」)·3 agentでフルspec化(playbook/research/2026-06-19_visual-editor-spec.md)·方針=反映はまずagent経由のみ(ライブ編集→構造化intent→ユーザーのclaudeがrepoの作法でコード化·既存sendDesignFeedbackレール再利用)·将来=ビルド計装で決定論をidiom横断に拡大(Tailwind限定にしない·記録)·CP1(bdf0053·UI)=ResizeをResizeControl1ボタンに集約(base-ui DropdownMenu=role=menuでwebview上に重なる·custom W×Hのみheader)+Edit Modeトグルを Preview Header に追加+Edit⊻Annotate排他(annotation-modeにlocked追加·Edit中は⌘⇧A/Annotateトグルno-op+disabled·注釈はscreenshot凍結/Editはlive必須で同時不可)·CP2(49b08d0·エンジン)=チャネルはTauri2.11のeval_with_callback(plain evalは値返さない)→Bezier→page=embed_browser_eval/page→Bezier=embed_browser_drain(eval_with_callback→bz-editイベント·pageにIPC付与せずDEC-130のSSRF方針維持)·bezier-overlay.ts(webview注入·closed Shadow DOMの選択/hoverボックス·cssPath·computed読取·inlineライブ適用·キュー·冪等/ナビ後再注入)·use-visual-edit.ts(注入+activate/再注入·drainループ+listen·applyStyleでdiff記録·selectParent/Path·clearEdits)·visual-edit-panels.tsx(Layerパネル=祖先/選択/子クリックで階層移動·Styleパネル=curatedグループ·ライブ適用·color picker·Pending bar)·preview-paneはEdit中3ペイン(Layer|webview|Style·webview縮みResizeObserver追従)·「コードに反映」=sendDesignFeedback(visualEditPrompt,"visual_edit")/「破棄」=reload·visualEditPrompt(en/ja)=diffを要素ごとにまとめrepoの作法で書けと指示·i18n edit.*新設+ve.*·本番11:10ビルド→ditto→/Applications(要⌘Q→再起動)·tsc0/eslint0/vitest69/cargo·first cut=CEO dogfoodでUX調整前提·弱点=selector脆さ(動的class)→tag/class/text補強·根治は将来ビルド計装/Layerは近傍1階層/構造編集未対応 / DEC-130 preview/Liveエンジニアリングレビューの修正実装(P0→P1→P2·CEO「P0やって、その後P1も。P2モヤって良い」)·P0=SEC-1(Rust http_ping/http_probe_innerにloopbackゲート+readPreviewConfigのexternalUrl検証·SSRF封じ)/SEC-3(sanitizeLogTailで4000字キャップ+鍵/トークン/JWT redact+両ロケールでdevログ尾部をUNTRUSTED DATAフェンス)/QA4.B(attach cleanup でstatus/urlリセット=解除後の行き止まり解消)/PE P1-3(LiveのreloadKeyをreloadNonce駆動=パス入力+リロードボタンが死んでた)·併せPE P2-1(buildDevCommandは既知ランナーのみ--ラップ·/run/誤爆解消)+PE P2-5(parseDevServerUrlは非selfポート優先だが3210が唯一なら採用)·P1=PE P1-1/1-2/1-5(start単一フライト/reattach順序/cancel後write抑止)+PE P0-2(単一embedded-browser webviewをref-countコーディネーター化·useId識別·owner追跡·adopt時self srcへnavigate·最後のunmountのみclose·他残ればhide→生存webview adopt·RepoLiveは実active伝播·単一instance不変)+QA1.B(ready後クラッシュ=stopped+errorをエラー表示·reattach終了でもsetError)/2.B(Live Fix-with-agentでagent不在なら案内·command not found出さない)/4.A(attach URLインライン検証+無効時Save不可)/4.C(attach setupターミナルを永続キーshell:<cwd>化=閉じてもdocker compose死なない)/5.B(待機ボタン「ターミナル」表記)·QA2.C は既にsendDesignFeedbackがconfirmDialog表示済で対応不要·SEC-2(trust ゲート)は配布が近づいた時点で専用DEC化=solo CEOには摩擦のみで延期·P2=PE P2-2(reattach成功でhttpFrameBlocked復元)/P2-3(ターミナルcwdは実行中サーバーのpackageDirに追従·runningPackageDir state)/QA5.A+PE P1-4(Fix-with-agentの前pty を再spawn前+Live unmountでkill·孤児claude累積停止)/QA4.D(Issue attach待機に解除ボタン·Live parity)/QA1.A(Live readinessに「このリポジトリを確認中…」ラベル)/QA5.C(start失敗ヒントを実在しない「下のOUTPUT」でなく依存インストール/下ターミナルへ)·延期=PE P2-6(detectAppsキャッシュ·staleness)/P2-7(observer再構築·perfのみ)/QA3.A3.B(app-picker smart default·後回し可)·検証tsc0/eslint0/vitest69(+sanitizeLogTail8/+nx-run/+3210単独)/cargo·本番10:05ビルド→ditto→/Applications(要⌘Q→再起動)·コミット1527e62/abbd67b/60f6808·チーム化前TODO=featureブランチ+PR+CI必須化·usePreviewServer/Rustプローブのテスト / DEC-129 「起動済みURLを映す」アタッチモード(Docker/Rails/任意の外部スタック·CEO「mikan-for-schoolできない」=Docker ComposeのRailsで自動検出外·DEC-128の「未対応」は誤り)·PreviewConfig.externalUrl?(loopbackのみ)あればattach=devサーバー起動せずhttpPingで1.5s監視→応答でEmbeddedBrowser表示/未応答は待機·UI=Live noAppカードreframe+URL入力·Issue PreviewにExternal URL欄·attach待機カード(docker compose upで起動+ターミナル+解除)·新規isLoopbackUrl·本番20:27·tsc0/eslint0/vitest59 / DEC-128 配布前P0=検出不可で無言idle止め理由+スコープ表示(noAppカード+devCommandUnset改訂·本番19:53) / DEC-127追補2 Issue PreviewがEADDRINUSE:4001で起動失敗→恒久対応(真因=追補1でSCR devCommandに-p4001ハードコード→Live(4001)とIssue worktreeが同ポート衝突·lingui成果物messages.tsはgitignoreでworktree無し→lingui:compile && next devの複合コマンド必須)·対応=①buildDevCommandを複合コマンド対応(&&/;/||含む時は--付けず末尾に直接-p<freePort>付与·--は素のnpm run script専用)②SCR config devCommandをnpm run lingui:compile && npx next dev(ポート削除)→各プレビューに個別freePort(Live/Issue別ポート)·本番19:37反映·tsc0/eslint0/vitest57 / DEC-127追補 診断バナーの誤検知修正(CEO「ログインしたらアプリ開いたのに500バナー残る」)·真因=http_probeはログインセッション無しのサーバーGETなので認証アプリにログイン後も500を見続ける+起動時一過性500を拾った後URL変化無しで再プローブされずstale·対策=use-preview-diagnosticを起動スモークテスト化(2.5s slow再プローブでstale解消+goodページ一度見たらsettleしドーマント=ログイン後の認証ルート誤検知を止める+ready遷移でリセット)·SCRはdevを--turbo無しに切替(.bezier/config.jsonのローカルdevCommand=npm run lingui:compile && npx next dev -p 4001・package.json不変)でClerk初回ハンドシェイクが出て/→/sign-in→ログイン→アプリ表示·本番19:26反映·tsc0/eslint0/vitest56 / DEC-127 ワンクリック「Agent に直してもらう」=エラー＋preview-doctorプレイブックをユーザー自身のAgentに渡す(CEO「配布後いちいち私と相談して環境調整は無理・どうしたら見えるかをユーザーごとのAgentに指示できる状況にしたい」＋「ship時エンジニア目線で違和感ない?」)·形=診断バナーに「Agent に直してもらう」ボタン(notFound/serverError/empty)·押すとverdict＋HTTPstatus＋URL＋devログ末尾60行＋preview-doctorプレイブック(原因クラス=env不足/authゲート/proxy・ポート/backend未起動/node版+制約=最小変更・dev専用は.env.local・無関係コードは別concern・環境要因はユーザーに伝える)をユーザーのAgentへ·配送=Issueは左ChatのsendDesignFeedback·LiveはdetectAgents→下パネルTerminalでclaudeをprompt付き起動(PreviewBottomPanelにterminalSpawn/spawnNonce追加)·/bezier:doctorコマンドも追加(commandPack JA/EN・~/.claude/commands/bezier/・再利用可)·懸念1回答=Clerk修正のunauthenticatedUrlはv6正式改善でship価値ありだがサイドバーPRに混ぜず別commit/.env.localはgitignoreで不可視·プレイブックがエージェントにPR衛生を守らせる·SCRは私が直接編集せず本機能経由でユーザーAgentが直す(第1号)·変更=prompts.ts/preview-diagnostic-banner/preview-pane/repo-live/preview-bottom-panel/i18n·本番18:50反映(要⌘Q→再起動)·tsc0/eslint0/vitest56·未対応=別PR自動化/マケプレskill化/backend自動起動 / DEC-126 Live/Preview下部にVS Code風ボトムパネル(OUTPUT＋Terminalタブ)=「見る→直す」を1画面(CEO「terminalを下部に出してClaude Codeに修正依頼する流れが自然かも」·特にLiveはチャット無いので価値大)·形=1つのリサイズ可能パネルにタブ[OUTPUT][Terminal](DEC-125追補の暫定ドロワー統合)·Terminalは素の対話シェル(claude/git/npmを自分で打つ·Run Claudeボタンや自動起動は不採用)·新規preview-bottom-panel.tsx(高さ記憶bezier:preview-panel-height+ドラッグ+タブバー+×·OUTPUT=logをdark ScrollArea·Terminal=TerminalPane dynamic activeタブ時のみmount·sessionKey=shell:${cwd}で永続=タブ切替/閉じ/画面移動跨いで再アタッチ+backlog·agent pty=issue.idと衝突なし)·usePreviewServerにcwd公開(=packageCwd(worktreePath,packageDir))·preview-paneは暫定ドロワー置換(panelOpen/panelTab·ヘッダOutputボタン=トグル·バナーShow output→OUTPUTタブ)·repo-liveはready状態でトグル+本体下にパネル(not-ready readiness/SetupTerminalは不変)·webview両立=shrink-0兄弟で本体縮む→embedded-browserのResizeObserverがスロット再ミラー·overlay-freeze誘発せず·流れ=500バナーShow output→原因→Terminalでclaude修正依頼·本番17:12反映(要⌘Q→再起動)·tsc0/eslint0/vitest56·未対応=複数端末/分割/Run Claudeボタン/not-readyログ統合/500自動投入·トレードオフ=shell ptyはアプリ稼働中生存 / DEC-125追補 診断バナーの行き止まり解消=Issue PreviewにOUTPUTログ・ドロワー追加(CEO「500と出たがどうすれば分からない」·真因=Previewは稼働中ログ非表示でバナーの「check OUTPUT log below」が指す先が無かった·LiveはログありPreviewは無し)·ヘッダ「Output」トグル＋バナー「Show output」(serverError/empty時)で下部にdevサーバーログ表示·×で閉じる·preview-pane.tsx+banner(onShowLog)+i18n·本番16:48反映(要⌘Q→再起動) / DEC-125 「どんなrepoでも開ける」恒久対応=プレビュー自己診断＋モノレポ・アプリ選択＋ポート尊重(CEO「いろんな環境で必ず何かにぶつかる・場当たりでなく恒久対応を」/真因=原因は環境ごとに違うのに全部「無言の空白」に収束·httpPingはどのHTTPステータスでもtrue·frameBlockedは計算済み未表示·モノレポ先頭一致誤選択)·原則=プレビューはstatus surface(描画不能なら必ず理由＋次の一手/本当のノブは可視調整可能)·Phase1自己診断=Rust http_probe(http_frame_blockedのソケット流用·{status,frameBlocked,contentType,bodyLen})+TS verdictFor(404/5xx/空/非HTML分類·frameBlockedはDEC-120のネイティブwebviewが無視するので非発火)+use-preview-diagnostic(DEC-121のonNavigateに相乗り·400ms debounce·同一オリジンのみ·ready初回プローブ)+preview-diagnostic-banner(role=statusのshrink-0バーをwebviewスロット上に·200HTML到達で自動消滅·dismiss可)·SPAクライアント空白は対象外明記·Phase2=detectApps(root+全直下+packages/apps/prototypes1段下)+pickDefaultApp(.env.local→framework新しさ→mtime)·保存packageDirあれば優先なければsmart default·app-picker.tsxをLive/Previewヘッダに(複数時)·選択でconfig保存+再起動·SCR実証=frontend(Next13)/new-frontend(Next15.env有)/pir-client(vite)→new-frontend既定·ideas §G解消·Phase3=buildDevCommandが既存-p/--port時は重複付与せず(next dev -p4001重複解消)·新規=use-preview-diagnostic/preview-diagnostic-banner/app-picker·変更=lib.rs/preview.ts/use-preview-server/repo-live/preview-pane/i18n·本番16:08反映(要⌘Q→再起動)·tsc0/eslint0/vitest56/cargo green / DEC-124 ⌘⇧↑↓は「issueのみ」を移動(Liveをキーボードナビ対象から外す·CEO「DEC-123入れてもまだもたつく→shortcutはissueだけ開く」)·DEC-123(プローブ遅延)では不足の理由=Liveエントリに乗るとselectLive→switchTo(root変更)→RepoLiveが可視のまま再マウントしUI描画+EmbeddedBrowser再生成が走る·Liveに乗らなければ可視再マウント自体起きない=根治·変更=app-sidebar.tsxでnavTargets(live+issue)をissueNav(issueのみ·全repo描画順·検索/ページネーション踏襲·expandedゲート撤去)に置換+moveSelectionをissue専用(issue上は隣へ·ホーム上はアクティブrepo基準で↓最初/↑前repo最後·着地でexpandRepo)+issueを全repo先読みロード(従来expanded+検索時のみ·listIssuesは1IPC/repo)·i18n shortcuts.sidebar*を「次/前のIssueへ(リポジトリ横断)」に·DEC-123維持(issueナビでrepo切替してもRepoLiveは裏active=falseでprobeEnabled falseのままプローブ0)·トレードオフ=Liveはキーボード到達不可(リポ名クリックで開く)+全repo先読みで件数バッジ全表示·本番11:41反映(要⌘Q→再起動)·tsc0/eslint0/vitest44 / DEC-123 Liveの重いhealthプローブを表示時まで遅延(⌘⇧↑↓でLiveを通過してももたつかない·CEO選択=Liveはナビ対象に残し軽量化)·真因=⌘⇧↑↓のナビ列が各repoのLive→issue群を交互に並べ別repoのissueへ行くにはLiveに一度乗る→selectLive→switchTo(root変更)→RepoLiveが可視のまま再マウント→repo-healthプローブ3つ(gitFetchネットワーク/probeReadinessはNode検出でpty起動/setup FS)がクリティカルパスで走る·対応=page.tsxがactive={!detail}を渡しrepo-live.tsxがactive&200ms留まった時だけprobeEnabled=true(通過だけならタイマー解除でプローブ走らず·留まれば一拍おいて実行·離脱cleanupでfalse·3 hookにenabled=true引数追加し初回プローブ&focus再プローブを!enabledでearly-return)·影響=3 hookはRepoLive専用·RepoLiveはpage.tsx専用·EmbeddedBrowser温存とLive⇆Issue即切替は不変·トレードオフ=バッジ即時更新はLiveを開いた時に(サイドバー低速ループが追従·軽微)·本番11:01反映(要⌘Q→再起動)·tsc0/eslint0/vitest44/cargo green / DEC-122 Previewのdevコマンドが再起動で「消える」問題を恒久解決(真因2層=①該当repoはconfig.json未保存で毎回detection頼み②モノレポ(prototypes-monorepo)の一部でworktree=モノレポrootのcheckout・lenzはprototypes/lenz(2階層)でdetectDevの走査subdir[app/web/frontend/client/site]外+subPath解決が非同期で初期""のレースで一瞬worktreePath=モノレポroot→検出失敗→devCommand空·保存無しで再起動毎に振り直し·欄のnpm run dev/appは両方placeholder=空config)·FixA検出をroot(detectDev(root))フォールバック(devスクリプトはコミット済=rootは信頼源·Live無影響)·FixB解決済configを必ず<root>/.bezier/config.jsonへ永続化(差分時のみ·空新規は書かない·冪等gitignore)=次回起動でdetection走らずともディスク復元·変更=use-preview-server.tsのみ·本番10:24反映(要⌘Q→再起動)·tsc0/eslint0/vitest44/cargo green / DEC-121 Previewのルートパス入力機構を修正＋ナビでアドレスバー自動同期(真因=「今どのパスか」をwebviewから読み戻す経路が無く・chom-chomの/coach/previewはログイン必須→未ログイン時/へリダイレクトで同じ画面に戻り「変わらない」に見えた)·①自動同期=Rust embed_browser_url(wv.url())をフロントが300msポーリング→onNavigate→同一オリジンのみpathname+search+hashをpath/pathDraftへ反映(OAuth中の外部originは無視/編集中focusはdraft非上書き)·wv.url()はフルナビもSPA pushStateも反映·②直接入力ナビ堅牢化=EmbeddedBrowserのnavigateをsrc変化から切離しreloadKeyバンプ時のみ(submit/リロードのみ·自動同期path→src書換のループ跳ね返り防止·マウント初回skip)·変更=lib.rs/ipc.ts/embedded-browser.tsx/preview-pane.tsx·本番10:02反映(要⌘Q→再起動)·tsc0/eslint0/vitest44/cargo green / DEC-120 Live/Previewを埋め込みネイティブブラウザに一本化(cmux方式=Tauri unstableのWindow::add_child·1st-party top-levelなのでGoogle/Facebook等のOAuthがインラインで成立+session永続·iframe廃止)·トグル廃止で単一モード·Map内部iframeはUIに「モード」として出さない·注釈=凍結方式(ネイティブwebviewは最前面でHTML重ねられない→annotation ONで`${issue.dir}/feedback/preview-freeze.png`にスクショ→静止画+既存AnnotationLayer·webview hide·真因=capture_regionは.bezier配下&grant済パスのみ書ける)·ダイアログ(role=dialog)表示中はwebview自動退避·別窓open_live_window(▢)は保険で残置·Slack風画像添付トレイ(terminal.tsx)も同梱landed·本番08:44反映(要⌘Q→再起動)·e2e=chom-chom(Supabase redirect)でPreview/Live両方ログイン+注釈凍結+ダイアログ退避をCEO dogfoodで確認·【追補】z順=オーバーレイ(dialog/menu/listbox)が実際に重なる時だけ退避+白消し回避でlive→凍結スクショに差し替え/はみ出し=デバイスをペインにキャップ/ショートカット完全優先=⌘K⌘⇧A⌘NをネイティブメニューGoのaccel化→menu-shortcut-bridgeが合成keydownへ変換(既存ハンドラ無改変)·【backlog G】モノレポ複数フロントでLiveが古いfrontend自動選択→app自身auth/invalid-api-key=アプリ選択UI未対応(ideas §G) / DEC-119 trybezier.com にLP暫定公開（site/＝Vercel project bezier/trybezier·Cloudflare DNS A@→76.76.21.21+www CNAME→cname.vercel-dns.com·両方DNS only）＋waitlistフォーム→Notion DB「Bezier Waitlist」(/api/waitlist=route.ts·NOTION_TOKEN/NOTION_WAITLIST_DB_ID env·内部インテグレーションtrybezier-waitlistをDB接続)·配線をcommit 9eadcefで恒久化(e89a2ecがendpoint:""へ戻していた)·apex/www HTTPS200+SSL+本番e2e実証·再デプロイ=cd site && vercel --prod --yes·次フェーズ=Bezier自身でLP反復改善（メモ=trybezier-lp-waitlist-deploy） / DEC-114 共有体験修正5点(アプリdeploy=vercel project add先行作成+--project小文字·Specチェックボックス/ネスト·Made with Bezierはfooterのみ·タブ横スクロール·共有ページにショートカット=CSP sha256許可)·deployはリモートビルドまで到達·ログ可読化+Inspectリンク·ビルド失敗の真因=VITE_APP_ENV未注入を解消·恒久対応(Option B)=Vercelプロジェクトにenv登録(vercel_sync_envでrepo全.envをproductionにupsert·同意ボタン)+deployを--prodで安定URL·ペルソナ検証でenv編集UIも破綻→エージェントがenv判断(ヘッドレスclaude -pがコミット設定だけ読む·.envはdeny物理遮断=秘密はAI非経由·parseで二重防御)·ペルソナはenvゼロ操作(初回同意1回→判断→Vercel登録→--prodデプロイ·2回目以降ワンクリック)·エージェント厳密値プロンプト+自己修復·【根本解】remote build連鎖failの真因=Vercelクリーン環境にアプリのネイティブ前提(.env/git/envモード)が次々欠ける→publish()をローカルビルド→成果物static deployに作り替え(worktreeでbuild·env焼込み·秘密はどこにも送らない·SPA rewrite·新規-webプロジェクト·〜2s build+〜15s deploy)·worktreeをgrant=出力検出修正(df5beab)·本番14:48反映 / DEC-113 dogfood体感改善3点(チャット色=pty TERM/COLORTERM·Live切替の重さ=iframe常時マウント·サイドバー縦移動⌘⇧↑↓)·本番09:28反映 / DEC-112 「入り口(Live)」実装完走＋どんなwebスタックでも開ける堅牢化(出力URL検出·nvm最新Nodeフォールバック·packageDir検証自己修復·PATH/agent再検出·devスクリプト名拡張·iframe不可→ブラウザ案内·Issue Preview worktreeへ.env symlink·Nodeピンを.node-versionでも読む)·全本番反映済(23:09ビルド) / 「またエラー」の真因=コードでなく古いバイナリ稼働(動作中プロセス21:54起動<修正22:21·ditto後は⌘Q→再起動まで案内必須) / DEC-111 repo readiness設計 / DEC-108 i18n 全部日英 / DEC-107 i18n基盤 / DEC-106 CPを履歴ドロワーへ / 中央IA確定(Design/Prototype 2エリア·Map·グローバル注釈モード) / DEC-105 Document View・状態=受入基準・Clarify=skillマーケット·Implement=Preview -->
# Bezier — 現在地
...
- **✅ 本番 `.app` 反映済（2026-06-17 10:43 ビルド）**: IA確定 / Map / 注釈モード / CP移設 / i18n全部日英 / repo readiness（DEC-111）/「入り口(Live)」堅牢化＋worktree env mirror＋`.node-version`（DEC-112）/ dogfood 体感改善3点（DEC-113）に加え、**共有体験修正（DEC-114）＝アプリ deploy（`project add` 先行作成＋`--project`）が**リモートビルドまで到達**＝Bezier 側の deploy ブロッカーは全解決・Spec チェックボックス/ネスト・badge は footer のみ・タブ横スクロール・共有ページにショートカット・deploy ログ可読化＋失敗時 Vercel Inspect リンク・`VITE_APP_ENV` 未注入を解消（`collect_public_env`）・**恒久対応＝Vercel プロジェクトに env 登録（Option B・`vercel_sync_env`＝repo 全 `.env` を production に upsert・同意付き「Vercel に env を登録」ボタン）＋ deploy を `--prod`＝安定 URL `<project>.vercel.app`**・**ペルソナ検証（Mai/Kenji）で env 編集 UI も「使えない」と判明 → エージェントが env を判断する形に（`c2dacdb`）**: 共有時、ヘッドレス `claude -p` が**コミット済みビルド設定だけ**を読んで公開 env を判断（例 `VITE_APP_ENV=development`）。`.env` は **deny で物理遮断＝秘密は AI に届かない**（実機検証：BLOCKED＋迂回拒否）。ペルソナは **env をゼロ操作**（初回同意1回 → 判断 → `publish-env.json` 永続 → Vercel 登録（秘密は Rust 経由）→ `--prod` デプロイ。2回目以降ワンクリック）まで全て反映済み（13:47 ビルド）。**判断＝AI（公開知識）／値・秘密＝Bezier（AI 非経由）** に分離。e2e 検証済み。エージェントは **enum 等の有効値を verbatim**で出す（略語禁止・3/3 安定）＋ **ビルド失敗時はエラーを差し戻して自己修復**（`97dca56`）。日常使いの `.app` が最新。再ビルド時は `npm run tauri -- build --bundles app` → `/Applications` へ **`ditto`**（[[bezier-prod-app-for-daily-use]]・`rm -rf /Applications` は権限拒否されるので ditto で in-place 置換）。反映確認は**バイナリ mtime**＋新コマンド grep。⚠️ **ditto はファイルだけ差し替え、起動中アプリはメモリ上の旧コードを使い続ける → 反映後は必ず「⌘Q→再起動」まで案内**（2026-06-16 夜の「またエラー」はこれが真因＝動作中プロセスが修正前 21:54 起動だった）。[[prod-update-timing-dogfood]]。
...
## ▶ 2026-06-24〜25 セッション（DEC-143〜149 — 動くHTML/⌘F → base branch → Share+ZIP → **v0.2.0 public 配布** → Map 再設計 → 新規作成モーダル）

- **きっかけ（CEO・複数トピック連続）**: ①「Design>HTML が動かない、Bezier が悪いのか HTML か」②「⌘F 検索が欲しい」③「未リリース機能を base 指定して直したい」④「配布用にバージョン上げてリリース。security と仕様抜け漏れチェック」⑤「Map のあるべき姿を提案して（Anam Hira の X）」⑥「Issue 新規作成 UX をよくしたい」。
- **流れ**:
  - **DEC-143/144（動くHTML＋⌘F）**: 診断＝「Design>HTML は意図的に静的ワイヤー固定（`sandbox=""`）でエージェントの JS が剥がれていた」。CEO 判断で **3面（md/html/prototype）維持のまま HTML 面を `allow-scripts`（隔離）化**＋プロンプト/`BEZIER.md` ガイドを「インライン JS 可」に。共有ページ（journey.ts）も CSP を hash→`'unsafe-inline'`＋design iframe を allow-scripts（opaque origin・default-src none で fetch 等遮断）で安全に。⌘F は md=CodeMirror search・html=注入オーバーレイ。**学び＝プロンプト変更は「prod 再ビルド＋再起動」＋「既存 issue の on-disk `BEZIER.md` は次 handoff で再生成」の両方が要る**。
  - **DEC-145（per-issue base branch）**: Bezier は元々 main 固定でなく「repo の現在ブランチ」追従だったが per-issue 固定が無かった → worktree を選んだ base から切り `WorktreeRef.base` に pin。**検索コンボピッカー**＋**自動 fetch でターミナル不要**（CEO「ペルソナは terminal git を触れない前提」→[[persona-no-terminal-git]]）。Rust `git_worktree_add` に base 引数＋`git_list_branches` 新設。
  - **DEC-146（Share 2タブ＋ZIP）**: Share ポップオーバーを「リンク/エクスポート」2タブに。エクスポート＝選択ページを md/html で .zip（**依存ゼロの自作 zip エンコーダ** `lib/zip.ts`・STORE 方式・ラウンドトリップ test 付き）＋保存ダイアログ＋Finder で表示。Rust 不要（既存 `saveFileDialog`/`writeFileBytes` 再利用）。
  - **DEC-147（v0.2.0 public 配布）**: principal-engineer（security・**ブロッカー無し**）＋qa-lead（完全性・B-1=Feedback/DL が private repo で塞がる）を並行レビュー。CEO 決定で **repo を public 化**（→Release DL＋Feedback issues 開通・`playbook/` 等も公開）＋**v0.2.0**。履歴シークレットスキャン clean。出荷前修正（版整合 0.2.0／base ピッカー即表示／Export 空理由／英語 README＋前提／gate iframe sandbox／古い CSP コメント）。`git tag v0.2.0` で `release.yml` が arm64 `.dmg` を GitHub Release に添付（**未署名・`xattr -dr com.apple.quarantine`**）。**GOTCHA: public 化直後のタグ push が一時 403「repository is disabled」→ 再 push で成功**。
  - **DEC-148（Map 再設計・Phase 1）**: CEO の削り込み＝❌全アプリ・インベントリ／❌ピクセル・パリティ／❌CI テスト／❌モックのスクショ／❌Live。✅ per-issue の coverage Map＝**worktree の実コードを screen×state で俯瞰**。方針＝**agent が (diff ∩ 要件) から撮る場所を導出→manifest→既存 authed-webview で撮影・要件にあるが到達不能＝空セル＝gap・auth-redirect は撮らず空セル**。Product 起草(ISSUE-006)→CTO/AI 両レビュー→統合→Phase 1 実装。**COO レビューで核心バグ修正（`reach.url` でなく素の route にナビ＝全状態セルが同画面になる致命傷）**。
  - **DEC-149（新規作成モーダル）**: New/⌘N → フォーカスしたモーダル（Chat 主役＋Folder＋Branch・Spec エディタ無し）。作成は Start まで遅延・着地後 `pending-start` で message+base から自動 start。DEC-083/084（作成時 repo ピッカー撤回）と整合。
- **配布状態**: **v0.3.0 を main で公開リリース**（v0.2.0=`1ce921d` の次）。Map Phase 1(DEC-148)＋新規作成モーダル(DEC-149/150)込み。前回 v0.2.0 = https://github.com/Sota-Mikami/bezier/releases/tag/v0.2.0 。
- **モーダル UX 磨き（DEC-150・CEO dogfood 反復で確定）**: 当初の自作スタイル→「フォントもレイアウトも変」→ **Linear の New issue を参考**に作り直し。①`backdrop-blur` 撤去（裏の webview をぼかす合成が「出る前の重さ」の主因）→軽いディム＋fade/zoom 入場で軽量化 ②閉じる✕・prominent な hero 入力・プロパティピル ③**`BaseBranchPicker` の `ml-auto` ハードコードで Base が中央に飛ぶ崩れ**を修正（呼出側で整列制御）④**汎用 `SearchableSelect`（ui/）を新設**し、folder（旧ネイティブ select）と branch を**同じ検索ドロップダウン**に統一＝folder も検索可能＋「フォルダを開く…」アクション、branch は**下向き**に。CEO「良い感じ」で確定。
- **再開ガイド**: Map は chat で `/bezier:states → /bezier:map → Map タブ`、新規作成は `⌘N`。次パス＝Map の自動再撮影/コマンド送信ボタン/seed・steps 実行（DEC-148）。
- **次パス候補（DEC-148/149 記録）**: Map=ターン終了の自動再撮影・ボタンからコマンド送信・seed/steps 実行エンジン・セルクリックで in-app ナビ。配布=Developer ID 署名＋notarize＋自動更新（option B）。security follow-up=`symlink`/`clone_dir` に `ensure_granted`・agent 未ログイン検知。

## ▶ 2026-06-23 セッション（DEC-142 — コア価値監査 → maker-loop 刷新 → dogfood 反復 / main `19a2dc4` マージ済）

- **きっかけ（CEO）**: 「『PdM/デザイナーのための AI Agent Orchestrator』というコア価値が実現できているか CTO として調査して」。
- **流れ**: ① CTO コア価値監査（6エリア実コード接地）→ ② build-vs-buy 合意（wrapper でよい）→ ③ maker-loop 刷新を実装 → ④ ペルソナ自己レビュー → ⑤ **CEO dogfood で多数の方向修正**（terrain 撤去・送信は inject・コメント一括・Pen/Pin 合流・PR=gh draft 等）→ ⑥ クリーンアップ → ⑦ **main にマージ＋origin push**。**全段 tsc0/eslint0/vitest105/cargo・本番 `.app` 反映済**。
- **要点（詳細は DEC-142）**:
  - **E-4 非線形ループ哲学**（loopBlock・完了ゲート無し・次の一手を提案）。**状態機械は不採用**（CEO「Spec 完了 という状態はない」）。**terrain は追加→ノイズ判定で撤去**。
  - **送信＝走る agent に inject（再起動しない）**＝`injectToAgent`＋`injectOrFeedback`。**コメントは溜めて一括**（md=テキスト選択コメント＝意味アンカー／mock=要素コメント／pin=「Instruct all at once」撤去）。
  - **mock Edit 統合**（要素 Pick→コメント/テキスト/スタイル＋**[選択｜注釈]** ＝Pen/Pin 合流）。**要素3案の土台**（必ず1HTML・横/縦 agent 選択）。**E-1a** モック repo 忠実度＋**alt3 ブランド忠実化**。**IA**=モードをサーフェス別バーへ・Annotate→Comment。
  - **PR=`gh pr create --draft --body-file`**（handoff 全文を本文に・docs/handoff ファイル非コミット＝実エンジニア指摘・URL 長対策・本物 PR URL 保存）。前提＝`gh auth`。
  - **NextStepCard**（agent が `<issue.dir>/next-step` に提案→チャット上カード＋ワンクリック続行）。
- **🔜 要 dogfood（次回 CEO 普段使いで確認）**: ① **inject の手触り**（走るチャットに paste が入り送信されるか＝全体 linchpin）② **E-1b の WKWebView 注入**（要素選択→スタイル反映→保存）③ **gh draft PR**（URL 長エラー解消・要 gh ログイン）。
- **次パス候補**: 要素3案カルーセル→Accept→書き戻し（Impeccable 完全版）／BlockNote（round-trip ゲート先行）／DS ガバナンス（Priya）／共有フィードバック。
- 競合分析: **Impeccable（impeccable.style）= 継承 skill として共存**（memory [[impeccable-competitor]]）。

## ▶ 2026-06-22 セッション（DEC-141 #5 ②③ — attach-first Preview を完走・本番反映）
- **きっかけ（CEO）**: 「STATUS 読んで #5 ②③ を進めて」。設計＝`playbook/research/2026-06-22_preview-attach-first-spec.md`（前セッションで issue-correct に確定）。① は既に出荷済。
- **方針**: 「Bezier は dev サーバを起動しない・**エージェント/人が起動→Bezier が検知して自動表示**」(cmux)。**スコープを「待機状態のみ」に限定**し、既存（管理起動 / attach DEC-129 / Live / Tauri / 注釈・login DEC-120）を壊さない。
- **②(b) Rust `discover_worktree_url(worktree)`**（`lib.rs`＋handler 登録）: macOS `lsof` 2パス＝①`-nP -iTCP -sTCP:LISTEN -Fpn` で **loopback/ワイルドカードの LISTEN ポートを pid 別収集**（`f`行は無視・LAN IP 除外）→②`-a -d cwd -p <csv> -Fn` で **各 pid の cwd**→**canonical cwd が worktree 配下のものだけ**採用→`http://localhost:<port>/` 昇順。非macOS/不一致は空（エラーにしない）。**実機 lsof 出力で `-F` 形式（p/f/n）実測検証済**。
- **②(a)(b) reader（`use-preview-server.ts`）**: 新 effect（**issue worktree 限定** `worktreePath!==root`・**web runner 限定**・`!externalUrl`）が 1.5s ポーリング＝**(a) `<issue.dir>/preview-url`**（`readDeclaredPreviewUrl`／純関数 `parseDeclaredPreviewUrl`＝prose 内 URL 抽出・origin 正規化・**loopback 限定=SSRF 封じ**）→無ければ **(b) `discoverWorktreeUrls`**→`httpPing` 生存→**adopt**（attach 同様 `setUrl+ready`・`autoAttached=true`）。**managed が動けば必ず譲る**（ref 生読み）・死んだら待機へ・cleanup で解放。新 param `declUrlFile`。
- **③ attach-first UI（`preview-pane.tsx`）**: 旧「未起動 EmptyState」→**待機カード**（スピナ＋「dev サーバ待ち・起動すれば自動表示」＋手動 URL＋「Bezier に起動させる」フォールバック＋ターミナル）。常設ヘッダ Start は**エラー再試行のみ**に格下げ（`showError`）。auto-attached の `RunningBadge` は **Stop 非表示**（プロセス非所有 `owned=false`）。
- **① プロンプト強化**: durable ガイド `bezierGuideDoc` に **「プレビュー」節（`previewBlock` JA/EN）**新設＝「URL を1行だけ `<issue.dir>/preview-url` に書く（ポート変われば更新）＋チャットでも一言」を**毎ターン**認識。intro の preview 文も更新。
- **検証**: tsc0/eslint0/**vitest96（+6）**/cargo Finished。**本番ビルド→/Applications 反映（13:46・要 ⌘Q→再起動）**。詳細＝DEC-141 追補。
- **🔜 要 dogfood**: 代表スタック（Next 3000 / Vite 5173 / Docker:3000 / mikan-for-school 等）で「エージェント起動→自動表示」／ターン跨ぎ・⌘Q 後の背景 dev サーバ生存／既存 attach・管理・注釈・login の回帰無し／**lsof 1.5s ポーリングのコスト**（待機中＋visible のみ・必要なら間引き）。
...
- **実装**: Rust `embed_browser_open/set_bounds/navigate/hide/close`（`embedded-browser` を 1 枚・label 取得）。`embedded-browser.tsx`＝スロット矩形を観測して webview をミラー＋非表示時/モーダル時は自分で hide（ネイティブは CSS 無視）。Preview/Live 両方を embedded に。
- **トレードオフ対策**: 注釈=凍結方式（`.bezier/feedback` にスクショ→静止画+AnnotationLayer。真因＝`capture_region` は `.bezier`&grant 必須だった）／デバイス枠=サイズのみ／ダイアログ=`role=dialog` で自動退避／Map=iframe のまま（UI に出さない）／キーボード=Annotate ボタンで回避。
- **同梱 landed**: Slack 風画像添付トレイ（前セッション分）＋DEC-119 doc（i18n 跨りで分離不可）。
- **検証**: tsc 0 / eslint 0 / vitest 44 / cargo Finished / next build green。**本番 08:44 反映**。e2e=CEO dogfood で **①Preview ログイン ②Live ログイン ③注釈の凍結 ④ダイアログ自動退避 ⑤単一モード** を全て確認 → コミット（`2d295ae`）。
- **追補（同日・別コミット）**: dogfood で出た 2 バグ（ネイティブ webview の最前面＆非クリップ由来）を恒久対策。①z順＝オーバーレイ(dialog/menu/listbox)が**実際に重なる時だけ**退避＋「白消し」をやめ **live→凍結スクショに差し替え**（modal/dropdown を静止画の上に）。②はみ出し＝デバイスプリセットを**ペインにキャップ**。③**ショートカット完全優先**＝⌘K/⌘⇧A/⌘N を**ネイティブメニュー(Go)の accel** 化→`menu-shortcut-bridge` が既存ハンドラ向け合成 keydown に変換（CEO「Bezier 完全優先・アプリの shortcut は ↗ 外部ブラウザで」）。tsc/eslint/vitest 44/cargo green・本番反映・CEO 確認済。
- **次セッションへ（backlog G）**: モノレポ（ルート package.json 無し・複数フロント）で Live が古い `frontend/`（Next13・env無）を自動選択→app 自身が `auth/invalid-api-key`。現行は `new-frontend/`（Next15・env有）。**アプリ選択 UI＋env 未設定検知**＝`playbook/ideas-backlog.md` §G に記録（理解のみ・実装後日）。
