#!/usr/bin/env node
/**
 * bezier ISSUE-004 — 汎用 preview ジェネレータ
 *
 * 使い方:
 ...
 *
 * 出力:
 *   <repoPath>/src/app/bezier-preview/page.tsx   (throwaway)
 *   <repoPath>/src/app/bezier-preview/layout.tsx (throwaway)
 *   <repoPath>/.gitignore に bezier-preview を追記
 *
...
function detectProviders(repoPath, appDir) {
  ...
  if (content.includes("CommentContextProvider")) {
    providers.push({
      ...
      // From preview dir (src/app/bezier-preview/), the comments dir is at ../comments/
 ...
 * preview ルート: src/app/bezier-preview/page.tsx
...
function fileToImportPath(file, hasSrc) {
  ...
  // app/components/Foo.tsx → ../../components/Foo (from app/bezier-preview/)
...
/**
 * bezier ISSUE-004 — 汎用 preview ページ (throwaway)
...
class ErrorBoundary extends Component<EBProps, EBState> {
  ...
  componentDidCatch(e: Error, info: ErrorInfo) { console.warn("[bezier preview] ErrorBoundary caught:", e.message); }
...
export default function BezierPreviewPage() {
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', fontFamily: 'sans-serif', background: 'var(--color-bg, #fff)', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'var(--color-primary, #f97316)', padding: '6px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        <span style={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>
          bezier preview — ${sceneGraph.label || indexName}
        </span>
...
${closeTags}`
  ...
  return `/**
 * bezier ISSUE-004 — standalone preview layout (throwaway)
 * generated by: spike/generate-preview.mjs
...
export const metadata: Metadata = {
  title: "bezier preview — ${indexName}",
};
...
// ─── write files ─────────────────────────────────────────────────────────────
// NOTE: ISSUE-005 以降、bezier-preview/ ディレクトリの作成・gitignore・AuthGate bypass は
//       ShimEngine (shim-engine.mjs) が担う。ここでは ShimEngine が作成済みのディレクトリに
//       page.tsx / layout.tsx を書き込むだけ。

const previewDir = path.join(appDir, "bezier-preview");
...
console.log(`\n=== 完了 ===`);
console.log(`  preview URL: http://localhost:<PORT>/bezier-preview`);
