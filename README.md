...
3. **初回起動だけ**: 未署名ビルドのため macOS が「開発元を確認できない」と出ます → **システム設定 → プライバシーとセキュリティ → 「このまま開く」** で一度許可

> 現状は **Apple Silicon・未署名（adhoc）** ビルドです。Developer ID 署名＋notarize＋自動アップデートは配布を広げる段で対応します。ソースからビルドする場合は [開発](#開発) を参照。
