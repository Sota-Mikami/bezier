...
## 追記 — R3（2026-06-19）: Layer パネルを「全ページツリー」化 ＋ ↑/↓ 並べ替え

CEO「Layer パネルは選択物に関連するものだけでなく**ページ全体のレイヤーを常時表示**し、選択中がハイライトされるだけにして。↑/↓ でレイヤー上の上下と順番が入れ替わるようにして」（左目L 選択 → ↓ で 右目R と入替）。

**実装（commit 448c02d）**:
- overlay `buildTree()` がページ全体を直列化（script/style/head/overlay 除外・800 ノード/深さ 16 上限）し、`tree` イベントを activate/select/reorder で emit。Layer パネルは旧「祖先/選択/子」近傍ビューを廃し、**全ページの再帰ツリー**（展開折りたたみ chevron・タグ別アイコン・選択=bg-accent＋自動スクロール・同親内ドラッグ並べ替え）に。
- **↑/↓ で選択要素を兄弟内で移動**（`moveSel(±1)`）。**キー処理は overlay 側（in-page）**＝プレビューで要素をクリックした直後（webview フォーカス）でも効く（Bezier 窓の keydown は webview フォーカス時に来ないため）。Bezier 側 keydown でも補助（レイヤー行フォーカス時）。ページ入力中は無効。
