...
# continuum 収益モデル提案 — local-first open-core の課金設計
...
daemon を自分で動かせない人向けに、continuum クラウドがエンジンを代行実行する。
- **BYOK**: ユーザーが自分の API キーを貼る（透明・安価・continuum はホスト代のみ）
- **Managed**: continuum 自身の Anthropic API キーで走らせ、**トークン従量＋マージン**
...
- continuum の thesis は「一人の maker が回す」。だが*価値が拡張する瞬間*は **maker が PM/Designer/Eng を招いて境界が溶ける時**。
...
- `playbook/strategy/2026-06-04_continuum-thesis-v1.md`（固い問題 + Sierra + ステージ）
