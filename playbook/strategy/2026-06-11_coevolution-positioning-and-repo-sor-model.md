...
# continuum — 共進化ポジショニング & repo-as-SoR データモデル（v0.5 設計）
...
> 3. **repo-as-SoR データモデル**（docs/ + .continuum/ 分離 / worktree / drift harness）（DEC-008）
...
3つのツール観測が、continuum の輪郭を逆照射した。**いずれも競合でなく「部品/パターンとして吸収する対象」**。

| 観測 | 正体 | continuum との関係 |
|---|---|---|
...
**結論**: 彼らは「生成」「準拠」「注釈→markdown」で**止まる**。continuum はそれらを入口/部品にして、**注釈→編集を実行し、決定として台帳に刻む**（レイヤC）。この最後の一段が誰にも無い核。
...
### 1.2 コア価値（一行）
> **continuum ＝ デザイナー&PM の「意図を入れて、判断を出す」層。エージェント開発ループを上から包み、taste を "エージェントが実行できるコンテキスト" に変換し、エージェントの出力を "生きた決定の記録" に変える。エンジンは持たず、ハンドルとログブックを握る。**

continuum はエンジン（コード生成）を作らない。握るのは**ループの両端＝入口（意図→コンテキスト＝Annotation §6）と出口（diff→判断→記録＝決定SoR §4）**。
...
→ **機械との継ぎ目は全部オープンプロトコル（MCP / markdown / git）、人間との継ぎ目だけ continuum 独自UI（Canvas + Annotation + Pen）。**「Claude Code が文脈読解を強化」「新エージェント登場」が作業ゼロでこちらの恩恵になる。

### 1.4 フライホイール — 価値 ∝ ループ回転速度
エージェントが速い → 1日の反復が増える → 下す決定が増える → 決定SoR が厚くなる → 「なぜ」の記憶価値が増す。**ループが速く回るほど continuum の資産が増える。** = Sierra「プロセスの SoR」を**プロダクト開発そのもの**に適用した形。
...
## 2. データモデル — 正本は repo（git）。continuum は repo に被せるレンズ

**鉄則: continuum は DB を持たない。** 独自ストアに決定データを正本で持った瞬間、囲い込みになり §1.3 の共進化前提（誰でも読めるオープン出力）が壊れる。正本は repo の中の markdown / yaml（DEC-006「正本＝ファイル&Git」を継承）。
...
└── .continuum/               ← continuum の機械machinery（非正本・ツール固有）
...
**この線引きが non-lock-in を構造で保証する**: `docs/` は continuum を消しても価値が残る人間資産。`.continuum/` はツールの足場。「**continuum を消したら docs/ が無価値**」になってはいけない。
...
```
リンクメタデータ = `.continuum/links.json`、参照は docs 内の相互リンク。Notion/Figma にできない、**実コードに接地した traceability** が product surface。
...
```
①注釈（main の実プロダクト上で）
②continuum が git worktree + branch を切る（変更を隔離・複数並行OK）
③注釈 → handoff（.continuum/）→ claude/codex を worktree で起動
...
**agentation との本質差分**: 彼らは注釈→markdown で終わり（記録しない）。continuum は注釈を**入口 modality**に過ぎないものとし、編集を実行し**決定SoR に刻む**（§4 ループ）＋**マルチモーダル(ペン)**を足す。
...
| ターミナル + 委譲 | `pty.ts` + `terminal.tsx` + `agents.ts` → `.continuum/handoff/*.md` → claude/codex 起動 | §3 ③④ 委譲 seam |
| Canvas | `canvas-board.tsx`（@xyflow）+ `screens.ts` → `.continuum/screens.json` | §2.2 実画面表示・§5 注釈の土台 |

### 6.3 ギャップ（目標モデルに対して欠けているもの）= v0.5 作業リスト
1. **docs/ SoR 規約が無い** — `docs/specs/` `docs/decisions/`(ADR) `AGENTS.md` 索引が無い。今の SoR は `.continuum/screens.json`（機械machinery）のみで、**人間に意味がある spec/decision を repo に貯める層が無い**
2. **決定ログ捕捉が無い** — 対象 repo に決定を書き出す導線が無い（決定ログは continuum 自身の playbook にしか無い）
...
- **DEC-008**: repo-as-SoR データモデル確定（docs/ 第一級 + .continuum/ 機械 の分離 / ADR 決定 / AGENTS.md 索引 / worktree-per-change で決定+コード同 commit / drift harness で docs を生かす / traceability links）
