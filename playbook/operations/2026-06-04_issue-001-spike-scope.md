...
> 目的: **Bezier の最大リスク** = 「任意 repo → 実コンポーネントを忠実に流用したモックを、cloud で・許容コスト/品質で生成でき、headless render で証明できるか」を、UI を作る前に **使い捨てスパイク** で検証する。kill / continue を判定する。build ≠ 検証。

## 0. なぜ今（並行の根拠）
WF(グレースケール)は dogfood 可能な土台になった（P0着地）。が、それは「絵が動く」だけ。**Bezier が v0/Lovable に勝てるかは、この生成パイプラインが本物かに懸かる**。WF磨きと並行で、ここを別レーンで証明する（DEC: 2026-06-04 並行採用）。
...
## 2. スパイクの置き場所
`~/Workspaces/Personal/projects/bezier/spike/`（本体 `app/` とは分離。使い捨て）。
