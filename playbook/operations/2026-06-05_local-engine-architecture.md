...
4. **Claude Code は repo の文脈をそのまま保持している**: ユーザーが VS Code / Claude Code を使いながら Bezier CLI を起動するシナリオでは、モデルがすでに repo を「知っている」ため scene-graph 生成の品質が上がる。
...
**Figma も code を知らない** という前提で比較すると、Bezier のローカルエンジン設計は Figma 以上のセキュリティポスチャーを提供できる。これは明確な enterprise 差別化メッセージになる。
...
|---|---|---|---|
| **npm CLI daemon** | 低（Node.js、スパイクの延長） | 低（`npx Bezier start`） | **NOW（Idea/MVP）** |
...
```bash
npx Bezier@latest init              # リポを登録・L1 抽出実行
npx Bezier@latest start             # ローカルdaemon起動（ポート開放・scene-graph生成待ち）
# → ブラウザで app.bezier.dev を開くとリアルタイムで連動
```
...
**VS Code 拡張**は NEXT での追加として有力: エディタ内で `@Bezier` を呼べる体験は、Claude Code との親和性が高く、Kiro/Cursor との差別化にもなる。
...
| 失うもの | 重大度 | 緩和策 |
|---|---|---|
| **制御・再現性**: Claude Code/Codex のバージョン・動作が変わると Bezier の出力が変わる | 中 | scene-graph を SoR に保存することで「いつ生成したか」を記録。out-of-sync を検出できる |
...
Claude Code / Codex が「repo 読んでモック生成」を直接やり始めたら、Bezier のローカルエンジンは要らなくなる。これが CEO の提起した「進化に乗れない」懸念の裏面。
...
これらは「エンジンを替えても残る」価値。Agent Data Platform（Sierra の概念）として蓄積され、「Bezier を外す = 設計意図の記録を全部失う」になる。**SoR こそが moat の実体であり、エンジンは交換可能な部品として設計すべき**。
...
Bezier も「engine を OSS で配り、クラウド SoR を商用」にするなら、**競合に engine+SoR をまるごとホスティングされない**ライセンス選択が moat 防衛上重要。選択肢は §下記 CEO 承認待ち参照。
...
Bezier を **OSS open-core（n8n型）** として提供する:
...
- ライセンスは **fair-code（n8n の Sustainable Use License 型）に確定**（CEO承認 2026-06-05）。ソース公開・self-host 自由・Bezier を SaaS として再販するのは禁止。純OSS(MIT)/BSL は却下。

### 決定（アーキ）

Bezier のアーキを以下のハイブリッドに移行する:
...
4. **AIエコシステムの進化に乗る**: Claude Code / Codex / 次世代エージェントが進化するほど
   Bezier の出力品質が向上する。我々はエンジンを所有する必要がない。
...
/ `playbook/strategy/2026-06-04_Bezier-thesis-v1.md`
```
