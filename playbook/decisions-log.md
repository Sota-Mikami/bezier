...
- **① アプリ deploy 失敗＝Vercel プロジェクト名が無効（真因）**: `usePublish` は `vercel deploy` を **`--project` 無し**で実行 → Vercel は **cwd の basename** からプロジェクト名を導出。worktree の葉は Issue の**大文字 ULID**（`01KV8GG8…`）で Vercel が拒否（400。エラー本文は名前規則の段落全文＝「`---` 不可」も含むが、実際の違反は**大文字**）。→ アプリが deploy されず、共有ページに Preview/Map/QA が乗らない（Design だけ）。**`--project <sanitized 小文字名>` を明示**（Vercel CLI v54 は `--name` 廃止）。`vercelProjectName()`＝小文字化・`[a-z0-9._-]` 以外を `-`・連続 `-` を畳む（`---` を作らない）・前後 trim・100 字上限。名前 = `<小文字id>-app`（journey の共有ページ project と別）。journey 側は元から小文字 dir で OK だったため無変更（`f587f46`）。
...
- 検証: tsc 0 / eslint 0 / vitest 36（journey 6＝ハッシュロック＋タスクリスト含む）/ build green / gitleaks クリーン。コミット `f587f46`（deploy）/ `cac4227`（journey UX）。**要 ⌘Q→再起動**。
- **要・CEO 実機確認**: `--project` での**新規プロジェクト自動作成**は live で未検証（Vercel を実際に叩くため）。再共有して Preview/Map/QA が乗るか確認 → ダメなら `vercel link --project` 明示リンクにフォールバック予定。
