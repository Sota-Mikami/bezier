...
- **進め方（CEO 確定）**: **面ごとに順次**（各ラウンド verify＋区切りコミット）。基盤＋第1スライス（top bar / History ドロワー / 設定上段）完了。残り ~45 ファイル。次: sidebar＋command palette → issues 一覧/詳細 → preview/design/map/qa → share/docs → （最後に）agent プロンプト/Spec テンプレ。tsc/eslint/vitest(22)/build 全 green。dev のみ。
