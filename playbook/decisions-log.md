...
- 検証: tsc 0 / eslint 0 / vitest 36 / cargo Finished / build green / gitleaks クリーン。コミット `f587f46`→`7bd227c`（deploy）/ `cac4227`（journey UX）/ `ad2f181`（ログ可読化＋Inspect）/ `95f2426`（public env 注入）/ `cda0473`（**Option B＝Vercel プロジェクト env 登録＋`--prod` 安定 URL**）/ `28029fc`（**ペルソナが UI で env 値を設定＝ファイル/CLI 編集不要**）。本番 = **12:09 ビルド**。**要 ⌘Q→再起動**。
