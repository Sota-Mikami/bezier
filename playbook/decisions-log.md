...
## DEC-157 (2026-06-26) — 画面収録(Screen Recording)を不要に：`screencapture`→WKWebView `takeSnapshot` → v0.5.0 公開リリース

> CEO: 画面収録の許可が**更新のたびに必要でダルい**。複数/消せない modal も残る。xattr のように**コマンドで事前許可**できない?

- **事前許可コマンドは macOS 仕様上不可**（TCC 権限はセキュリティ保護され**スクリプトで GRANT 不可**＝対話同意必須。xattr が効くのは quarantine が TCC 保護外だから）。→ CEO 選択＝**そもそも画面収録を不要にする**。
- **原因**: `capture_region` が `screencapture`（画面領域キャプチャ）を使用＝画面収録権限必須。さらに **ad-hoc 署名で識別子がビルド毎に変わる** → TCC が「別アプリ」とみなし**更新ごとに許可リセット**。複数/消せない modal も未署名＋連続キャプチャの典型。
- **修正**: ObjC ヘルパー(`snapshot_helper.m`・`cc` で `build.rs` がコンパイル・WebKit/AppKit リンク)で **WKWebView `takeSnapshotWithConfiguration:`**（自分の web コンテンツを直接スナップ＝**権限不要**・async 完了を runloop spin で待つ→NSBitmapImageRep で PNG）。新 `webview_snapshot(label,x,y,w,h,out)` コマンド（Preview=`embedded-browser` をフル / mock=`main` を iframe 矩形クロップ）。**`capture_region`(screencapture) は通常経路から撤去**（deprecated・未使用）。セキュリティガード（`.bezier` 配下・traversal 拒否・grant 確認）は同一。
- **リリース**: 版 **0.5.0**。Rust 変更ありなので `cargo fmt`＋`clippy -D warnings`(CI=1.83) を通し（`webview_snapshot` に `#[allow(clippy::too_many_arguments)]`）**CI 緑維持**。main コミット→`v0.5.0` タグ→`release.yml` で `.dmg`。検証 tsc0/eslint0/vitest107/cargo build release 成功。確認=画面収録ダイアログが出ない＋注釈/Map のスクショに印が写る。[[bezier-release-distribution]]。

---

## DEC-156 (2026-06-26) — Comment をライブ・スクロール化（凍結廃止・印はコンテンツ追従）＋スクショを画像添付に

> CEO: Comment モードで**画面がスクロールできなくなるのをやめたい**。普通にスクロールで探索しながらコメントしたい。**印がスクロールでずれる**のも直したい。送信時は**スクショ＋コメントをチャットに**（画像として見える形で）。

- **原因**: 旧 annotation は **webview を1枚のスクショに凍結**してその上に DOM オーバーレイ（印=ビューポートの 0–1 座標）＝スクロール不可＋再凍結で印ドリフト。凍結は「ネイティブ webview に上から HTML を描けない(DEC-120)」ための方便だった。
- **新方式**: visual-edit と同じ **ページ内オーバーレイ**に統一（`bezier-overlay` を v2 にし annotate サブモード追加・webview=eval / iframe=postMessage）。印は**ページ内（コンテンツ座標）に描画 → スクロールに追従**（ドリフト解消）。ページはライブ＋スクロール可能。コメント入力は webview に被らない**側パネル**(`live-annotation-panel.tsx`)。送信時に**ライブ画面＋印**をスナップ。状態管理は新 `use-annotation-data.ts` に抽出。`visual-edit-transport` に `ensureOverlay?()` 追加。
- **#1（スクショを画像添付に）**: `prompts.ts` の `shotRef`(JA/EN) を **`![](path)` の画像添付**形式に（DEC-151 のチャット画像添付と統一・agent が確実に画像として受け取る）。
- **繰り延べ**: 旧形式の保存済み注釈の page 座標欠落 / Pen の WKWebView 描画なめらかさ / mock の mode 切替時 iframe リロードのフラッシュ / 「after」shot の意味。

---

## DEC-155 (2026-06-26) — Edit 中の要素削除（Delete/Backspace）＋ Backspace=戻る バグ修正

> CEO: Edit 中に要素を選択して **Delete で消せる**ようにしたい。今 Delete を押すと **issue 詳細から別画面に遷移**してしまう。

- **削除**: overlay(`bezier-overlay`) に `removeSel`（要素を live 削除＋`delete` イベント＋選択クリア）＋ onKey に **Delete/Backspace**（`preventDefault`＋`stopPropagation`）＋ `removeSelected` API。webview/iframe にフォーカスがある時を **in-page** で処理。
- **メインアプリ側**: preview-pane / issue-design の Edit キーハンドラにも Delete/Backspace→`vedit.deleteSelected`（Layer/Style パネルにフォーカスがある時用）。**この `preventDefault` が WKWebView の Backspace=戻る（＝画面遷移バグ）を無効化**。
- **コードへ反映**: `use-visual-edit` に `deletes` intent・`editCount` に加算・`deleteSelected()`。`visualEditPrompt` に `deletes`（agent が source から要素を削除）＋ `veDelete` フレーズ(JA/EN)。

---

## DEC-154 (2026-06-25) — HTML mock の Edit を opaque iframe + postMessage で修正 → v0.4.0 公開リリース

> CEO: **HTML(mock) の Edit が効かない（Preview は効く）。なぜ? 中身の性質が違うから? 要調査して適切に直して。**

- **原因（徹底調査）**: Preview = native webview（`embedBrowserEval` で overlay 駆動）で動く。一方 **mock の Edit 用 iframe だけ `sandbox="allow-scripts allow-same-origin"`** で、親から `contentWindow.__bzEdit`（操作/イベント）と `contentDocument`（保存時 serialize）を**直接**読む設計。**この same-origin 依存が WKWebView で脆く**通らない。VIEW 用 iframe は `allow-scripts` のみ(opaque)で「最も WKWebView-robust」と実績あり。
- **修正（webview/Preview 経路は不変・iframe 経路だけ堅牢化）**: **Edit iframe も opaque(`allow-scripts`)** にし、overlay 操作・イベント・保存を全部 **postMessage ブリッジ**経由に＝実績ある VIEW iframe と同方式・origin 非依存。`mock-edit.ts` に `BRIDGE_JS`（q を親へ push・コマンド受信・serialize 返却）/ `iframeTransport` を postMessage 化＋`serialize()` 追加 / `saveMock` を `contentDocument`→`serialize()` に。
- **リリース**: 版 **0.4.0**（`package.json`/`tauri.conf.json`）。main コミット→`v0.4.0` タグ→`release.yml` が arm64 `.dmg` を公開 Release に添付。本バッチ（DEC-151〜154）は **frontend のみ**＝Rust 不変なので DEC-150 後の CI 修正（Rust pin 1.83 / hono / fmt）で**緑を維持**。検証 tsc 0 / eslint 0 / vitest 107。

---

## DEC-153 (2026-06-25) — Prototype IA: Map/QA に (Beta) ＋ Edit⊻Comment を統一モード切替に

> CEO: ①Map/QA は未検証だから配布先が誤解しないよう **(Beta)** を付けたい。②HTML/Preview の **Edit と Comment が同じ Pencil アイコン・別スタイル・配置バラバラ**で微妙。情報設計/IA を見直して。

- **Beta**: `build-review.tsx` の Map/QA タブに小さな Beta バッジ（モノクロ）。`prototype.beta` i18n。
- **統一モード切替**: 新規 **`mode-toggle-group.tsx`（セグメント・排他・`SquarePen`=Edit / `MessageSquare`=Comment）** を mock(`issue-design`) と Preview(`preview-pane`) の**両方**で使用＝同一見た目・co-locate。排他は `editDisabled || commenting`＋`commentLocked`(editing 連動)で処理。
- **IA 整理**: Comment(`AnnotationToggle`) を build-review のタブバーから撤去し **Preview 面の ModeToggleGroup に移動**（Edit と co-locate）。Map/QA は実プレビューが無く annotate 不適なので Comment 無し（Beta 表示）。

---

## DEC-152 (2026-06-25) — Proceed 等「再起動して送る」を全て inject に統一（no-restart）

> CEO: NextStep の **Proceed** を押すと「Interrupt the running agent?」が出て**チャットが新しく切り替わる**＝もうやめたはずの設計。**走っている既存チャットの入力欄に prefill/inject** したい。

- 原因: `sendDesignFeedback`（agent 再起動）を**直接**呼ぶ箇所が4つ — **NextStep Proceed** / preview-doctor「Agent に直してもらう」/ visual-edit「コードに反映」/ mock を code に反映。
- 修正: 全て **`injectOrFeedback`**（走っている agent に inject＝再起動しない・いなければ feedback にフォールバック＝strictly better）に。[[DEC-142]] の no-restart seam に一本化。

---

## DEC-151 (2026-06-25) — 画像添付（モーダル / AI Chat / Pin・Comment で共有 UX）

> CEO: 新規 Issue 作成・AI Chat・Pin/Comment で**テキストだけでなく画像**を添付したい。UX 統一: paste/drop で添付・入力欄近くに**小プレビュー**・ホバーで **× 削除**・クリックで**大プレビュー**（複数は矢印/←→/Esc）。

- **共有部品**: `lib/use-image-attachments.ts`（in-memory 管理・object URL リーク防止）/ `ui/attachment-tray.tsx`（小プレビュー・ホバー×・クリック拡大）/ `ui/image-lightbox.tsx`（大プレビュー・‹›＋←/→・Esc）。
- **AI Chat**: 既存の TQ-2 トレイ(`terminal.tsx`)を共有部品に統一（クリック拡大が新たに追加・保存/送信挙動は不変）。
- **新規モーダル**: worktree 未作成なので画像は **in-memory で保持**、Start 時に `pending-start`(blobs=Uint8Array)→worktree の `.bezier/chat-attachments/` へ materialize＋最初のメッセージに `![](path)` 付与（agent が画像を読む）。
- **注釈 Composer**(`design-annotations.tsx`): paste→tray→lightbox、送信時に画像パスを注釈プロンプトへ。
- 繰り延べ: 注釈の一括送信バー（入力欄なし）/ md テキスト選択コメント / mock 要素コメント / ドロップ時ハイライト。

---

## DEC-150 (2026-06-25) — 新規作成モーダルを Linear 風に磨き＋選択 UI 統一（SearchableSelect）→ v0.3.0 公開リリース

> CEO dogfood 反復（「Modal 微妙／フォントもレイアウトも変／**出る前が重い**／Linear を参考に／branch は**下向き**に／folder も独自の検索ドロップダウンに」）→ 詰め切って「良い感じ！version 切って」。[[DEC-149]] の continuation。

- **「出る前が重い」の主因＝`backdrop-blur` の全アプリ合成**（裏のネイティブ preview webview までぼかす）→ **撤去して軽いディム＋`animate-in fade/zoom` 入場**。即・なめらかに。
- **Linear 参考の再デザイン**: ヘッダに**閉じる✕**・via agent・**prominent な hero 入力**・footer にプロパティピル＋Start。アプリ標準の `text-sm`/`text-xs`・**完全モノクロ**（緑ドット/amber を撤去）に揃えて「フォント変」を解消。
- **レイアウト崩れの真因修正**: `BaseBranchPicker` root の **`ml-auto` ハードコード**（agent ヘッダ用）がモーダルで効いて **Base が中央に飛んでいた** → コンポーネントから外し、**呼出側で align/margin 制御**（agent ヘッダは `ml-auto` ラッパ）。
- **選択 UI 統一**: 新規 **`ui/searchable-select.tsx`（汎用検索コンボボックス・`align`/`placement`/`action`/`onRefresh` 対応）**。`BaseBranchPicker` を**薄いラッパに作り替え（API 不変）**、**folder（旧ブラウザ標準 select）も SearchableSelect 化**＝branch と完全に同じ見た目・操作（検索・↑↓/Enter/Esc・✓・末尾「フォルダを開く…」行）。**branch は下向き**ドロップダウン（上向きの浮きを解消・モーダル `overflow-visible`＋12vh で非クリップ）。i18n `newIssueModal.folderSearch/folderNoMatch`。
- **リリース**: 版を **0.3.0**（`package.json`/`tauri.conf.json`）。**main にコミット＋push → `v0.3.0` タグで `release.yml` が arm64 `.dmg` を公開 Release に添付**。**Map Phase 1(DEC-148)＋新規作成モーダル(DEC-149)も同梱**（repo は DEC-147 で public 化済・未署名 `xattr`）。Map P1 は legacy(DEC-133)フォールバックあり＝`/bezier:map` 未実行でも退行なし。
- **検証**: tsc 0 / eslint 0 / vitest 107 / cargo green。ローカル prod も 0.3.0 で再ビルド→`/Applications`。

---

## DEC-149 (2026-06-25) — 新規 Issue 作成を「フォーカスした作成モーダル」に（Chat 主役・Folder/Branch 集約）— 実装・dogfood 検証待ち

> CEO「Issue 新規作成の UX をよくしたい。Chat メインで良い（Spec を書く状態でなくてよい）。Chat / Folder / branch だけ＝Modal でも」。COO 提案（Modal・Chat 主役・作成は Start まで遅延）→ CEO 承認「Modal で。実装勧めて」。

- **痛み**: 今は New/⌘N → 現在 repo に即・空 issue 作成 → **フル issue 画面（空の Design/Prototype チョーム＋ChatStart）**に着地。作成の瞬間に雑音が多い。chat-first なのに Spec チョームが見える。
- **[[DEC-083]]/[[DEC-084]] との整合（重要）**: DEC-083 が「作成時 repo ピッカー」を入れ DEC-084 が撤回（CEO「入り口制御より開いた後に既定＋切替」／**完全ファイル遅延は侵襲的**＝データ層が issue=フォルダ前提）。今回は **(a) chat が主役**（folder/branch は既定値）で DEC-083 の「repo を選ぶだけのゲート摩擦」とは別物、**(b) createIssue を Start クリックに移すだけの軽い遅延**＝DEC-084 が嫌った「一時 issue/データ層作り替え」には当たらない（Start の瞬間に実フォルダ化）。→ 同じ轍は踏まない。
- **決定**: New / ⌘N / ⌘K「新しい Issue」→ **NewIssueModal を開く**（即作成＋着地をやめる）。中身＝**Chat（主役・autofocus・Enter で Start）＋ Folder（開いた repo の dropdown・既定=現在）＋ Base branch（DEC-145 の `BaseBranchPicker` 再利用・選択 folder に対し自動 fetch）＋ agent 表示**。Spec エディタは出さない（agent が書く）。
- **作成タイミング = defer-to-Start**: Start で `createIssue(folder,"")` → `setPendingStart(id,{message,base})`（新規 `lib/pending-start.ts` のモジュール内レジストリ）→ navigate。**キャンセルは何も残さない**。
- **自動 start の肝（COO 重点レビュー・正しいことを確認）**: 着地後 `use-implement-session` が Effect A で pending を消費→ref 格納、**Effect B が ready 時（gitRepo＋agent＋worktree 無し＋action 無し）に、`chosenBaseRef` を同期セットしてから `handleStart(msg)` を一度だけ**発火（msg ref を先に null 化で二重発火防止、deps で再評価）。base が worktree を切る前に確実に入る。
- **エッジ**: agent 未検出/未ログイン or folder が非 git → Start 無効＋inline 案内（`noAgentGuide`/`gitIsRepo` 流用）。サイドバー一覧から開く非モーダル経路（既存ヘッダ BaseBranchPicker）は不変。Rust 変更なし。i18n JA/EN。
- **実装**（PE・新規 `new-issue-modal.tsx`＋`pending-start.ts`／`app-sidebar.tsx`・`use-implement-session.ts`・i18n 改修）。検証 tsc 0 / eslint 0 / vitest 107。**prod ビルド→`/Applications` 差し替え（CEO 検証用）。コミットは検証 OK 後**。Map Phase 1（DEC-148）と同じ working tree に同居。

---

## DEC-148 (2026-06-25) — Map 再設計（agent が diff∩要件で撮る coverage Map）— Phase 1 実装・dogfood 検証待ち

> CEO が Anam Hira（Revyl・元 Uber）の X を引いて「**Map のあるべき姿**」を相談 → 削り込み → Product 起草 → CTO/AI 両レビュー → 統合 → Phase 1 実装。痛み＝「**誰もアプリの本当の姿を知らない／intent⇔実装の乖離を人が手で照合**」。Bezier が**変更の source-of-truth**を持つ。詳細＝`product/issues/ISSUE-006.md` ＋ `playbook/quality-reviews/2026-06-25_issue-006-*`（CTO / AI-eng）。

- **CEO が LOCK したスコープ**: ❌全アプリ・インベントリ／❌ピクセル・パリティ（design vs built diff）／❌CI テストランナー／❌モックのスクショ／❌Live・既存アプリ。✅ **per-issue の coverage Map ＝ この変更が触る画面 × その状態 を、worktree で実際にコード化された実装のスクショ で俯瞰**。
- **方針（CEO 承認）**: **agent が（コード diff ∩ Design 要件）から撮影対象を導出**し、capture **manifest** を書く。各エントリ `{id,label,route,state,source,reach,citation}`、`reach.kind∈{url,seed,steps,harness,manual}`（step-DSL は今フル定義・**P1 は url のみ実行**）。既存 authed-webview で撮影。**要件にあるが到達不能＝空セル＝coverage gap**（ピクセル比較なし）。「嘘のスクショより空セルが誠実」が核の賭け（両レビュー一致）。
- **レビュー収束のブロッカー（P1 に畳み込み済）**: ① **エントリ ID は Bezier 側で正規化**（`manifestEntryId`・churn 防止） ② `/bezier:map` に状態ラベル規約＋**auth ルートは既定 manual** ③ **`/bezier:states` を機械可読化**（`<!-- bezier:states -->` ブロック・prose 解析を排除・前倒し） ④ **auth-redirect ガード**：ナビ後に最終 pathname を検証し、login 等に飛んだら**撮らず空セル**。Zod 検証＋各エントリ citation。
- **COO が実装レビューで捕まえた核心バグ（修正済）**: PE 実装が manifest エントリで**素の `route` にナビ**していた → 状態到達用の **`reach.url`（クエリ付き）にナビしていない**＝**全状態セルが同じ default 画面になる**致命傷。→ `reach.url` へナビ＋redirect 判定を**pathname 同士**に修正（`preview-pane.tsx`）。
- **実装**（PE・8ファイル）: 新規 `lib/map-manifest.ts`（Zod schema・`manifestEntryId`・read/write・helpers）。`prompts.ts`（`/bezier:map`＋`/bezier:states` 構造化・JA/EN）。`issue-map.tsx` 全面改修（screen×state 2D グリッド・理由付き gap セル・citation tooltip・legacy DEC-133 フォールバック）。`build-review.tsx`/`preview-pane.tsx`（manifest 撮影経路＋redirect ガード）。**Rust 変更なし・journey.ts 不変・DEC-133/141 退行なし**。新依存 `zod@4`。
- **P1 で意図的に繰り延べ（P2/P3）**: manifest エントリのターン終了自動再撮影（P2）／`/bezier:map` をボタンから送信（P2・現状は chat で打つ）／`seed`/`steps`/`harness` 実行エンジン（P3・DSL は定義済・gap 表示のみ）／セルクリックで in-app Preview ナビ（P2・現状は外部ブラウザ）。
- **検証**: tsc 0 / eslint 0 / vitest 107 / cargo（Rust 無変更）。**prod ビルド→`/Applications` 差し替え（CEO が触って検証するため）。コミットは検証 OK 後**。
- **新規性**: 「**diff∩要件で agent が何を撮るか推論し、その不確実性が空セル（gap）として可視化される coverage Map**」は業界に無い（Storybook=component isolation／Chromatic=regression。これは coverage awareness）。Sierra フレーム＝既存 QA SoR（Playwright/Chromatic）の上に立つ SoA。

---

## DEC-147 (2026-06-25) — v0.2.0 配布レディネス・レビュー（security + 完全性）→ 出荷前修正 → public 化して外部 FB 配布

> CEO「配布用にバージョンを上げてリリース。周りに何人か使ってもらって FB が欲しい。配布できる状態か (1) セキュリティ (2) 仕様の抜け漏れ をチェックして」。principal-engineer（security）＋ qa-lead（完全性）に並行レビューさせ、配布の足回り（version・release 機構・git 状態・履歴シークレット）を確認。

- **レビュー結果**: **security＝ブロッカーなし**（クローズドな少人数なら GO）。バンドル秘密なし／テレメトリなし／git は配列引数でシェル注入なし／FS は traversal+grant 二重ガード／共有ページのエスケープ健全／パスワード gate 暗号 OWASP 準拠（PBKDF2 210k・AES-GCM）／design iframe は opaque origin。**完全性＝B-1 がブロッカー**＝Feedback ボタンと .dmg DL が **private repo** で両方塞がっている。
- **CEO 決定**: ① repo を **public 化**（Release DL・Feedback issues が両方開通） ② version **0.2.0**（minor・今セッションで機能多数）。
- **公開前の安全確認**: git 履歴のシークレットスキャン＝**クリーン**（ヒットは研究 doc とパターン文字列/テスト fixture のみ・実 `.env` のコミットなし）。
- **出荷前修正（実装済）**: version 0.1.x→**0.2.0**（package.json／tauri.conf 整合）。qa: ① Base ピッカーを branch ロード前から**即表示**（チラつき解消） ② **Export 無効ボタンに理由を inline 表示** ③ README に**英語 Install ＋前提条件**（agent ログイン要件を明記）。sec（多層防御）: ④ gate ページ復号 iframe に **sandbox 付与** ⑤ DEC-143 後の**古い CSP コメント**（`journey-script.ts`）修正。env 同意は publish フローが `collectPublicEnv`（public のみ）＋文言も正確＝**修正不要**と確認。
- **follow-up（GO に影響なし・次パス）**: sec S-1（`symlink`/`clone_dir` に `ensure_granted`＝state 注入要・worktree grant 不確実でリリース直前リスク回避） ／ qa S-1（agent「導入済だが未ログイン」検知の inline 案内＝現状は README＋welcome で補完） ／ N 系（branch refresh 失敗の表示・⌘F discoverability・in-app Docs リンク）。
- **検証**: tsc 0 / eslint 0 / vitest 107。**リリース手順**: main にコミット＋push → `gh repo edit --visibility public` → `git tag v0.2.0 && git push origin v0.2.0` で `release.yml` が arm64 `.dmg` を GitHub Release に添付（未署名・`xattr -dr com.apple.quarantine` で隔離解除）。
- **⚠️ public 化の含意**: `playbook/`（決定ログ・戦略・競合メモ）・`product/` も恒久公開される（README は既に brand 戦略を公開リンク済＝一定の openness は許容済み）。戦略を private に保ちたい場合は **app/＋site/ だけの公開 repo に分離** or **out-of-band 配布**が代替。

---

## DEC-146 (2026-06-24) — Share に ZIP エクスポートを追加（URL 共有と二択・タブ「リンク / エクスポート」）

> CEO「URL 共有とは別に、選択ページを **ZIP でエクスポート（ダウンロード）**したい。md / html として Slack 等に展開。ファイルはローカルにあるが保存場所の特定が難しいので、**保存先を選べる**形に。Share ボタンのドロップダウンに**大きなタブ2つ**（URL 共有 / ZIP エクスポート）。タブ名は最適案を提案して」。

- **タブ名（提案・採用）＝「リンク」/「エクスポート」**（en: Link / Export）。理由：親ボタンが「共有」なので子タブを「シェア」にすると重複。「URL」より「リンク」が製品語彙として自然（Notion / Figma 流）。「URL」希望なら i18n 1語差し替えで可。
- **決定**: Share ポップオーバーを2タブ化。**共通の「共有する内容」選択（チェックボックス）は両タブで共有**し、出力方法だけ分岐：① **リンク**＝従来のホスト URL 共有（パスワード・アプリ deploy 等そのまま） ② **エクスポート**＝選択ページを md / html ファイルとして .zip に束ね、**保存ダイアログで任意の場所**へ書き出す。
- **実装（Rust 不要・新依存なし）**: 既存 `ipc.saveFileDialog`（DEC-081）＋`writeFileBytes`＋`revealInFinder` と `gatherJourneyData(issue, null, cfg)` を再利用。新規 **`lib/zip.ts`＝依存ゼロの最小 ZIP エンコーダ（STORE 方式・CRC32 自前）**（小さな text ファイルなので無圧縮で十分）。`lib/share.ts` に `buildExportEntries`（design doc→`NN-<label>.md` ／ html→`.html` ／ QA→md テーブル・連番＋重複回避＋ファイル名サニタイズ）＋`exportShareZip`（gather→保存ダイアログ→`zipSync`→`writeFileBytes`）。**Preview / Map はライブ URL／スクショなのでファイル化せず skip**（URL 共有側の領分）。
- **UX**: エクスポート後に保存パス＋「Finder で表示」（保存場所が分かりにくい問題への直接対応）。`anyExportable`（design か QA 選択時のみ活性）。空選択は dialog で通知。
- **検証**: tsc 0 / eslint 0 / **vitest 107**（zip ラウンドトリップ＋UTF-8 のテスト2件追加・i18n パリティ含む）。prod 再ビルド→`/Applications` 差し替え。
- **スコープ外/今後**: フォーマット切替（全ページを md に統一等）／Preview のスクショ同梱／自己完結 1 HTML（journey）同梱。

---

## DEC-145 (2026-06-24) — per-issue base branch（pin＋ピッカー）：未リリース機能の上で課題を潰せる

> CEO「stg でデザインレビューした未リリース機能の課題を、リリース前に Bezier 上で潰したい。ベースにする branch を指定したい／PR を main 以外に出したい」→ 調査＝Bezier は元々 main 固定でなく「リポの現在ブランチ」に追従（`git_base_branch` = `current_branch`・OPEN-001）。だが暗黙的＆ライブ追従で **per-issue 固定が無い**のが唯一のギャップ → base を per-issue で pin＋UI ピッカーを入れる（CEO 承認「1と2をやりたい」）。

- **決定**: issue の worktree を「選んだ base branch」から切り、その base を **`WorktreeRef.base` に pin**。Sync / Merge / PR / diff / behind は **pin した base** を向く（リポの checkout を後で別ブランチに変えても取り違えない＝ライブ追従の落とし穴を解消）。base は**作成時に選択・開始後は read-only**（変更＝rebase なので v1 は対象外）。
- **Rust**（`lib.rs`）: `git_worktree_add` に `base` 引数（空＝従来どおり HEAD／local ブランチや `origin/x` を start-point に解決）。`git_list_branches`（`current` ＋ local heads ＋ remote-tracking、`origin/HEAD` 除外、current も一覧に残し戻れる）を新設＋ `invoke_handler` 登録。
- **TS/ロジック**（`use-implement-session.ts`・`git.ts`・`issues.ts`）: `WorktreeRef.base` 追加。`chosenBase`/`branches`/`setChosenBase`/`hasWorktree` を session に公開。base 解決を `gitBaseBranch`→`gitListBranches` 化。**`basePinnedRef` で 2 つの mount effect（ref 読込／branch 一覧）の race を回避**（pin が常に勝つ）。作成時に chosenBase から切って pin。**既存（base 無し）ref はライブ current にフォールバック**（後方互換）。
- **UI**: start 空状態のヘッダに **base ピッカー**（`issue-agent-panel.tsx` の `<select>`・既定＝リポ現在ブランチ・local＋`origin/*`）。開始後はヘッダに **`branch → base`** を常時表示。Ship メニューは従来どおり「Merge to {base}」「Sync with {base}」で base を表示。i18n `agentPanel.baseLabel/baseTip/branchInto`（JA/EN）。
- **使い方（stg シナリオ）**: New →（必要なら下地リポで一度 fetch）→ ヘッダの **Base で機能ブランチ**（例 `feat/x` または `origin/feat-x`）を選ぶ → 開始 → worktree はそこから切られ、Sync / Merge / PR も `feat/x` を向く。
- **却下/スコープ外**: 開始後の base 変更（rebase 要・v1 外）／PR-target を Bezier 内で別途指定（draft PR なので **GitHub の compare/PR GUI で base 変更可**・openPR は既に base を渡す＝[[DEC-143]] 系の流れ）。
- **検証**: tsc 0 / eslint 0 / vitest 105 / **cargo check 0**。prod 再ビルド→`/Applications` 差し替え。

**追補（同日・設計原則）**: CEO「ペルソナはターミナルで git を触れない前提。『一度 git fetch』のような操作を要求する設計にしない」。→ picker のリモートブランチ取得を **Bezier が自動 fetch** で賄う。① 未開始 issue を開いた時に `gitFetch`（best-effort・`GIT_TERMINAL_PROMPT=0` で fast-fail）→ 再一覧（`refreshBranches`）で、別所で push されたばかりの機能ブランチもターミナル無しで選べる。② picker 横に**更新ボタン**（spinner）＝ワンクリック再取得。`refreshingBranches` で状態表示。pin 済みは上書きしない。③ ブランチが多い repo 向けに picker を**検索可能なコンボボックス化**（新規 `base-branch-picker.tsx`・フィルタ入力＋↑↓/Enter/Esc・更新ボタン内蔵・cmdk/popover 等の新依存なしの自作）。**学び＝ペルソナ向け機能は「ターミナル前提の前段ステップ」を UI/自動化で内製する**（git fetch / checkout を要求しない・[[persona-no-terminal-git]]）。tsc 0 / eslint 0 / vitest 105。

---

## DEC-144 (2026-06-24) — Design ペインに ⌘F 検索（md=CodeMirror search / html=注入オーバーレイ）

> CEO「⌘+F の検索が欲しい。md や html の時に。欲しく感じる」→ Design ペインの md(Spec) と html(デザイン案) で文中検索できるように。md と html は機構が違う（md=親DOMの CodeMirror／html=サンドボックス iframe で親から DOM 検索不可）ので、それぞれ最適解を採る。

- **md（CodeMirror）**: 標準拡張 **`@codemirror/search`** を有効化（`search({top:true})` ＋ `searchKeymap` を defaultKeymap の前に）。⌘F でパネル・⌘G 次/前・件数・ハイライト・置換。パネルは **shadcn トークンでテーマ付け**（`markdown-editor.tsx` editorTheme に `.cm-panels`/`.cm-search`/`.cm-textfield`/`.cm-button`/`.cm-searchMatch*`）＝stock CM の見た目にしない。
- **html（iframe）**: design タブは `sandbox="allow-scripts"`（same-origin 無し）＝**親から中身を検索できない**。→ **render 時に自己完結の検索オーバーレイを html に注入**（新規 `lib/design-find.ts` の `withDesignFind(html, placeholder)`・名前空間 `__bzfind`）。iframe 内 ⌘F（フォーカスが iframe 内）または親からの `postMessage({__bzfind:"open"})`（フォーカスがアプリ chrome）で開く。**テキストノードを `<mark>` でラップ**してハイライト（innerHTML 再パースしない＝maker の JS/ハンドラを壊さない）、次/前・件数 n/N、閉じると unwrap。**保存ファイルは不変**（描画時のみラップ）。親 `issue-design.tsx` は **view タブ時のみ** ⌘F を横取りして iframe へフォワード（md タブは CodeMirror が処理・edit モードは別サーフェスなので対象外）。
- **CSP 整合**: in-app の design iframe(srcDoc) は Tauri webview CSP（`script-src 'self' 'unsafe-inline'`）を継承＝注入 inline script も maker の inline JS も許可される（＝[[DEC-143]] の allow-scripts が実際に効く裏付け）。
- **スコープ外（現状）**: 共有ページ（`journey.ts`）の ⌘F は未対応 — 受信側はブラウザ標準 ⌘F が doc テキストに効く（html タブ iframe 内は別途・必要なら follow-up で同じ注入を流用可）。
- **検証**: tsc 0 / eslint 0 / vitest 105（i18n パリティ含む）＋ 注入 IIFE を `node --check` で構文検証。i18n キー `design.findPlaceholder`（検索 / Find）追加。prod 再ビルド→`/Applications` 差し替え。

---

## DEC-143 (2026-06-24) — Design > HTML を「動く」に：sandbox を `allow-scripts` 化（静的ワイヤー縛りを撤廃・in-app＋共有ページ実装済／DEC-113 supersede）

> CEO「Design > HTML って実際に触れる/動きのあるものになってる？こういうの作ってもらったけど何も動かない（hover だけ）。Bezier の設計が悪いのか HTML が悪いのか？」→ 診断＝**どちらも壊れていない**。Design > HTML は意図的に**静的ワイヤー**（iframe `sandbox=""` で全 script 剥奪・プロンプトも「外部JS禁止/静的描画」を明記）に固定されていた。エージェントが書いた `<script>` は読み込み時に剥がされ、生きるのは CSS `:hover` だけ＝「hover しか動かない」の正体。エージェントは静的サーフェス向け成果物を「動くプロトタイプ」と命名し、出せない動きを約束していた（mislabel）。

- **CEO 決定（このフレーム再定義）**: 「**安全配達・dev サーバ不要は維持したい。が、HTML 1枚の中で動き/インタラクションを確認したい。新しい『interactive HTML』サーフェスは足さない** — 今まで通り **Markdown / HTML / Prototype（実 repo ベースの最終形）** の3面のまま、**HTML 面の自由度をあえて締め付けない**。`allow-scripts` 方向で。」
- **決定**: Design > HTML を「静的ワイヤー」から「**単一・自己完結 HTML（dev サーバ不要・外部依存ゼロ）だが script は走る**」に再定義する。新サーフェスは作らない（3面のまま）。
- **実装（in-app・`app/` repo 済）**:
  - `components/issues/issue-design.tsx` の閲覧 iframe を `sandbox=""` → **`sandbox="allow-scripts"`**（**`allow-same-origin` は付けない**＝opaque origin で隔離維持：親アクセス・storage/cookie 不可・top-nav 不可）。Edit モードの iframe（既に `allow-scripts allow-same-origin`）は不変。
  - プロンプト `lib/prompts.ts` を **4箇所（designBlock JA/EN ＋ variantBody JA/EN）** 改訂：「**プレーン CSS のみ・JS 禁止・静的描画**」→「**単一自己完結 HTML（外部 CSS/JS/CDN/画像なし＝dev 不要・安全）／インライン JS もOK（トグル・選択・モーダル等の触れる動きを入れてよい）／隔離 sandbox なので状態はメモリ内のみ（localStorage/cookie/fetch 不可）**」。あわせて designBlock の内部矛盾（「インタラクションのスケッチでもよい」直後に「JS 禁止」）を解消。
- **Phase 2＝共有ページ（`lib/journey.ts`）も「動く」に（CEO 承認「共有時にもHTMLの自由度を持ちたい」・実装済・DEC-113 supersede）**: 被共有ページの design タブ iframe を `sandbox=""` → **`sandbox="allow-scripts"`**（same-origin 無し＝隔離）。だが **srcdoc は親 CSP を継承**するため、内側ページの `script-src '<SHARE_SCRIPT_HASH>'`（hash-lock・DEC-113）を **`script-src 'unsafe-inline'`** に変更（**hash と unsafe-inline は排他**なので hash を撤去＝**DEC-113 を supersede**。ゲートページが既に採用済みのやり方と統一）。`SHARE_SCRIPT_HASH` 定数は撤去。
  - **公開ページでも安全な理由（hash ではなく『隔離＋エスケープ』で担保）**: ① design iframe は `allow-same-origin` 無し＝**opaque origin** で親（復号済み内容・パスワード・localStorage/cookie）に触れない ② 継承される `default-src 'none'` が **fetch/XHR/WebSocket/forms/top-nav を全ブロック** ＝ prototype は**自分の DOM しか動かせない**（外部送信・ビーコン不可） ③ 親ページ本体は title/markdown/QA を**全てエスケープ生成**するので注入 `<script>` を形成不可（hash-lock は多重防御だった・主ガードはエスケープ）。暗号化（パスワード）共有でも同経路で成立（ゲートの same-origin `view` iframe 内に内側ページ＝unsafe-inline、その中の design は allow-scripts で隔離）。
  - 受信側の「動き」確認は従来 Prototype タブ（実アプリ preview）に限られていたが、これで **共有 Design タブ自体が触れる**ようになった。
- **検証（Phase1+2 まとめ）**：`app/`（親 bezier と同一 git repo）で **tsc 0 / eslint 0 / vitest 105 pass**（journey テストは hash-lock→隔離 sandbox に書き換え）。Rust 変更なし。**Phase1+2 込みで prod `.app` を1回ビルドして `/Applications/Bezier.app` 差し替え**（CEO 指示）。
- **却下**: 新「interactive HTML」サーフェス新設（CEO 明示却下＝3面を保つ）／design iframe に `allow-same-origin` 付与（sandbox 脱出の footgun・prototype に storage 不要なので付けない）／共有ページの hash-lock 維持＋別ファイル化で非継承化（暗号化共有では plaintext を別ファイルにできず破綻・複雑化するため、隔離 sandbox 一本化を採用）。

**追補（同日・実機 dogfood で発覚→修正）**: prod 差し替え後も空シェルのままで、issue 内エージェントが「**BEZIER.md が『静的描画』と言っている**」と純CSS化を試みた。真因2つ：① **アプリ未再起動**（旧ビルド稼働＝iframe がまだ `sandbox=""` で JS 剥がし。空シェルが証拠） ② **`<issue.dir>/BEZIER.md` はディスク上の永続ガイド**で、旧ビルドが書いた「静的描画/JS禁止」テキストが残存し、エージェントはそれを読む。BEZIER.md は `buildImplementHandoff` のたびに `writeFile(guidePath, bezierGuide(issue))` で**再生成**（`issues.ts:847`）＝**新ビルドで次の handoff が走れば自動で治る**（`bezierGuideDoc` は更新済 `designBlock` を再利用）。あわせて『inline CSS のみ』と読める**残り4箇所**（`reviseKeepConvention` ＋ `/design` コマンド説明・JA/EN）を「**インライン CSS と JS は OK**」に統一。再ビルド→`/Applications` 差し替え（16:57・size 12950360）。検証：`out/`（埋め込み元）に新ガイド文言あり・旧『インライン CSS のみ』消滅・design `allow-scripts` あり。tsc 0 / eslint 0 / vitest 105。**学び＝プロンプト/ガイド変更を効かせるには (a) prod 再ビルド＋アプリ再起動 (b) 既存 issue の on-disk BEZIER.md は次 handoff で再生成、の両方が要る。**

---

## DEC-142 (2026-06-23) — コア価値監査 → maker-loop 刷新 → dogfood 反復（21コミット・main `19a2dc4` マージ済・本番反映済）

> CEO「『PdM/デザイナーのための AI Agent Orchestrator』というコア価値が実現できているか CTO として調査して」→ CTO監査 → build-vs-buy 合意 → 実装 → ペルソナ自己レビュー → CEO dogfood で多数の方向修正。**全て main(`19a2dc4`)・origin・本番 `.app` に反映済**。詳細＝`playbook/research/2026-06-23_core-value-cto-audit.md` ＋ `2026-06-23_maker-loop-rework-spec.md` ＋ `playbook/quality-reviews/2026-06-23_persona-experience-review.md`。

- **A. コア価値監査（CTO・6エリア実コード接地）**: 結論＝「**配管(成果物/データ)はあるが体験レイヤーが薄い／看板の Orchestrator(会話で導く)が一番弱い**」。Web プレビュー＋repo-attach＋本物ログインは強い moat。
- **B. build-vs-buy 合意（wrapper でよい・全体設計が価値＝Sierra SoA 思想）**: **自作=オーケストレーション層＋repo-attach preview＋意図の継承**。**buy/wrap=md エディタ(BlockNote・無料コア・但し優先度低／main persona Mai が痛点でない)／共有コメント(Liveblocks・今は不要＝繰り延べ)／hosting(Vercel)／履歴(git)**。**ネイティブ Preview=skip**。競合 **Impeccable(impeccable.style)** は design-in-code バンドが重なるが「**継承 skill** として共存」＝自作で語彙を再発明しない（[[impeccable-competitor]]）。
- **C. maker-loop 刷新（実装）**: **E-4 非線形ループ哲学**=`bezierGuideDoc` の loopBlock（**完了ゲート無し**・返信末で次の一手を1つ提案＝前後どちら）。状態機械は**不採用**（CEO「Spec完了 という状態はない」）。**E-1a** モック生成が repo トークン/部品を読む＋実装は実トークンに突合（magic number 禁止）。**alt3 もブランド忠実化**（グレースケール廃止）。**E-1b** モック iframe で Layer/Style/テキスト編集→ファイル直接書き戻し（`mock-edit.ts`/`visual-edit-transport.ts`）。**IA**=モードをキャンバス側サーフェス別バーへ・Annotate→**Comment** 改称。**terrain は追加→CEO dogfood で「人にもAIにもノイズ」→撤去**。
- **D. ペルソナ自己レビュー → CEO dogfood 反復（実装）**:
  - **送信＝「走ってる agent のチャットに inject」（再起動しない）**: `session.injectToAgent`（bracketed paste＋CR）＋`injectOrFeedback`。注釈/コメント全サーフェスがこの1経路。**linchpin＝Claude Code TUI に paste が入るか（要 dogfood）**。
  - **コメントは溜めて一括送信**: md=**テキスト選択コメント**（意味アンカー・XY座標でない）→Notion風サイドバー→一括。mock=要素コメント。pin=「Instruct all at once」撤去（各 pin の文が指示）。
  - **mock Edit 統合**: 要素 Pick→コメント/テキスト/スタイルを1フロー。**Pen/Pin 合流**＝**[選択｜注釈]** ツール切替（mock は iframe ＝注釈レイヤ共存可）。**要素3案の土台**（3案は必ず**1 HTML**・横/縦は agent 選択＝1枚で見比べて判断）。
  - **NextStepCard**: agent が `<issue.dir>/next-step` に提案 → チャット上にカード＋ワンクリック続行。
- **E. ハンドオフ→PR（最終形）**: `docs/handoff/<id>.md` の**コミットを廃止**（実チームエンジニアが「不要ファイル」と指摘）→ PR 本文へ。compare-URL prefill は **URL 長すぎで GitHub エラー**（"Whoa there! Your request URL is too long"）→ **`gh pr create --draft --body-file`**（handoff 全文を本文に・**URL 制限なし・ファイル非コミット・人が Ready 確認・本物の PR URL を保存**）。Rust `gh_pr_create` に `--base`/`--draft`。**前提＝`gh auth login`**。
- **F. クリーンアップ**: dead code（terrain 残骸・未使用 i18n/関数）除去＋共通化（`injectOrFeedback`/`batchCommentsPrompt`）＋i18n パリティ。**main 全体 tsc0/eslint0/vitest105/cargo green**。
- **🔜 残（次パス候補）**: ① **inject の手触り**（再起動せず走るチャットに入るか）と **E-1b の WKWebView 注入** を CEO dogfood で検証。② 要素3案の **カルーセル→Accept→書き戻し**（Impeccable 完全版）。③ BlockNote（round-trip ゲート先行）④ DS ガバナンス（Priya）⑤ 共有フィードバック。**現ステージ＝Idea→MVP（dogfood で芯を証明中）／次＝CEO 普段使い→必要時レビュー**。

---

## DEC-141 (2026-06-22) — dogfooding 修正5件（#1・#4 実装／#2・#3・#5 は方向確定・次パス）

> CEO dogfooding フィードバック5件。#3・#5 は「どう思う？」相談→意見＋承認。今回 **#1・#4 を実装**、**#2・#3・#5 は方向確定済みで専用パスへ**（#5 は重い preview ライフサイクル改変のため独立実装）。

- **#1 タブ保持（実装）**: Design/Prototype エリアは `issues/page.tsx:967` で**条件マウント**＝⌘⇧[]切替で相手が unmount→サブタブ/Preview パスがリセット。新規 **`src/lib/view-state.ts`**（per-issue 一時キャッシュ）で **Design の selected / Prototype の tab / Preview の path** を remount 跨ぎ保持。3コンポーネント（`issue-design`/`build-review`/`preview-pane`）が lazy-init＋effect で読み書き。DEC-133 の changed-route と両立（remount は status=ready で becameReady 不発＝上書きしない）。
- **#4 通知の [untitled] 解消（実装）**: 原因＝issue は "Untitled" 既定でタイトルは spec H1 から後追い設定、その前の通知が "Untitled"。修正＝`handleStart` で**最初のメッセージから暫定タイトルを即設定**（`issue-domain.titleFromMessage`・`isUntitled` 時のみ・persist＋sidebar も改善）＋通知は `notifyTitle = 実title || provisional || "Bezier"` で **"Untitled" を構造的に出さない**。
- **#2 複数案は1HTMLにまとめる（確定・次パス）**: 現状 1案=1タブで agent が 1/複数HTML を不安定生成。→ variant 生成プロンプト（`prompts.ts`）＋`alt3` スキルに「**同時生成の複数案は必ず1HTMLにまとめる**」を明文化。注意＝N案からの個別 Adopt 粒度は消える（比較主目的なら可）。
- **#3 Open PR はプリフィル（確定・次パス）**: 現状 `gh_pr_create` で**完全作成**。→ **branch push→GitHub の PR 作成ページを title/body プリフィルで開く→人間が Create**（外向きアクションは人間が最終確定・diff レビュー可・gh 依存も外れる）。本文は簡潔に（handoff は既に diff 内）。
- **#5 Preview を attach-first に（確定・次パス・最重要）**: 真因＝**任意スタックの dev サーバ自動検出・自動起動**が壊れやすい（DEC-121〜130 は全部その綻び）。表示は DEC-120 で既にネイティブ webview＝cmux 方式。→ **「エージェント/人が localhost 起動→Bezier は URL を映すだけ」を主役に**（DEC-129 attach を昇格）。自動起動は便利オプションに格下げ＝critical path から外す。
- **検証**: tsc0/eslint0/vitest90。Rust 変更なし。本番ビルド→/Applications。

**追補（同日・#2/#3 実装）**:
- **#2 複数案は1HTMLにまとめる（実装）**: 真因＝`design` コマンド（`prompts.ts` JA/EN）が**「複数ファイルでも1htmlでも可」と両許容**していた（＝不安定の元）。→ `design` コマンド＋`variantBody`（JA/EN）＋`~/.claude/.../alt3.md` を「**同時生成の複数案は必ず1つの html に横並び（カラム/グリッド）でまとめる・案ごとに別ファイルにしない**」に統一。各案にラベル（案 01/02…）＋トレードオフ1行。Design ボードには1つの比較タブとして並ぶ（個別 Adopt 粒度は消えるが CEO 承認済）。
- **#3 Open PR プリフィル（実装）**: `openPR` を **branch push → `gitHubSlug(gitRemoteUrl)` で owner/repo 解決 → GitHub の `compare/<base>...<branch>?expand=1&title=&body=` を `openExternal` で開く → 人間が Create** に変更。`ghPrCreate`/`buildPrBody`/`setPrUrl`/PR-created 通知を撤去（PR URL は人間作成後なので Bezier は保持しない）。本文は簡潔（spec/決定/QA は既に diff の `docs/handoff/` に同梱）。非 GitHub origin はエラー表示。i18n `session.prPrefill*`/`prNotGitHub`。
- 検証 tsc0/eslint0/vitest90。Rust 変更なし。本番反映。

**追補（#5 attach-first・設計確定＋staging）**: CEO 体験像＝「**AI Agent が能動的に dev サーバ起動 → Bezier が検知したら自動でブラウザを開く**／手動 URL はフォールバック（cmux 型）」。真因＝表示は DEC-120 で既に webview、脆いのは「任意スタックの dev サーバ自動検出/起動」。**最も sensitive な subsystem（DEC-120〜130）を触る大改修＋実機検証不可**のため、丁寧に staging：①エージェント起動＋URL報告（プロンプト規約・低リスク）を**先行実装・出荷**／②自動検知＋③attach-first UI（待機が主役・手動URL＋自動起動はフォールバック）は**専用パス**。確定設計＝`playbook/research/2026-06-22_preview-attach-first-spec.md`。①でエージェントは「Bezier はサーバを起動しない・起動済みを映す・URL を報告して」を毎ターン認識する。
- **②の改訂（CEO 指摘・重要）**: 当初案「common port を `httpPing` 探索→最初の応答」を**撤回**。理由＝**複数イシュー同時進行では各 worktree が別ポートで dev サーバを立て、A の preview が B のサーバを誤検知する**（ポートで紐付けるのが誤り）。→ **ポートでなく worktree で紐付ける**：**(a) per-issue URL 宣言**（エージェントが `<issue.dir>/preview-url` に書く→Bezier はそのイシューのファイルだけ読む・曖昧さゼロ）＋**(b) lsof worktree-scoped 検出**（cwd が worktree 配下の listen ポートのみ採用）。両方 issue-correct。CEO 確定＝(a)+(b)。

**追補（同日・#5 ②③ 実装）**: 設計（`2026-06-22_preview-attach-first-spec.md`）どおり ②自動検知＋③attach-first UI を実装。**スコープは「待機状態のみ」に限定し、既存の管理起動 / attach(DEC-129) / Live / Tauri を壊さない**。
- **②(b) Rust `discover_worktree_url(worktree)`**（`lib.rs`・handler 登録）: macOS `lsof` 2パス＝①`lsof -nP -iTCP -sTCP:LISTEN -Fpn` で **loopback/ワイルドカードの LISTEN ポートを pid 別に収集**（`f`fd 行は無視・`*`/127.0.0.1/0.0.0.0/[::1] のみ採用＝LAN IP は除外）→②`lsof -a -d cwd -p <csv> -Fn` で **各 pid の cwd** を取り、**cwd が（canonical）worktree 配下のものだけ**採用→`http://localhost:<port>/` を昇順で返す。lsof 不在(非macOS/CI)や不一致は空配列（エラーにしない）。実機 lsof 出力で `-F` フィールド形式（`p`/`f`/`n`）を実測検証済。
- **②(a)(b) reader（`use-preview-server.ts`）**: 新規 effect（**issue worktree 限定**＝`worktreePath !== root`・**web runner 限定**・`!externalUrl`）が 1.5s ポーリングで **(a) `<issue.dir>/preview-url` を読む**（`readDeclaredPreviewUrl`／純関数 `parseDeclaredPreviewUrl`＝prose 内 URL 抽出・origin 正規化・**loopback 限定=SSRF 封じ**）→ 無ければ **(b) `discoverWorktreeUrls`** → `httpPing` で生存確認 → **adopt**（attach 同様 setUrl+ready・`autoAttached=true`）。**managed が動いていれば必ず譲る**（ptyIdRef/startingRef/registry を ref で生読み）／adopt 後は生存監視＝死んだら待機へ／cleanup で auto 状態を解放（manual URL=attach が上書き・既存 attach effect が処理）。新 param `declUrlFile`（issue=`previewUrlDeclPath(issue.dir)`／Live=未指定）。
- **③ attach-first UI（`preview-pane.tsx`）**: 旧「未起動 EmptyState（Start を押せ）」を **待機カード**（スピナ＋「dev サーバの起動を待っています…起動すれば自動で開きます」＋**手動 URL 入力**＋「**Bezier に起動させる**」フォールバック＋ターミナル）に置換。ヘッダの常設 Start は**エラー時の再試行のみ**に格下げ（`showError`）。auto-attached の `RunningBadge` は **Stop を出さない**（プロセス非所有・`owned=false`）。
- **① プロンプト強化**: 既存の implement intro の preview 文 + **durable ガイド（`bezierGuideDoc`）に「プレビュー」節（`previewBlock`・JA/EN）**を新設＝「Bezier はサーバを起動しない・dev サーバを起動し続け **URL を1行だけ `<issue.dir>/preview-url` に書く**（ポート変わったら更新）＋チャットでも一言」を**毎ターン**認識させる。
- **検証**: tsc0/eslint0/**vitest96（+6＝parseDeclaredPreviewUrl/previewUrlDeclPath）**/cargo Finished。本番ビルド→/Applications（要 ⌘Q→再起動）。**要 dogfood**＝代表スタック（Next/Vite/Docker等）でエージェント起動→自動表示・ターン跨ぎ/⌘Q 後の背景 dev サーバ生存・既存 attach/管理/注釈/login の回帰無し。**lsof 1.5s ポーリングのコスト**は dogfood で要観察（待機中のみ・visible なときのみ）。

---

## DEC-140 (2026-06-22) — アプリのメニューから手動アップデート確認（GitHub に行かず・署名不要）

> CEO「v0.1.1 の前に更新の仕組みを。自動更新は無理なのは分かってる（＝署名前提）が、**GitHub に行かず手動で更新**できるように。Mac app のメニューからできるはず」。

- **形（option A の延長・署名不要）**: ネイティブ App（「Bezier」）メニューに **「Check for Updates…」** を追加（`lib.rs` の `SubmenuBuilder`・id `check-updates`）→ `on_menu_event` が `bezier://check-updates` を emit → frontend `UpdateChecker`（layout 常駐・`app-close-guard`/`MenuShortcutBridge` と同型）が処理。
- **挙動**: `getVersion()`（=tauri.conf の version）と GitHub `releases/latest`（`fetch`）の tag を比較。最新なら `messageDialog`「最新です」。新しければ `confirmDialog`（DL するか＋**DL後の drag＋`xattr` 手順を本文に明記**）→ OK で `openExternal(.dmg URL)`。失敗時はメッセージ。
- **配線**: CSP の `connect-src` に **`https://api.github.com`** を追加（webview から release API を叩く・GitHub は CORS 許可）。version を **0.1.0→0.1.1**（`tauri.conf.json` ＋ `Cargo.toml`）。i18n `update.*`（en/ja）。
- **限界（正直）**: これは**自動更新ではない**。インストールは依然 **drag→Applications＋初回 `xattr`**。でも「メニューでチェック→新しければ DL を開く」で **GitHub を探さなくて済む**。真のワンクリック更新（ダウンロード＋in-place 置換）は **Tauri updater＋署名（option B）** で後段。
- **検証**: tsc0/eslint0/vitest90/cargo green。本番ビルド→/Applications（要 ⌘Q→再起動）。この後 **v0.1.1 タグ→GitHub Release** を発行（DEC-138 のパイプライン）。ブランチ＝main 直。

---

## DEC-139 (2026-06-22) — dogfooder 配布前の「作り手だけは踏まなかった穴」を塞ぐ（A/B/C/E+G）

> CEO「dogfooder に配ると思うと、足りない機能/体験・見落としは？」→ 実コード接地でギャップ分析（`playbook/research/2026-06-22_dogfood-distribution-gaps.md`）→ A・B・C・E+G を承認・実装。

- **根本**: Bezier は「ユーザーの repo で・ユーザーの agent に委譲」＝前提の大半がユーザー環境側。作り手(CEO)は全部揃っていて無痛だったが、**dogfooder は持っていない/知らない**＝穴は機能でなく前提・初回・オンボーディングに集中。
- **A 認証ガイダンス**: agent の「ログイン済みか」を**堅牢に自動判定するのは困難**（claude 等の保存先がバラバラで壊れやすい）と判断 → **強いガイダンス**で実装。`issue-agent-panel` の `ChatStart` で `!agentAvailable` 時に「claude/codex を入れて**一度ログイン**してから戻る（自動再検出）」を本文表示（従来は placeholder だけ）。`agentPanel.noAgentGuide`。
- **B+E オンボーディング**: `issues/page.tsx` の `NoFolder` 空状態を**welcome 化** — ①Bezier とは ②前提チェックリスト（**agent インストール＋ログイン**／git／プレビューに Node）③信頼の一文（「あなたの agent を・あなたの repo で動かす＝あなたの Claude サブスク使用／変更は隔離 worktree・main は PR 経由のみ」）。新 `welcome` i18n 名前空間。
- **C 共有の既定を中立化**: `DEFAULT_CONNECTIONS` の scope を **`"bezier"`（CEO の Vercel team）→ `""`（空＝サインイン中ユーザーの個人アカウント）**＋ label を「Personal (your Vercel)」に。新規 install の共有が**他人(CEO)のチームに誤爆しない**。⚠️ **CEO 影響**: これまで既定の bezier scope に依存して publish していた場合、Settings → Publish connections で **bezier 接続を1回追加**すれば従来通り（保存済み接続があれば無影響）。
- **G フィードバック経路**: サイドバー下部に「フィードバック」→ `openExternal` で **GitHub new-issue**（body テンプレ付き）。dogfooder の壊れ方を回収（クラッシュは依然ローカルログのみ＝Sentry 無しの方針は維持）。`sidebar.feedback/feedbackBody`。
- **検証**: tsc0/eslint0/vitest90。Rust 変更なし。本番ビルド→/Applications。**P2 は別途**＝署名/notarize/自動更新（option B）・サンプル repo/ガイド付き初回タスク・arm64 のみ→universal。ブランチ＝main 直（これまで通り）。

---

## DEC-138 (2026-06-22) — 配布: GitHub Releases に .dmg を自動ビルド（option A・未署名）

> CEO「GitHub に挙げて最新版を落とせるようにしたい。不要？」→ 「自分で・別Macでも更新したい」ニーズを満たす良い一手と判断。署名/auto-update（option B）は配布拡大時に分離。

- **現状整理**: 日常使い = `/Applications/Bezier.app`（ローカル `tauri build`→ditto・**adhoc 署名のみ**）。更新はビルド実行時だけ＝私無しで自己更新する手段が無かった。既存 `ci.yml` は品質チェック専用で配布物を作らない。
- **対応 = `.github/workflows/release.yml`**: タグ `v*` push（or 手動 `workflow_dispatch` で tag 入力）→ `tauri-apps/tauri-action` が **macOS .dmg（Apple Silicon/arm64）** をビルド → **GitHub Release に添付**。Node20/`rust-toolchain@stable`/rust-cache は ci.yml に合わせ、`projectPath: app`、`args: --bundles dmg`、`permissions: contents: write`。
- **初回リリース実証**: `v0.1.0` を発行 → run 成功 → **`Bezier_0.1.0_aarch64.dmg`（~4.6MB）** が Release に。URL: https://github.com/Sota-Mikami/bezier/releases/tag/v0.1.0
- **未署名（adhoc）の代償**: 落とした .dmg は初回起動で Gatekeeper 警告 → システム設定→プライバシーとセキュリティ→「このまま開く」で一度許可（Release body に明記）。**Developer ID 署名＋notarize＋Tauri 自動アップデータ（option B）は他人配布フェーズで**（[[production-readiness-observability-decision]] の「自分のMac外に配るまで署名保留」を踏襲）。
- **運用**: 新版は**タグを切るだけ**（`git tag vX.Y.Z && git push origin vX.Y.Z`）。`tauri.conf.json` の version とタグは別管理（揃えると dmg 名と一致）。ローカルビルド→ditto の従来運用も併存可。
- 既知の注意: GitHub が actions の Node20 を Node24 強制実行（非ブロッキング警告・将来 setup-node 等のメジャー更新で解消）。

---

## DEC-137 (2026-06-21) — 通知トリガーを拡張（共有 ready / プレビュー / リポ準備 / PR）

> CEO「その他、通知が必要な内容を特定して。例：共有の準備に時間がかかるなら、共有 ready で URL 発行されたら通知」。完了点を洗い出し → multiSelect で**4種すべて承認**。

- **共通ルール**: すべて **`!document.hasFocus()` ゲート**（見ている時は鳴らさない＝過多防止）＋ DEC-136 の `notify()` 再利用＋クリックで該当 Issue。エージェント完了（DEC-136）はそのまま。
- **①共有 ready（URL 発行）**: `use-journey.ts` の deploy **exit ハンドラ**（`status="ready"`＋`stableUrl`）で通知。**mount 復元（DEC-135）では鳴らさない**＝実完了時のみ。`journey.notifyShareReady`。
- **②プレビュー ready / 起動失敗**: `use-implement-session` に `preview.status` 遷移 effect を追加（`starting→ready`／`starting→error`・最大150秒待ちの離席対策）。`session.notifyPreviewReady` / `notifyPreviewError`。
- **③リポジトリ準備完了**: `use-readiness` の `fixAll`（「全部準備する」＝依存/Node インストール・数分）完了後に通知。repo 単位なので issue target なし。`session.notifyPrepared`。
- **④PR 作成完了（URL）**: `openPR` の `setPrUrl` 後に通知（body＝既存 `session.prCreated` の URL 付き文）。
- i18n は en/ja 追加（session 3 + journey 1）。**検証 tsc0/eslint0/vitest90。Rust 変更なし**。本番ビルド→/Applications。要 dogfood（特に共有 ready は実際に離席して確認）。ブランチ同上（未コミット）。

---

## DEC-136 (2026-06-21) — push 通知を公式 Tauri 通知プラグインで「ちゃんと」実装（フル）

> CEO「push 通知をちゃんと実装したい。今どんな状況？」→ 調査で**2系統がアドホック併存**と判明。フル実装を承認。

- **現状（before）**: ①Rust `notify`＝`osascript 'display notification'`（DEC-118・macOS のみ・通常「Script Editor」名義でアイコンが Bezier にならない）が**開いている Issue**用 ②webview の `new Notification()`（`app-sidebar`）が**裏の Issue**用。バックエンドが割れ・背景動作/identity が不安定・クリックで何も起きない・On/Off なし。
- **対応＝公式プラグインに一本化**: `tauri-plugin-notification`（Rust・`lib.rs` で `.plugin(init())`＋capability `notification:default`）＋ `@tauri-apps/plugin-notification`（JS）。新規 **`src/lib/notify.ts`** の単一 `notify({title,body,target})` に両 call site を集約（`use-implement-session`＝開いてる Issue・focus 外時／`app-sidebar` notifyAttention＝裏 Issue）。Bezier 名義＋アイコンで通知センター・背景発火・権限 API。
- **権限オンボーディング**: `ensureNotificationPermission()`＝エージェント起動時（focus 中＝出しても自然）に warm-up＋初回 notify で lazy 要求。`isPermissionGranted`/`requestPermission`。
- **クリック→フォーカス＋該当 Issue**: 各通知に 32bit `id` を振り `Map<id,target>` 保持。`onAction` で window を `unminimize/show/setFocus` ＋ `bezier:open-issue` CustomEvent → sidebar が `issuesByRepo` から repo 解決して `selectIssue(repo,id)`。
- **設定 On/Off**: `Settings.notifications`（default true・migrate 追加）＋ Settings に Section（`OnOffToggle`・ON 時に権限 warm-up）＋ i18n `settings.notifications.*`（en/ja）。`notify()` は設定 OFF で no-op。
- **掃除**: Rust `notify`＋`applescript_quote`＋handler 登録＋`ipc.ts notify` を撤去（web Notification も廃止）＝2系統を1系統に。
- **MSRV 対応**: plugin の transitive `mac-notification-sys 0.6.15` が edition2024（Rust 1.85）要求 → **`mac-notification-sys = "=0.6.4"` をピン**（このプロジェクトの MSRV ピン方針に沿う・1.83 toolchain で build 可）。`notify-rust 4.17` で解決。
- **検証**: tsc0/eslint0/vitest90/`cargo check` green。本番ビルド→/Applications 反映（要 ⌘Q→再起動）。**要 dogfood 確認**: 実通知の見た目（Bezier 名義/アイコン）・背景時の発火・クリックでフォーカス＆該当 Issue 遷移・**未署名 .app での通知許可挙動**（macOS は署名なしだと通知が出にくいことがある＝出なければ署名/配布フェーズ要）。ブランチは同 `feat/2026-06-19/preview-changed-route-and-map-screenshots`（未コミット）。

---

## DEC-135 (2026-06-20) — 共有 URL をディスクに永続化（再起動しても Share から見つかる）

> CEO「すでに共有済みなら URL がずっと残ってほしい。再起動しても Share から見つけられてほしい。今は再起動すると消える」。

- **真因（2層）**: ①**最終的な共有ページ URL（`use-journey.ts`）は `useState(null)` のみで一切永続化されていなかった**＝再起動で確実に消える（本命）。②アプリ URL（`use-publish.ts`）は localStorage 保存だが、**prod の WKWebView localStorage が再起動を跨いで永続しない**ケースがある。`previewKey = issue.id`（ULID）は安定なので、原因はストレージ側。
- **対応＝ディスク永続化（`.bezier`・gitignored・issue.id キー）**:
  - `use-journey.ts`: `<root>/.bezier/share-urls.json`（`{issueId:url}`）に**共有成功時に保存**、**mount 時に復元**（`setUrl`＋`status="ready"`）、**Discard(`clear`) で削除**。URL は `https://{vercelProjectName(id)}.vercel.app`＝issue.id から決定的。
  - `use-publish.ts`: 同型で `<root>/.bezier/publish-urls.json` にアプリ URL を永続化（localStorage は高速表示用に併用）。
- **効果**: 再起動後に Issue を開くと journey が `ready` に復元 → Share パネルが既存 URL＋[コピー]/[開く]＋「再共有」を表示（`issue-share.tsx` 既存 UI）。つまり**共有済み URL がずっと残り、Share から見つかる**。
- **検証**: tsc0/eslint0/vitest90。Rust 変更なし。本番ビルド→/Applications 反映（要 ⌘Q→再起動）。ブランチは同 `feat/2026-06-19/preview-changed-route-and-map-screenshots`（未コミット）。メモ＝[[share-vercel-deploy-naming]] 系。

---

## DEC-134 (2026-06-19) — 被共有（受け取る側）ページの UX 改善6点

> CEO 依頼（被共有側体験のアプデ）。対象＝`app/src/lib/journey.ts`（共有ページ生成）。DEC-117/118 のヒューリスティック指摘の一部（maker 向け lead 撤去・受信者 IA）も同時に解消。1バッチで本番反映（要 ⌘Q→再起動）。

1. **maker 向け lead 撤去**: トップの「Not just the finished result — a record of how it was made.（出来上がりだけでなく…）」＝邪魔、削除（`journey.footerLead` 撤去＝DEC-118 §新規発見①の解消）。
2. **パスワードのデバイスキャッシュ**: 一度開けたら、ゲートページが PW を `localStorage`（`bz-pw-<salt>`・TTL ~7日）に保存し、再訪時は**自動解錠**（PBKDF2 再導出）。ローテーション（復号失敗）でキャッシュ自動破棄→フォーム再表示。秘密は受信者端末のみ（ソフト保護の範囲・JSDoc 明記）。
3. **被共有側にも ToC**: doc タブで見出し（h1–h3）から**目次**を生成（見出しに id 付与・アンカーリンク＝スクリプト不要で strict CSP 維持）。**レベルでインデント**（lv2/lv3 を字下げ）。見出し2個以上の時のみ表示。スティッキーヘッダ下に隠れぬよう `scroll-margin-top`。
4. **トップページを大幅にワイド化（プロト Header+Contents 構成）**: 旧 `.wrap max-width:860px`（→Preview が小デバイス幅で窮屈）を廃し、**スティッキー `.topbar`（タイトル+meta+badge）＋ `.page`（max-width 1280px）**へ。Preview/デザイン iframe を `height:min(82vh,880px)`＝大きく表示。doc 本文は可読性のため `max-width:880px`。"Made with Bezier" は topbar に1回（footer の二重表示解消）。
5. **md 表示の修正**: `inlineMd` に**打ち消し線 `~~..~~`→`<del>`** と**イタリック `*..*`/`_.._`→`<em>`** を追加（bold `**` の後に処理・lookbehind 不使用で旧 WKWebView 対応・`_` は語境界限定で snake_case を誤イタリック化しない）。

**検証**: tsc0/eslint0/vitest90（journey.test +4＝italic/strike・ToC・no-ToC・wide-layout）。既存テスト（CSP/hash script/sandbox/escape/task list/segbar）全維持。Rust 変更なし。本番ビルド→/Applications 反映。ブランチは DEC-133 と同じ `feat/2026-06-19/preview-changed-route-and-map-screenshots`（未コミット・2概念同梱＝PR 時は分割候補）。**Map と同様、見た目は dogfood 確認推奨**。

---

## DEC-133 (2026-06-19) — プレビューは「変更したページ」を開く＋Map をスクショ方式で認証アプリ対応

> CEO「エージェントがターンを返すたびプレビューを開くのは良いが、**必ずトップ `/` に行く**。トップ以外を変更している時は**その変更ページ自体**を開けるようにしたい」＋「**Map が認証アプリで必ずログイン画面しか出ない**ので調査・対応して」。実コード調査→ CEO 承認（#1=**(a)+(b)**／Map=**A スクショ方式**／**1バッチで本番反映**）。

**#1 変更ページを開く（実装＝a / 保留＝b）**
- **(a) 実装済**: 新規 `app/src/lib/changed-route.ts`（純粋関数）= Next.js App/Pages Router のファイル→ルート導出（route group `(x)`・並行 `@slot` 除去、動的 `[id]` は静的接頭辞で停止、複数変更は最深ルート優先、トップ/page以外/不明スタックは `null`＝従来 `/` に安全 degrade）。`preview-pane.tsx` に配線：**プレビュー ready 化時＋エージェントのターン完了時**に worktree の `git status`→変更ルートを導出し、`/` でなくそのページへ自動遷移（DEC-121 のナビ基盤＝`setPath`+`reloadNonce` を再利用・`pathRef` で手動ナビを尊重し次ターンで再追従）。単体テスト10件。
- **(b) 保留（理由）**: 「エージェントが宣言したルートを自動で読む」は、`thread` が**構造化イベント（type/note）のみでエージェント自由テキストを保持しない**（実出力は pty 側）ため今の配線では拾えない。別チャネル（エージェントにマーカーを書かせる＋プロンプト多言語調整＋eval＝[[prompt-tuning-multilingual-eval]]）が必要なので別作業に分離。(a) だけで「変更ページを開く」核は成立。

**Map = A スクショ方式（login-critical コーディネータ非改変）**
- 原因＝`issue-map.tsx` が各ルートを `<iframe>` 表示（DEC-120 で Preview/Live はネイティブ webview 化したが Map は iframe のまま）→ 認証アプリでセッションを持てず**全タイルがログイン画面**。
- 対応＝**Preview が既に持つ認証済み webview ＋ `captureRegion`（注釈フリーズで実証済）＋パス遷移を再利用**。Map の「Map を更新」→ BuildReview が Preview に切替＋`captureReq.nonce` bump → PreviewPane が各ルートへ順送り遷移→スクショを `<issueDir>/map/<slug>.png`（granted .bezier）へ保存→ Map に戻り**静止画タイル**表示（未撮影は placeholder・保存済は Preview 未起動でも閲覧可）。**単一 webview コーディネータ（DEC-120/130＝login の要）には一切触れない**＝退行リスク回避。共有ヘルパー `scope.ts: routeSlug/mapStillPath`。i18n `map.updateMap/capturing/notCaptured/captureHint`（en/ja）。
- **トレードオフ**: 「常にライブ」設計注記を覆し**撮影スナップショット（手動更新）**に。ターン後自動再撮影は fast-follow（今回は手動更新ボタンが first cut）。

**検証**: tsc 0 / eslint 0 / vitest 85（changed-route 10 新規）。Rust 変更なし。**本番ビルド→/Applications 反映を実施（⌘Q→再起動が必要）**。**Map-A はログイン＋目視の dogfood 確認待ち**（capture の settle タイミングは実測で要調整の可能性）。ブランチ `feat/2026-06-19/preview-changed-route-and-map-screenshots`。

**追補（同日・best UX 仕上げ）**: CEO「1,2 それぞれベストな UX に」。
- **#1**: 複数変更ページを **mtime ランクで最新を自動オープン**（`pathMtime`・「今いじったページ」を優先）＋**変更ページのチップ列**（プレビュー上ストリップ・クリックで即移動・現在地ハイライト・`preview.changedPages`）。`changed-route.ts` に `deriveRoutesFromChangedFiles`（全候補＋元ファイル）追加・テスト計86。
- **#2**: 固定待ち(500/1500ms)を **`navigateAndSettle`（実 URL 着地をポーリング＋paint・auth リダイレクトは timeout で何かは撮る）** に置換＝撮影の信頼性↑／**ターン後に変更ページの Map 静止画を自動更新**（プレビュー可視時のみ・追加チラつき無し＝#1 と統合し「ターン後自動再撮影」を実現）／**一括撮影の進捗をヘッダ表示**（ネイティブ webview は HTML を重ねられず pane 上オーバーレイ不可→ヘッダに `Map を撮影中 n/total /route`）。
- 検証 tsc0/eslint0/vitest86。Rust 変更なし。本番再ビルド→/Applications（要 ⌘Q→再起動）。**Map 撮影の見た目（チラつき/タイル品質）は要 dogfood 確認**。

---

## DEC-132 (2026-06-19) — Bezier を「あらゆるコーディングエージェント対応」に（agent-agnostic v1）

> CEO「Claude Code 前提で作ったが、実体はローカルラッパー。思想として Claude に縛りたくない。Codex 等、ユーザーがローカルで普段使うエージェントを自由に使える設計にして。①通知の共通化 ②起動の汎用化 ③切替 UX を含め網羅的に」。3 Explore で結合マップ→Plan agent で adapter 設計→CEO 承認（**フル v1・組込=claude/codex＋任意 CLI のカスタム**）。実装・本番反映（18:30）。詳細設計＝`playbook/research/2026-06-19_agent-agnostic-design.md`。

**核 = AgentAdapter（能力宣言レジストリ・`app/src/lib/agent-adapters.ts`）**: `if(id==="claude")` 分岐を、prompt 渡し方/resume/contextDirs/settings(hooks)/headless/notify/commandsDir/conventionFiles/newline/template をデータ宣言する adapter に置換。`buildLaunch()` が cmd/args/initialInput/notify/eventsPath を生成（claude の variadic 安全な順序を封入）。組込=claude(全機能)/codex(idle・no-resume・fold・AGENTS.md)、`customAdapter`=任意 CLI（argv テンプレ `{prompt}`/`{cwd}`）。

**①通知共通化**: 状態を exit code（全）＋hooks（claude）＋**idle（非hook 新規）**で統一。Rust `pty_statuses` を strategy 別（hooks=events 増／idle=had_output && 無出力≥idle_ms／exit-only）。had_output ガード＋`pty_write` で clock リセット＋idle の waiting は OS 通知抑制。**②起動汎用化**: launchAgent/handleResume/fixWithAgent を buildLaunch 化、isClaude 撤去。prompt は per-adapter（未知は stdin 既定で安全）。**③切替 UX**: detectAgents が組込＋custom 統合→既存ピッカー反映、codex coming-soon 解除。Settings に**カスタムエージェント追加 UI**＋能力ヒント（resume?/待機検出精度/継承規約ファイル）。

**moat**: 各エージェントが自分の規約ファイル（CLAUDE.md/AGENTS.md/.cursorrules 等）を読む＝Bezier は spec を渡すだけ＝自然に汎用化。

**コミット**: ad31f37(adapters+buildLaunch+test)／9235367(call-site refactor+custom 検出+settings)／f4bcad0(Rust idle-waiting)／e5e0d2e(custom UI+能力ヒント+文言中立化)。claude 挙動はバイト一致。tsc0/eslint0/vitest75/cargo green。

**v1 延期（記録）**: headless(`claude -p` の env 判定)は claude 限定据置（`.env` 物理遮断の deny が claude `--settings` 依存・非対応は推論スキップで安全）／slash パック(`~/.claude/commands`)claude 専用据置(非 claude 無害・baseline はインライン)／terminal Shift+Enter newline 列据置／per-issue adapterId 永続／cursor・aider・gemini の組込 adapter（実フラグ検証込み）＝今は custom 登録でカバー。

---

## DEC-131 (2026-06-19) — Preview に Figma/Framer 風ビジュアル編集（Edit Mode：Layer/Style パネル＋実コード反映）

> CEO「Preview の上に Layer パネル（左）＋Style パネル（右）で、Figma 慣れユーザーが GUI でスタイル・並び替え。GUI 編集は inspect 相当で可能なはず。実コード反映が、スタック自由な今の論点」。→ 3 agent（head-of-product/principal-designer/principal-engineer）でフル spec 化（`playbook/research/2026-06-19_visual-editor-spec.md`）。CEO 追加要望＝① Edit を **Mode** 化し Preview Header に追加 ② Header の **Resize 系を 1 ボタンに集約**（散らかり整理）③ Edit と Annotate は**排他**（同時不可に注意）。「その他は提案通りまず作って→見ながら UX 調整」。本 DEC で CP1（UI/モード）＋CP2（編集エンジン）を実装・本番反映。

**確定方針（CEO）**: 反映方式 = **まず agent 経由のみ**（ライブ編集→構造化 intent→ユーザーの claude が repo の作法でコード化・既存 sendDesignFeedback レール再利用・スタック非依存）。**将来＝ビルド計装で決定論を idiom 横断に拡大**（Tailwind 限定にしない・spec §7 / 着手時に本ログ＋memory へ正式記録）。

**CP1（コミット bdf0053）— UI/モード**
- **Resize 集約**: `ResizeControl`（base-ui DropdownMenu＝role=menu で webview 上に重なる＝既存 overlay-freeze が効く）に device プリセット＋回転を集約。custom W×H のみ header（webview の上＝常時可視）。旧アイコン群/回転/サイズ表示を撤去。
- **Edit Mode トグル**を Preview Header に追加（app 稼働中のみ）。
- **Edit⊻Annotate 排他**: `annotation-mode` に `locked` 追加。Edit ON 中は ⌘⇧A / Annotate トグルが no-op＋disabled、Edit 突入で annotate を強制 OFF（注釈は webview を screenshot 凍結・Edit は live 必須＝物理的に同時不可）。

**CP2（コミット 49b08d0）— 編集エンジン**
- **チャネル（Tauri 2.11 で検証）**: plain `eval` は値を返さない／`eval_with_callback` は JSON 直列化値をコールバックに渡す。→ Bezier→page = `embed_browser_eval`（apply/activate/inject）、page→Bezier = `embed_browser_drain`（eval_with_callback→`bz-edit` イベント）。**page に Tauri IPC を一切付与しない**＝DEC-130 の SSRF/IPC 方針と矛盾しない。
- **`bezier-overlay.ts`**: webview に注入する in-page agent。closed Shadow DOM の選択/hover ボックス、cssPath、computed 読取、inline ライブ適用、イベントキュー。冪等・ナビ後再注入。
- **`use-visual-edit.ts`**: Edit ON で注入+activate / ナビで再注入、drain ループ＋`bz-edit` listen、applyStyle（diff 記録）、selectParent/selectPath、clearEdits。
- **`visual-edit-panels.tsx`**: Layer パネル（祖先/選択/子・クリックで階層移動）＋Style パネル（curated グループ・ライブ適用・color picker）＋Pending bar。
- **preview-pane**: Edit 中は 3 ペイン（Layer｜webview｜Style）。webview は flex-1 で縮み ResizeObserver 追従。「コードに反映」= `sendDesignFeedback(visualEditPrompt, "visual_edit")`／「破棄」= reload で inline を戻す。
- **`visualEditPrompt`（en/ja）**: diff を要素ごとにまとめ、**repo の作法で書け**（Tailwind/CSS/token/CSS-in-JS）と指示。
- i18n: `edit.*` 新設＋`ve.*`／Resize・Edit 系キー（en/ja parity）。

**差別化**: 走っている実アプリの実状態（ログイン後・モーダル・hover・実データ）を直接編集＝静的デザインツールが触れない領域。操作感＝ライブ即時、確定（コード反映）は別ステップ（agent 数秒）。

**検証**: tsc 0 / eslint 0 / vitest 69 / cargo green。本番 build → ditto → `/Applications`（⌘Q→再起動）。**これは first cut**＝CEO dogfood で UX 調整・feedback 前提（CEO 明言）。

**R1 ペルソナ検証＋CEO feedback → Figma-feel 強化（commit d6a2858・本番 12:01）**: persona-{Mai/Leo/Kenji/Priya/Tom} に触らせて収集。実装＝数値 ↑/↓ ステッパー（Shift×10/Alt×0.1）／プロパティ大幅拡張（margin L/R バグ修正・border・box-shadow・letter-spacing・flex-grow/align-self・Position+overflow）／enum ドロップダウン＋条件表示／per-prop リセット＋override 強調／Undo(⌘Z)／**Layer ドラッグ並べ替え**（ライブ＋reorder intent）／Esc→親・⌘Z（Bezier フォーカス時）／「反映するまで実コードは変わらない」明示。延期＝コピペ⌘⌥C/V・トークンガバナンス（Priya＝企業配布の前提・strict/監査/PR 必須）・テキスト編集・複数選択・webview フォーカス時のショートカット（native accel）。詳細は spec の R1 追記。

**R2（commit aa6603e＋db8e735・本番 12:19）— 並べ替え根治＋Figma/Framer UI 寄せ**: CEO「並べ替えできてない・根本調査」「Figma/Framer を研究して UI も寄せて」。① **並べ替え真因＝Tauri/WKWebView の HTML5 DnD は `dataTransfer.effectAllowed`(dragstart)＋`dropEffect`(dragover) 設定が必須**（既存 `useDragReorder` は設定済・初稿が欠落＝onDrop 不発）＋overlay moveNode を選択中の親再報告に修正＝連続並べ替え可。② **principal-designer が Figma/Framer inspector を研究→Style パネルを Figma セクション順（Frame/Position/Layout/Spacing/Fill/Stroke/Effects/Type）に再構成**：2 カラム数値ペア・整列はアイコン SegmentedControl・Fill/Stroke はスウォッチ＋HEX・条件表示・override ring＋↺。Layer はタグ別アイコンの Figma 風ツリー。割り切り＝scrub/複数Fill/blend/grid/space-around は省略。tsc/eslint/vitest69 green。

**R3（commit 448c02d・本番反映）— Layer を全ページツリー化＋↑/↓ 並べ替え**: CEO「選択物関連だけでなくページ全体のレイヤーを常時表示・選択をハイライト・↑/↓ で上下と入替」。overlay `buildTree()` でページ全体を直列化（800 ノード上限）→`tree` イベント→Layer パネルを再帰ツリー（展開/折りたたみ・タグアイコン・選択 bg-accent＋auto-scroll・同親ドラッグ並べ替え）に。`moveSel(±1)` で選択要素を兄弟内移動、**↑/↓ は overlay 側 in-page 処理**＝プレビュークリック直後（webview フォーカス）でも効く。キーボード並べ替えも reorder intent 記録。**R3.1**＝大規模ページ対策で既定全展開をやめ「2 階層の概観＋選択へ auto-expand（選択は常時可視・パスが手動 collapse に優先）＋scroll-into-view」に。**R4**＝テキスト要素（element 子なしのリーフ）選択時に Style パネル先頭に Content 欄→ライブで textContent 変更＋text intent を agent へ（`veText`）。tsc/eslint/vitest69 green。
**既知の弱点/次**: selector の脆さ（動的 class）→ tag/class/text で補強、根治は将来のビルド計装。Layer は近傍 1 階層のみ（完全ツリーは次段）。構造編集（並べ替え）は未対応。状態依存 UI の e2e は dogfood で確認。

---

## DEC-130 (2026-06-19) — preview/Live エンジニアリングレビューの修正実装（P0→P1→P2）

> CEO「エンジニア目線でめちゃくちゃしっかりレビューして」→ 3 エージェント並行レビュー＋検証で `2026-06-18_preview-engineering-review.md` に集約。CEO「P0 やって、その後 P1 も続けて。P2 モヤって良い」→ 本 DEC で P0→P1→P2 を順に実装・検証・本番反映。各バッチごとにコミット（1527e62 / abbd67b / 60f6808）。

**P0（配布前・セキュリティ＋死んだ操作）**
- **SEC-1（SSRF）**: `http_ping` / `http_probe_inner`（=`http_probe`＋`http_frame_blocked`）に `parse_local_url` の loopback ゲートを追加（Rust）。`readPreviewConfig` も `externalUrl` を `isLoopbackUrl` 検証してから採用。repo 内 `config.json` が off-box ホストを指して自動 poll/embed される穴を塞ぐ。
- **SEC-3（prompt-injection／秘密漏れ）**: `sanitizeLogTail()`（4000 字キャップ＋鍵/トークン/JWT を redact）＋両ロケールで dev ログ尾部を「UNTRUSTED DATA」フェンスで囲ってから "fix with agent" prompt に貼る。
- **QA 4.B**: attach effect の cleanup で status/url をリセット → 解除しても「ready/starting」のまま死ぬ行き止まりを解消。
- **PE P1-3**: Live の `reloadKey` を `reloadNonce` 駆動に（旧 `reloadKey={0}` 固定でパス入力＋リロードボタンが無反応だった）。
- 同ファイルで併せて: **PE P2-1**（`buildDevCommand` は既知ランナーのみ `--` ラップ。旧 `/\brun\b/` が `nx run` 等を誤爆）／**PE P2-5**（`parseDevServerUrl` は非 self ポートを優先しつつ 3210 が唯一の URL なら採用。旧 unconditional skip は 3210 で動く repo を壊した）。

**P1（ライフサイクル／webview singleton／QA）**
- **PE P1-1/1-2/1-5**: `start()` 単一フライト（startImpl＋startingRef）／reattach は start が pty を保持中なら退避／torn-down 後の config 書き込み抑止。
- **PE P0-2（webview singleton）**: Rust の単一 "embedded-browser" を複数 EmbeddedBrowser（Live は issue を開いても mount 継続）が奪い合う問題を ref-count コーディネーター化（useId 識別・owner 追跡・他 instance の webview を adopt したら自分の src へ navigate・**最後の unmount のみ close**、他が残れば hide→生存 webview を adopt）。RepoLive は実 `active` を伝播。単一 instance の挙動は不変。
- **QA**: 1.B（ready 後クラッシュ=stopped+error をエラー表示。reattach 終了でも setError）／2.B（Live "Fix with agent" で agent 不在なら案内、`command not found` を出さない）／4.A（attach URL のインライン検証＋無効時 Save 不可）／4.C（attach の setup ターミナルを永続キー `shell:<cwd>` 化＝閉じても `docker compose up` が死なない）／5.B（待機ボタンを「ターミナル」表記に）。
- **不変更**: QA 2.C は既に `sendDesignFeedback` が実行中 agent を kill する前に `confirmDialog` 表示済（対応不要）。
- **SEC-2（未信頼 repo の trust ゲート）は意図的に延期**: 自分の repo を動かす solo CEO には摩擦のみで便益なし。**配布が近づいた時点で専用 DEC として判断**（review のトリガーも「配布時」）。

**P2（磨き・安全な高価値サブセット）**
- **PE P2-2**: reattach 成功時も `httpFrameBlocked` を復元（戻ったとき空白でなく "open in browser" を出す）。
- **PE P2-3**: ターミナル cwd は**実行中**サーバーの packageDir（新 `runningPackageDir` state）に追従。設定だけ変えて未再起動でも別 dir に開かない。
- **QA 5.A / PE P1-4**: "Fix with agent" の前回 pty を再 spawn 前＋Live unmount で kill（各 `agent:<cwd>:<nonce>` 永続キー）。孤児 claude の累積を止める。
- **QA 4.D**: Issue attach 待機に解除ボタン（Live と parity）。**QA 1.A**: Live readiness を「このリポジトリを確認中…」ラベルに。**QA 5.C**: start 失敗ヒントを実在しない「下の OUTPUT」でなく「依存インストール／下のターミナル」を指すよう修正。
- **延期（perf/低リスク不一致）**: PE P2-6（detectApps キャッシュ＝staleness リスク）／PE P2-7（observer 再構築＝perf のみ・sync が delicate）／QA 3.A/3.B（app-picker smart default＝「後回し可」）。

- **検証**: tsc 0 / eslint 0 / vitest 69（+sanitizeLogTail 8・+nx-run・+3210 単独）/ cargo check / 本番 build → `ditto` → `/Applications/Bezier.app`（CEO は ⌘Q→再起動で反映）。e2e は CEO dogfood で確認。
- **チーム化前の TODO（コードでなく運用）**: feature ブランチ+PR+CI 必須化／usePreviewServer・Rust プローブのテスト追加。

---

## DEC-129 (2026-06-18) — 「起動済み URL を映す」アタッチモード（Docker/Rails/任意の外部スタック）

> CEO「mikan-for-school できないの？困る」→ 調査で **Docker Compose の Rails アプリ**（`docker compose up`→localhost:3000）と判明＝Node 単一 dev サーバーでないので自動検出外。DEC-128 の「未対応」表記は**誤り**（Bezier の engine はローカル URL を出すものなら何でも映せる）。CEO 選択＝**「起動済み URL を映す」モード**を新設。本番反映（20:27 ビルド）。

- **モデル**: `PreviewConfig.externalUrl?`（`.bezier/config.json`・loopback http(s) のみ）。**設定があれば attach モード**＝Bezier は dev サーバーを起動せず、その URL を 1.5s ごとに `httpPing` で監視し、応答したら EmbeddedBrowser で表示／未応答なら「待機」。devCommand より優先。
- **usePreviewServer**: attach 派生＋poll effect（up→ready+url、down→starting＝待機・ハードタイムアウト無し）。`start()` は attach 時 no-op。既存の managed pty があれば attach 時に drop。`attach`/`cwd` を公開。
- **UI**: ① Live の noApp カードを**reframe**（「自動検出できませんでした／ローカル URL を出すものなら何でも／dev コマンド or 起動済み URL を指定」＝DEC-128 の「未対応」断定を撤回）＋**URL 入力**（loopback 検証）→ saveConfig externalUrl。② attach 中は **待機カード**（「{url} を待っています — `docker compose up` で起動して」＋[ターミナル]＋[解除]）→ 起動で自動表示。③ Issue Preview の SettingsForm に **External URL 欄**＋待機カード。Live でも設定できる（DEC-128 の「Live に設定 UI 無し」も部分解消）。
- **再利用**: EmbeddedBrowser/parse_local_url（loopback URL 描画・DEC-120）／httpPing＋診断バナー（DEC-125）／saveConfig 永続化（DEC-122）／ボトムパネル Terminal（`docker compose up` を打つ場・DEC-126）。
- **新規**: `isLoopbackUrl()`（loopback http(s) 検証・test 付）。**変更**: preview.ts（externalUrl＋isLoopbackUrl）／use-preview-server（attach）／repo-live（reframe＋URL 入力＋待機）／preview-pane（External URL 欄＋待機）／i18n。
- **検証**: tsc 0/eslint 0/vitest 59（isLoopbackUrl 追加）/ cargo / next build green。e2e は CEO dogfood（mikan-for-school で URL 貼付→`docker compose up`→表示）で確認予定。
- **未対応**: リモート/非 loopback URL（＝共有の話）／Docker・DB の自動セットアップ（ユーザーが各スタックの初回設定）／URL 自動推測。

---

## DEC-128 (2026-06-18) — 配布前ハードニング P0：検出できないアプリで「無言の行き止まり」を止める（理由＋スコープ＋次の一手）

> CEO「配布が怖い。安全ハーネスはどれほど？」→ 対応マトリクス（`playbook/research/2026-06-18_preview-coverage-matrix.md`）で唯一“約束を裏切る”のが **非 Node／検出不可スタックの「無言で止まる」** と判明。その P0 を実装。本番反映（19:53 ビルド）。

- **問題**: 非 Node（Python/Ruby/Go/静的）や検出漏れの repo は、Live で `apps=[]`＆devCommand 空 → Run を押すと `dev コマンド未設定` という**意味不明エラー**で終わる（＝事実上の行き止まり）。
- **対応**: ① Live に **noApp 説明カード**＝`configLoaded && runner!=="tauri" && apps.length===0 && !devCommand` の時、Run ボタンの代わりに「ここで動かせる web アプリが見つかりません／Bezier は Node・JS の web アプリを起動します／これがそうなら Issue を作って Preview 設定で dev コマンドを指定／Python 等は未対応・ターミナルで自分で起動はできる」＋[ターミナルで起動]。② `previewServer.devCommandUnset` エラーを**スコープ明示**に書換（Run 失敗時も Issue Preview でも“なぜ動かないか”が出る）。
- **効果**: 「無言/意味不明で詰む」→「理由＋できること＋スコープ」に変換。配布時の最大の不安（非 Node の沈黙）を説明に変える。
- **変更**: `repo-live.tsx`（noApp 派生＋カード）／i18n `live.noAppTitle/Body`＋`previewServer.devCommandUnset` 改訂。tsc 0/eslint 0/vitest 57。次は P2＝実アプリ横断ストレステスト。

---

## DEC-127 (2026-06-18) — ワンクリック「Agent に直してもらう」＝エラー＋preview-doctor プレイブックをユーザー自身の Agent に渡す

> CEO「うまくいった後、配布したら色んな環境・状況がある。いちいち私とあなたで相談して環境調整するのはしんどい。**どうしたら見えるようになるかを、ユーザーごとの Agent に指示できる状況にしたい**」＋「ship（PR）した時にエンジニア目線で違和感ない？」。→ 今回うまくいった型＝「エラー可視化→エージェントに渡す→直す」を**ワンクリック化＋プレイブック化**し、各ユーザーが私に相談せず自走できるように。本番反映（18:50 ビルド・要 ⌘Q→再起動）。Plan = `~/.claude/plans/goofy-churning-turing.md`。要点メモ = [[live-preview-robustness]]。

- **形**: 診断バナーに **「Agent に直してもらう（Fix with agent）」** ボタン（notFound/serverError/empty 時）。押すと Bezier が持っている文脈（verdict＋HTTP status＋URL＋dev ログ末尾60行）に **preview-doctor プレイブック**を添えて、ユーザー自身の Agent に渡す。
- **preview-doctor プレイブック（恒久 IP・`prompts.ts` に bilingual）**: `previewDoctorPrompt({verdict,status,url,logTail})`＝原因クラスのチェックリスト（env 不足／auth ゲート／proxy・ポート／バックエンド未起動／node 版・依存）＋**制約（最小変更・dev 専用は .env.local・無関係コードは別 concern に切り出し・環境要因はユーザーに何を設定/起動すべきか伝える・最後に要約）**。→ **懸念1（PR 衛生）を仕組みで担保**＝エージェントに毎回「最小・分離・PR クリーン」を守らせる。
- **配送**: Issue=左 Chat の既存 `sendDesignFeedback`（claude を prompt 付き再起動／実行中は確認ダイアログ）。Live=チャット無いので、下パネル Terminal で `detectAgents()`→claude を doctor prompt 付き起動（`PreviewBottomPanel` に `terminalSpawn`＋`spawnNonce` を追加＝クリック毎に fresh 起動・keyed でタブ切替でも生存・wrap で終了後はシェルに戻る）。
- **`/bezier:doctor` コマンドも追加**（`commandPack` JA/EN、同じプレイブックを共有）＝`~/.claude/commands/bezier/` にインストールされ、Live のターミナルでも自分のターミナルでも手動で使える＝「各ユーザーの Agent に指示できる状況」の再利用可能版（既存インストールは Settings から再インストールで取得）。
- **エンジニア目線の判定（懸念1への回答）**: 今回の Clerk 修正＝`auth.protect({ unauthenticatedUrl: '/sign-in' })` は Clerk v6 の正式改善（未認証を 500 でなく /sign-in へ）＝ship 価値あり。ただし**サイドバー issue の PR には混ぜない**（別 commit/PR）。`.env.local` は gitignore で PR 不可視。これらをプレイブックがエージェントに守らせる。SCR は私が直接編集せず、この機能経由でユーザーの Agent が直す（第1号デモ）。
- **変更/新規**: `prompts.ts`（previewDoctorPrompt＋doctorBody＋doctor コマンド JA/EN）／`preview-diagnostic-banner.tsx`（onFixWithAgent prop＋Wand2 ボタン）／`preview-pane.tsx`（Issue=sendDesignFeedback）／`repo-live.tsx`（Live=detectAgents→spawn）／`preview-bottom-panel.tsx`（terminalSpawn/spawnNonce）／i18n `previewDiag.fixWithAgent`。
- **検証**: tsc 0 / eslint 0 / vitest 56 / cargo / next build green。e2e は CEO dogfood（SCR 500 で「Agent に直してもらう」→エージェントが Clerk を診断・修正）で確認予定。
- **未対応（据え置き）**: 無関係修正の別ブランチ/PR 自動化（プレイブックで指示＋ユーザーがレビュー）／プレイブックのマケプレ skill 化（§E）／バックエンドの自動起動。
- **追補（同 2026-06-18・本番19:26・dogfood）**: 診断バナーの**誤検知（false positive）を修正**。CEO「ログインしたらアプリは開いたのに 500 バナーが残る」。真因＝診断プローブ（`http_probe`）は**ログインセッション無しのサーバー GET**なので、(a) 認証アプリにログイン後もログアウト状態として 500/redirect を見続ける、(b) 起動時の一過性 500 を拾った後 URL 変化が無いと**再プローブされず stale**。対策＝`use-preview-diagnostic` を**起動スモークテスト**化：① 2.5s の slow 再プローブで stale/一過性を解消、② **good ページ（2xx/redirect=verdict null）を一度見たら settle して以後ドーマント**（ログイン後に認証ルートを誤検知し続けない）、③ ready 遷移で settle リセット。これで「アプリが出ているのに 500 バナー」が消える。SCR は **dev を `--turbo` 無しに切替（`.bezier/config.json` のローカル devCommand＝`npm run lingui:compile && npx next dev -p 4001`・チーム package.json 不変）→ Clerk 初回ハンドシェイクが出て `/`→`/sign-in`(307→200)→インラインログイン→アプリ表示**で解決（Turbopack だと Clerk dev-browser handshake interstitial が出ない＝エージェントが指摘した broader 原因。Bezier プレビュー側の起動設定で回避）。tsc 0/eslint 0/vitest 56。
- **追補2（同 2026-06-18・本番19:37・dogfood）**: Live は出たが **Issue Preview が `EADDRINUSE :::4001` で起動失敗**。真因＝追補1で SCR の devCommand に **`-p 4001` をハードコード**したため、Live(4001) と Issue worktree が**同じポートで衝突**（DEC-125 のポート尊重で Bezier は append しない→両方 4001）。さらに lingui のコンパイル成果物 `src/locales/*/messages.ts` は **gitignore**＝worktree に無いので devCommand は `lingui:compile && next dev` の**複合コマンド必須**。恒久対応2点：① **`buildDevCommand` を複合コマンド対応**＝`&&`/`;`/`||` を含む時は `--` を付けず**末尾に直接** `-p <freePort>` を付与（`--` は素の `npm run <script>` 専用。複合だと flag が誤爆していた）。② SCR config の devCommand を **`npm run lingui:compile && npx next dev`（ポート指定を削除）**に→ Bezier が各プレビューに**個別の freePort** を付与（Live と Issue で別ポート＝衝突解消）。これで Live も Issue Preview も turbo 無し・別ポート・lingui 済みで起動。tsc 0/eslint 0/vitest 57（buildDevCommand 複合テスト追加）。

---

## DEC-126 (2026-06-18) — Live/Preview の下に VS Code 風ボトムパネル（OUTPUT＋Terminal）＝「見る→直す」を1画面で

> CEO「もはや terminal を開けるようにしても良い..？Live や preview のページの下部に（VS Code のように）terminal を表示できて、そこで Claude Code に修正を依頼する流れが自然かも」。→ DEC-125 で「空白の理由」を出せるようになった次の一手＝**プレビュー直下に terminal**。特に **Live にはエージェントチャットが無い**ので価値大。本番反映（17:12 ビルド・要 ⌘Q→再起動）。Plan = `~/.claude/plans/goofy-churning-turing.md`。要点メモ = [[live-preview-robustness]]。

- **形（CEO 選択）**: 1つの**リサイズ可能ボトムパネル**にタブ **[OUTPUT][Terminal]**（DEC-125 追補の暫定 OUTPUT ドロワーを統合）。Terminal は**素の対話シェル**（`claude`/`git`/`npm` を自分で打つ＝「Run Claude」ボタンや自動起動は不採用）。
- **新規 `preview-bottom-panel.tsx`（共有）**: 高さ記憶（`bezier:preview-panel-height`）＋ドラッグハンドル（repo-live の `onLogResizeStart` パターン流用）＋タブバー＋×。OUTPUT=`log` を dark な ScrollArea/pre。Terminal=`TerminalPane`（dynamic ssr:false）を **active タブの時だけ**マウント、`sessionKey="shell:${cwd}"` で**永続**（タブ切替・パネル閉じ・画面移動を跨いで再アタッチ＋backlog 再生／agent pty=issue.id とはキー衝突しない）。cwd 無しは「dev サーバーを起動して」と案内。
- **配線**: `usePreviewServer` に `cwd`（=`packageCwd(worktreePath, packageDir)`＝dev サーバーが動く実ディレクトリ）を公開。`preview-pane` は暫定ドロワーをこのパネルに置換（`panelOpen`/`panelTab`・ヘッダの Output ボタン＝トグル・バナー「Show output」→OUTPUT タブで開く）。`repo-live` は **ready 状態**でヘッダにトグル＋本体下にパネル（not-ready の readiness/SetupTerminal フローは不変＝スコープ外）。
- **ネイティブ webview との両立**: パネルは `shrink-0` の兄弟＝本体 `flex-1` が縮む→`embedded-browser` の ResizeObserver がスロット矩形を再ミラー＝webview が上に収まる（DEC-125 追補の OUTPUT ドロワーで実証済の挙動）。Terminal は `role=dialog/menu/listbox` でないので overlay-freeze を誘発しない。
- **流れ**: 500 バナー→「Show output」で OUTPUT に原因スタック→Terminal タブで `claude "このエラー直して"`→タブを往復してもセッション維持。非エンジニアでも「見える→直す」が1画面で完結。
- **検証**: tsc 0 / eslint 0 / vitest 56 / cargo（Rust 変更なし）/ next build green。e2e は CEO dogfood（⌘Q→再起動）で確認予定。
- **未対応（据え置き）**: 複数ターミナル/分割／「Run Claude」ボタン／Live の not-ready ログ＋SetupTerminal の統合／500 スタックの自動 claude 投入。
- **既知トレードオフ**: shell pty（keyed）はアプリ稼働中は生存（agent pty 同様）＝パネルを閉じても `claude` は走り続ける（再オープンで戻る）。掃除は将来。

---

## DEC-125 (2026-06-18) — 「どんな repo でも開ける」恒久対応＝プレビューを自己診断化＋モノレポのアプリ選択＋ポート尊重

> CEO dogfood：SCR の `new-frontend` を開いたら Live も Preview も「動いてる様子がない（TanStack アイコンだけ）」。→ さらに CEO「**いろんな環境で使おうとして必ず何かにぶつかる。場当たりでなく、使い勝手を考えた恒久対応を**」。調査で判明した本質＝**原因は環境ごとに違うのに、全部が『無言の空白』に収束**している（`httpPing` はどの HTTP ステータスでも true・`frameBlocked` は計算済みなのに未表示・モノレポは先頭一致で誤選択）。恒久原則＝**プレビューは status surface：描画できない時は必ず理由と次の一手を出す／本当のノブ（どのアプリ・どのパス・どのポート）は可視で調整可能に**。フル 3 フェーズで実装。本番反映（16:08 ビルド・要 ⌘Q→再起動）。要点メモ = [[live-preview-robustness]]。Plan = `~/.claude/plans/goofy-churning-turing.md`。

- **Phase 1（要）＝空白の理由を必ず提示（自己診断）**: ready 後に**実際に読み込まれた URL を GET 検証**し、404/5xx/空 を**非ブロッキングの dismissible バナー**で説明（webview はそのまま表示＝カスタム 404 ページも見える／200 HTML に着いたら自動で消える）。
  - Rust: 既存 `http_frame_blocked` のソケットコードを `http_probe_inner` に抽出し `http_probe(url) → {status, frameBlocked, contentType, bodyLen}` を追加（依存追加なし・`http_frame_blocked` はそれの薄いラッパに）。`generate_handler!` 登録。
  - TS（`preview.ts`）: `httpProbe`（失敗時 null＝直前の verdict 維持）＋純粋分類器 `verdictFor`（≥500→serverError／404・401・403→notFound／3xx→null＝最終URLが決める／2xx で本文<200B か非HTML→empty／それ以外 null）。**frameBlocked は型/UI/i18n に残すが verdictFor は発火させない**（DEC-120 のネイティブ top-level webview は X-Frame-Options を無視＋SAMEORIGIN が普遍＝誤検知源）。
  - フロント: 共有フック `use-preview-diagnostic.ts`（DEC-121 の `onNavigate` に相乗りして再プローブ・400ms debounce・同一オリジンのみ・OAuth 外部オリジンは verdict クリア・ready で初回プローブ・dismiss）＋ `preview-diagnostic-banner.tsx`（`role="status"` の `shrink-0` バーを webview スロットの**上**に置く＝ネイティブ webview は HTML を重ねられないため／freeze も誘発しない）。repo-live と preview-pane の両方に配線。
  - **限界（コードに明記）**: クライアント描画 SPA の「200 だが JS エラーで空白」はサーバ視点では見えない＝対象外。
- **Phase 2＝モノレポのアプリ選択（ideas-backlog §G 解消）**: `detectApps(dir)`（root＋全直下サブdir＋`packages/apps/prototypes/examples` の1段下を走査し、dev スクリプトを持つ全アプリを `{packageDir, scriptName, framework, frameworkVersion, hasEnvLocal, mtime}` で返す）＋ `pickDefaultApp`（`.env.local` 有無→フレームワーク major 新しさ→更新時刻）。`use-preview-server` が**保存 packageDir があればそれ、無ければ smart default** を採用（先頭一致 detectDev は detectApps が空の時のフォールバックに降格）。`apps`/`selectApp` をフックから公開し、新規 `app-picker.tsx` ドロップダウンを Live と Preview のヘッダに（複数アプリ時のみ表示）。選択で packageDir を `.bezier/config.json` に保存＋稼働中なら再起動。**SCR 実証**: `frontend`(Next13)/`new-frontend`(Next15・.env.local)/`pir-client`(vite) を検出し **new-frontend を既定選択**（従来は `frontend` を誤選択）。
- **Phase 3＝ポート尊重**: `buildDevCommand` は dev コマンドが既に `-p`/`--port`（空白/`=` 形）を持つ時は**何も足さない**（`parseDevServerUrl` が実バインドポートを読むので検出は追従）。`next dev -p 4001 … -p 54460` の重複を解消＝アプリは作者意図の 4001 で起動。
- **検証**: tsc 0 / eslint 0 / vitest 56（preview.test に verdictFor＋buildDevCommand を追加）/ cargo Finished / next build green。実機 curl で new-frontend `/`=404（→notFound バナー）・`/sign-in`=200（→バナー無し）を確認。e2e は CEO dogfood（⌘Q→再起動）で確認予定。
- **新規ファイル**: `use-preview-diagnostic.ts` / `preview-diagnostic-banner.tsx` / `app-picker.tsx`。**変更**: `src-tauri/src/lib.rs`・`preview.ts`・`use-preview-server.ts`・`repo-live.tsx`・`preview-pane.tsx`・i18n en/ja。
- **再利用（作り直さず）**: DEC-121 onNavigate / DEC-120 ネイティブ webview / DEC-122 config 永続化 / `FreshnessBanner` レイアウト / `list_dir`・`pathMtime`。
- **未対応（据え置き）**: SPA クライアント空白／非 Node スタック（Python/Go/static）／asdf・fnm・volta（DEC-112 残）。
- **追補（同 2026-06-18・本番16:48・dogfood 即フィードバック）**: 診断バナーは出たが CEO「**500 と出たけどどうすれば良いか分からない**」。真因＝**Issue Preview には稼働中の OUTPUT ログが無い**ので「check the OUTPUT log below」が dead-end だった（Live にはリサイズ可能ログがあるが Preview は ready 時にログ非表示）。修正＝Preview に **OUTPUT ログ・ドロワー**を追加（ヘッダの「Output」トグル＋バナーの「Show output」ボタン＝serverError/empty 時、で開く・下部に dev サーバーログ表示・× で閉じる）。これでバナーの指す「ログ」が実在＝行き止まり解消。CEO は 500 の実エラー（その route のサーバー側スタック＝多くはバックエンド未起動 or データ取得失敗 or 新規コードのバグ）を見て、エージェントに修正依頼できる。非エンジニアでも「見える→渡せる」。tsc 0/eslint 0/i18n parity OK。`preview-pane.tsx`＋`preview-diagnostic-banner.tsx`（`onShowLog` prop）＋ i18n（`preview.output/outputTip`・`previewDiag.showLog`）。

---

## DEC-124 (2026-06-18) — ⌘⇧↑↓ は「issue のみ」を移動（Live をキーボードナビ対象から外す）

> CEO dogfood：「（DEC-123 の遅延を入れても）やっぱりもたつきを感じる。なので shortcut では issue だけを開くようにしよう」。→ DEC-123（プローブ遅延）に続く根治。**⌘⇧↑↓ のナビ対象から Live エントリを外し、issue だけを横断**する。Live はリポ名クリックで従来どおり開ける。本番反映（11:41 ビルド・要 ⌘Q→再起動）。要点メモ = [[live-preview-robustness]]。

- **なぜ DEC-123 だけでは不足だったか**: DEC-123 は「Live を表示した時の**プローブ**」を遅延しただけ。⌘⇧↑↓ で Live エントリに乗ると依然として `selectLive`→`switchTo`（root 変更）→ `<RepoLive key={root}>` が**可視のまま再マウント**し、Live の UI 描画（readiness シェル等）＋場合により EmbeddedBrowser ネイティブ webview の再生成が走る。これが残りの「もたつき」。**Live に乗らなければこの可視再マウントが一切起きない**＝根治。
- **変更（`app-sidebar.tsx`）**: ① `navTargets`（live＋issue 混在）を **`issueNav`（issue のみ）** に置換。全 repo を描画順に走査し、検索/ページネーション（showAll/PAGE）は踏襲、`expanded` ゲートは撤去。② `moveSelection` を issue 専用に: issue 上なら隣の issue へ、Live/ホーム（selectedId 無し）上なら**アクティブ repo を基準に** ↓＝その repo の最初の issue／↑＝前 repo の最後の issue（recents 順で算出）。着地時に `expandRepo` でその repo を開いて行を可視化。③ issue を全 repo で**先読みロード**（従来は expanded＋検索時のみ）— `listIssues` は 1 IPC/repo と軽く、これで折り畳み repo の issue にもキーボードで届く＋折り畳み中も件数バッジが正確に。
- **i18n**: `shortcuts.sidebarDown/Up` を「次/前の Issue へ（リポジトリ横断）」に更新（en/ja）。
- **DEC-123 は維持**: プローブ遅延は併存して有効。issue ナビで repo を切り替えても RepoLive は**裏（hidden, active=false）**で remount＝`probeEnabled` false のままプローブゼロ。DEC-123＋124 で issue ナビ中の Live コストは実質消える。
- **トレードオフ**: Live はキーボードで到達不可に（リポ名クリックで開く）＝CEO が明示的に選択。全 repo 先読みは件数バッジが全 repo に出る副作用あり（情報増＝許容）。
- **検証**: tsc 0 / eslint 0 / vitest 44 / next build green。e2e は CEO dogfood（⌘Q→再起動後、⌘⇧↑↓ で repo 跨ぎに issue 間を流す＝Live を踏まず滑らか）で確認予定。

---

## DEC-123 (2026-06-18) — Live の重い health プローブを表示時まで遅延（⌘⇧↑↓ で Live を通過してももたつかない）

> CEO dogfood：「⌘⇧↑↓ でサイドバーを行き来できるようにしたが、Live がどうしても重い。以前より軽くなったけど他の issue に行く時にラグを感じる。Live をもっと軽くできるなら軽量化を、無理なら shortcut は issue だけにしてもいい」。→ CEO 選択＝**Live はナビ対象に残して軽量化**。本番反映（11:01 ビルド・要 ⌘Q→再起動）。要点メモ = [[live-preview-robustness]]。

- **真因**: ⌘⇧↑↓ のナビ列は「各 repo の Live → その repo の issue 群」を交互に並べる（`app-sidebar.tsx` の `navTargets`/`moveSelection`）。別 repo の issue へ行くには Live エントリに**一度乗る**→ `selectLive` → `switchTo`（root 変更）→ `<RepoLive key={root}>` が**可視のまま再マウント**。その瞬間に repo-health プローブ3つがクリティカルパスで走る＝`useRepoFreshness`→`gitFetch`(ネットワーク)／`useReadiness`→`probeReadiness`(Node 検出で **pty プロセス起動**)／`useSetupSignals`(FS スキャン)。DEC-113 で「裏に隠して常時マウント＝同 repo の Live⇆Issue 即切替」は済んでいたが、**repo 切替の再マウント**ではプローブを毎回払い直していた。
- **対応＝表示＋落ち着き時のみプローブ（`active` + 200ms settle ゲート）**:
  - `page.tsx`：`<RepoLive active={!detail}>`（issue/trash が上に乗っていない＝Live が表示層の時だけ active）。
  - `repo-live.tsx`：`probeEnabled` を「active かつ 200ms 留まった時だけ true」にする settle タイマー。**Live を通過するだけ（200ms 以内に離れる）→ タイマー解除でプローブは一切走らない**／留まれば一拍おいて実行（シェルは即描画済み）。離脱時は cleanup で false に戻し、復帰時は再 settle。
  - 3つの hook に `enabled = true` 引数を追加し、初回プローブ effect（と useReadiness の focus 再プローブ）を `!enabled` で early-return。`enabled` は deps にも追加。default true で後方互換。
- **影響範囲（閉じている）**: 3 hook は **RepoLive 専用**・RepoLive は **page.tsx 専用**（grep 確認済）。EmbeddedBrowser の温存（DEC-113）と同 repo の Live⇆Issue 即切替は**不変**（プローブだけをゲート、描画はゲートしない）。
- **トレードオフ（許容）**: サイドバーのバッジ（needs setup / update available）の**即時**更新は「その repo の Live を開いた時」になる。サイドバー自前の低速プローブループが結局は追従するので軽微（`repo-live.tsx:80–82` のコメント参照）。
- **対象外（次回メモ）**: Live 表示中＋サーバー稼働時の EmbeddedBrowser の `MutationObserver(body, subtree)` は常時コストだが今回のナビ・ラグとは別系統＝据え置き。
- **検証**: tsc 0 / eslint 0 / vitest 44 / cargo Finished / next build green。e2e は CEO dogfood（⌘Q→再起動後、⌘⇧↑↓ で repo 跨ぎ issue 移動＝Live でもたつかない／Live に留まると一拍で readiness 表示）で確認予定。

---

## DEC-122 (2026-06-18) — Preview の dev コマンドが再起動で「消える」問題を恒久解決（検出を root フォールバック＋解決済み config を必ず永続化）

> CEO dogfood：「プレビューした後に ⌘ で閉じてもう一度開くと、プレビュー用のコマンドが消えていることがよくある。『コマンドがないから実行できない』で終わる。ちゃんとキャッシュ等で保持できてる？何度再起動しても絶えず使えるようにして」。→ 真因を特定して2点で恒久対応。本番反映（10:24 ビルド・要 ⌘Q→再起動）。要点メモ = [[live-preview-robustness]]。

- **真因（lenz で再現・2層）**: ① **lenz は `.bezier/config.json` を一度も保存していなかった**（毎回 detection 頼み）。② lenz は **prototypes-monorepo の git リポの一部**＝issue の worktree は**モノレポ root の checkout**で、lenz は `prototypes/lenz`（2階層）にある。`detectDev` は root＋直下の `app/web/frontend/client/site` しか見ない＝**lenz が見つからず devCommand 空**。さらに `subPath`（root が git toplevel からの相対 = `prototypes/lenz`）の解決は**非同期で初期値 ""**＝マウント直後の一瞬 `worktreePath` がモノレポ root を指す**レース**もあり、検出失敗が「よく」起きていた。保存が無いので**再起動のたびにこのサイコロを振り直していた**。表示上は「dev command 欄に `npm run dev`、packageDir 欄に `app`」に見えたが、両方とも**プレースホルダ**（空 config）で、実体は `{devCommand:"", …}`。
- **Fix A（検出を root フォールバック）**: worktree 検出が dev スクリプトを見つけられなかった時、**実 repo root（`detectDev(root)`）にフォールバック**。dev スクリプトは**コミット済み**なので root は常に信頼できる供給源（Live は `worktreePath===root` なので影響なし＝issue worktree のみフォールバック追加）。lenz なら `detectDev(lenz)` が root の `next dev` を発見＝レース中でも空にならない。
- **Fix B（解決済み config を必ず永続化＝CEO の言う「キャッシュ」）**: config-load で解決した usable な config を `<root>/.bezier/config.json` に**書き出す**。detection 任せだった構成も保存されるので、次回起動で detection が走れなくても（worktree 未準備でも）**ディスクからコマンドが復元**。書込みは**差分があり、かつ空コマンドの新規作成でない時だけ**（冪等＝`.bezier` は gitignore でチャーン無し）。旧「packageDir 自己修復」write はこれに包含。
- **変更ファイル**: `app/src/components/issues/use-preview-server.ts` のみ（config-load effect に root フォールバック検出＋永続化ロジック）。`writePreviewConfig`/`detectDev` は既存。
- **これで満たすこと**: 初回オープン時点で（保存ゼロでも）Fix A がコマンドを埋め、Fix B が即永続化 → **2回目以降は detection の状態・レースに関係なくディスクから確実にロード**＝「何度再起動しても絶えず使える」。残るレース＝Start を subPath 解決前(〜数十ms)に押すと cwd がモノレポ root になり別エラー（"npm run dev" 失敗）になり得るが、worktreePath 変化で自動再解決＝自己修復（別系統・軽微）。
- **検証**: tsc 0 / eslint 0 / vitest 44（preview.test 含む）/ cargo Finished / next build green。実機の lenz で `.bezier/config.json` に `{devCommand:"npm run dev",port:4155,packageDir:""}` が保存されていることを確認（再起動後はここからロード）。e2e は CEO dogfood（⌘Q→再起動）で確認予定。

---

## DEC-121 (2026-06-18) — Preview のルートパス入力機構を修正＋ナビゲーションでアドレスバー自動同期

> CEO dogfood：「チャットが『/coach/preview を開いて』と案内してるのに、右のプレビューのパス入力欄に `/coach/preview` を入れても何も変わらない。ルートパス入力の機構を見直して、ちゃんと動くようにして。希望は2点＝①プレビュー内のページ遷移に応じてパス表示が自動で切り替わる ②直接パス入力でそのページを表示」。→ DEC-120 の埋め込みネイティブブラウザに**アドレスバーの双方向同期**を実装。本番反映（10:02 ビルド・要 ⌘Q→再起動）。要点メモ = [[live-preview-robustness]]。

- **真因（なぜ「変わらない」に見えたか）**: 入力→ナビ自体は動いていた疑いが濃いが、(a) chom-chom の `/coach/preview` は**ログイン必須**で未ログインだと `/`（ログイン画面）へリダイレクト＝結果が同じ画面に戻り「変わらない」ように見える。(b) そもそも**「今どのパスにいるか」を webview から読み戻す経路が無かった**ため、リダイレクトされても**バーが更新されず**、機構が壊れているように見えた。①の自動同期を入れると「`/coach/preview` 入力→`/` に戻った」がバーに出て、機構が動いている事実が可視化される。
- **① ナビ自動同期（新規）**: Rust に `embed_browser_url`（`wv.url()` を返す）を追加。フロント（`embedded-browser.tsx`）が **300ms 間隔で現在 URL をポーリング**し、変化したら `onNavigate(url)` を発火（webview 表示中のみ）。`wv.url()` は WKWebView 由来で**フルナビ（リンク/リダイレクト/OAuth 復帰）も SPA の `pushState` も**反映するので、リンク追従・認証リダイレクト・OAuth 復帰すべてでバーが追従。`preview-pane.tsx` が **同一オリジンのときだけ** `pathname+search+hash` を `path`/`pathDraft` に反映（OAuth 中の外部オリジンは在宅パスに表現できないので無視＝最後の in-app パスを保持）。**ユーザーが入力欄を編集中（focus 中）は `pathDraft` を上書きしない**ガードあり。
- **② 直接入力ナビ（堅牢化）**: `EmbeddedBrowser` のナビゲーションを **`src` 変化から切り離し**、**`reloadKey` バンプ時のみ navigate**（マウント初回はスキップ＝`add_child` が既に `src` ロード済み）。理由＝①の自動同期は `path`→`src` を書き換えるので、もし `src` 変化で navigate すると**自分のナビで webview が跳ね返る／ループ**する。入力欄 submit とリロードボタンだけが `reloadNonce` をバンプ＝意図したときだけ navigate。
- **変更ファイル**: `app/src-tauri/src/lib.rs`（`embed_browser_url` ＋ `generate_handler!` 登録）／`app/src/lib/ipc.ts`（`embedBrowserUrl`）／`app/src/components/issues/embedded-browser.tsx`（`onNavigate` prop・URL ポーリング・navigate を reloadKey 駆動に）／`app/src/components/issues/preview-pane.tsx`（`onEmbedNavigate` マッピング・focus ガード・パス入力欄を `w-28→w-36`）。Live（`repo-live.tsx`）は今回アドレスバー無しのため `onNavigate` 未配線（任意 prop）。
- **検証**: tsc 0 / eslint 0 / vitest 44 / cargo Finished / next build green。e2e は CEO dogfood（⌘Q→再起動後）で確認予定。
