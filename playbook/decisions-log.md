...
**追補（#5 attach-first・設計確定＋staging）**: CEO 体験像＝「**AI Agent が能動的に dev サーバ起動 → Bezier が検知したら自動でブラウザを開く**／手動 URL はフォールバック（cmux 型）」。自動検知は **両方（ターミナル parse＋ポート探索）** で確定。真因＝表示は DEC-120 で既に webview、脆いのは「任意スタックの dev サーバ自動検出/起動」。**最も sensitive な subsystem（DEC-120〜130）を触る大改修＋実機検証不可**のため、丁寧に staging：①エージェント起動＋URL報告（プロンプト規約・低リスク）を**先行実装・出荷**／②自動検知（待機状態で `parseDevServerUrl`/エージェント pty parse → 無ければ common port を `httpPing` 探索→ attach・auto-open）＋③attach-first UI（待機が主役・手動URL＋自動起動はフォールバック）は**専用パス**。確定設計＝`playbook/research/2026-06-22_preview-attach-first-spec.md`。①でエージェントは「Bezier はサーバを起動しない・起動済みを映す・URL を報告して」を毎ターン認識する。
