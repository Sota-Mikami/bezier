...
- **未決（次に詰める）**: 「2つの localhost」の中身＝(a) 同一 issue の **before 状態 vs 現在**（＝2つ目の worktree＋dev サーバを古い状態で起動・重い）／(b) **別 issue を 2 枚**（§C スプリットビュー）／(c) 単に現在プレビューの**柔軟タイル**（同一サーバ・ルート違い等）。アーキ（2サーバの是非）が分かれるので CEO に確認してから着手。
- 関連: ideas-backlog §C スプリットビュー（2 issue 並べる）と合流の可能性。
