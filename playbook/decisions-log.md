...
- **IME二重（①）**: ASCII では起きず日本語のみ＝composition 起因。xterm **6.0.0 は出たばかりの最新メジャー**で composition 回帰の疑いが濃厚。実績ある **5.5.0 に下げる**（addon-fit 0.10.0 / addon-web-links 0.11.0）。API（`options.theme` live 代入・`attachCustomKeyEventHandler`・`ITheme`）は 5.x でも同一、tsc/build 通過。
- **要検証**: IME 二重が 5.5.0 で解消するかは実機目視（headless 再現不可）。直らなければ深掘り（Claude TUI の bracketed-paste 等）。改行は確実。
