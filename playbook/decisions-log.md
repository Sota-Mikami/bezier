...
- **唯一の残り**: `.bezier` 活動ログの**自由ノート**（"案3を生成"・"衝突N ファイル" 等、use-implement-session の logEvent 5箇所）。**書込時 tt() は不可**（保存先が固定言語に凍結）→ **描画時構造化リファクタ**（thread に {type,params} を保存し render で t()）が前提。イベント**ラベル**は既に locale 追従（History/PR body）。
