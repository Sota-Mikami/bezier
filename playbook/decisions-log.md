...
- 検証: tsc 0 / eslint 0 / vitest 36（journey 6＝ハッシュロック＋タスクリスト含む）/ build green / gitleaks クリーン。コミット `f587f46`→`7bd227c`（deploy 2段）/ `cac4227`（journey UX）。本番 = **10:21 ビルド**。**要 ⌘Q→再起動**。
- **残・CEO 実機確認**: project 作成 → `deploy --project` 解決は CLI で実証済み。未検証は**リモートビルドが通って URL が返るか**（＝ユーザーのアプリがビルドできるか＝Bezier 外）。再共有で Preview/Map/QA が乗るか確認。なお当該 issue の project（`01kv8gg82mysdxpa90swvksy23-app`）は調査時に先行作成済み。
