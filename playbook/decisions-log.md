...
- **移行スコープ（重要・別途確定）**: 対象は **UI クローム**（ボタン/ラベル/ダイアログ/設定）。**agent 向けテキスト（プロンプト buildPrompt・Spec テンプレ・コミットメッセージ・`.bezier` 内容）は対象外**＝機能/内容であり、agent への指示言語は別 UX 判断（推奨: 当面 as-is）。
- **進捗**: 基盤＋第1スライス（top bar / History ドロワー / 設定上段）を移行。残り ~45 ファイルは面ごとに順次。tsc/eslint/vitest(22)/build 全 green。dev のみ。
